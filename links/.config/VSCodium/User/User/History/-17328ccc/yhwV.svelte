<script>
	...
	$seo.title = isOwner
		? $_('pageTitles.account')
		: $_('pageTitles.publicProfile') + ' ' + viewingUser.username;
	...
	async function follow() {
		...
		if (user.following.includes(viewingUser.id)) {
			// Removing the user from the followings
			user.following = user.following.filter((u) => u !== viewingUser.id);
			await pb.collection('users').update(user.id, { following: user.following });
		} else {
			// Adding the user to the following
			user.following = [...user.following, viewingUser.id];
			await pb.collection('users').update(user.id, { following: user.following });
		}
		isFollowing = false;
	}
...
<div
	class="flex flex-col sm:flex-row items-start gap-2 lg:p-4 p-2 transition-all sm:pl-0 {isOwner
		? 'pt-0'
		: 'lg:h-[108px] md:h-[70px] mb-2 max-sm:-mt-10'}"
>
	{#if !isOwner}
		<!-- Profile picture -->
		<div
			class="rounded-full transition-all lg:h-36 md:h-24 h-16 ring-[5px] ring-gray-0 dark:ring-gray-950 sm:-translate-y-1/3 aspect-square bg-center bg-cover"
			style="background-image: url({getImageUrl(
				viewingUser.collectionId,
				viewingUser.id,
