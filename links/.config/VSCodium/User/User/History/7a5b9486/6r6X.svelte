<script>
	...
	function handleTouchEnd(event) {
		if (!currentCard?.startX) return;
		const diffX = currentCard.currentX - currentCard.startX;
		const endCardPos = window.innerWidth / 2 + 225 * (diffX > 0 ? -1 : 1);
		console.log(endCardPos);
		...
		if (Math.abs(diffX) > event.target.closest('.articleSwipeCard').clientWidth / 4) {
			...
			const rotation = endCardPos / CARD_ROTATION_FACTOR;
			currentCard.transitionDuration = endCardPos;
			currentCard.transform = `translateX(${endCardPos}px) rotate(${rotation}deg)`;
