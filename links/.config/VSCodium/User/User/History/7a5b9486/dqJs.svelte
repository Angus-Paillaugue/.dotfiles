<script>
	...
	function handleTouchMove(event) {
		...
		currentCard.opacity = 1 - Math.abs(diffX) / window.innerWidth / 2;
	}
	...
	function handleTouchEnd(event) {
		...
		if (Math.abs(diffX) > cardWidth / 4) {
			...
			currentCard.opacity = 1 - Math.abs(endValue) / window.innerWidth / 2;
			setTimeout(() => {
				if (items.length > 0) {
					items = items.slice(1);
					currentCard = items[0];
		...
		{:else if currentCard}
			{#key currentCard.url}
				...
				<div
					...
					style="transform: {currentCard.transform || 'translateX(0) rotate(0)'}; transition-duration: {currentCard.transitionDuration ?? '75'}ms; opacity: {currentCard.opacity ?? '1'};"
