...
class RencontreDAO {
  ...
  public function getNext($limit) {
    ...
    $sql = 'SELECT * FROM rencontre WHERE date_heure < NOW() ORDER BY date_heure ASC '.$limit.';';
  ...
  public function getPrevious($limit) {
    ...
    $sql = 'SELECT * FROM rencontre WHERE date_heure < NOW() ORDER BY date_heure DESC '.$limit.';';
    $rows = $this->conn->run_query($sql);
