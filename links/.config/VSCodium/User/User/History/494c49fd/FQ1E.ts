...
export function hasEveryPermission(role: string | undefined, ...permissions: string[]): boolean {
  ...
  if(permissions.includes(PERMISSIONS.ALL) || permissions.includes(PERMISSIONS.ANY)) {
    return true;
  }
...
export function hasPermission(role: string | undefined, permission: string): boolean {
  ...
  if(permission.includes(PERMISSIONS.ALL) || permission.includes(PERMISSIONS.ANY)) {
    return true;
  }
  if (!rolePermissions[role]) {
    return false;
