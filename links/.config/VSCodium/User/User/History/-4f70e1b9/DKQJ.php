...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) .
  ...
  http_build_query(
    $_GET
  ); ?>" class="max-w-screen-xl w-full mx-auto p-4 rounded-xl border space-y-4 border-primary-100">
  <h1>
    Modifier un commentaire
  </h1>
  <textarea name="commentaire" id="commentaire" class="w-full h-32 border border-primary-100 rounded-lg p-2" placeholder="Entrez votre commentaire ici"><?php echo $commentaire->getContenu(); ?></textarea>
