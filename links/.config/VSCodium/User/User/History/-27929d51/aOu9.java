          ...
          for (Component subComponent : subPanel.getComponents()) {
            if (subComponent instanceof JCheckBox) {
              ...
              if (checkBox.isSelected()) {
                ...
                Personne locataire = this.vue.logement.getColocation().getContrats().stream().findFirst((location) -> location.getLocataires().stream().findFirst((loc) -> loc.getIdLocataire() == idLocataire).isPresent()).get().getLocataires().stream().findFirst((loc) -> loc.getIdLocataire() == idLocataire).get();
