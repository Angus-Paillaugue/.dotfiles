...
#define MAXPORTLEN 6
#define FTP_SERVER "ftp.fau.de"
#define FTP_PORT "21"

void debug(const char *format, ...) {
  va_list args;
  va_start(args, format);
  printf("[\033[0;34mDEBUG\x1B[0m] ");  // Add "DEBUG" in blue using ANSI
                                        // escape codes
  vprintf(format, args);
  va_end(args);
}
...
void debug_err(const char *format, ...) {
  va_list args;
  va_start(args, format);
  printf("[\x1B[31mERR\x1B[0m] ");  // Add "ERR" in red using ANSI escape codes
  vprintf(format, args);
  va_end(args);
}
...
int handle_ftp_proxy(int client_sock) {
  ...
  connect2Server(FTP_SERVER, FTP_PORT, &ftp_sock);
...
int main() {
  ...
  // Récupération du nom de la machine et du numéro de port pour affichage à
  // l'écran
