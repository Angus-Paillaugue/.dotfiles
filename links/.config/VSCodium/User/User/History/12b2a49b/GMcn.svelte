<script lang="ts">
	...
	function search(val: string) {
		...
		// searchFocusIndex = searchFocusIndex >= searchResults.length ? searchResults.length - 1 : searchFocusIndex;
	}
	...
	function onSearchKeyDown(e: KeyboardEvent) {
		if(e.key === 'ArrowDown') {
			...
			// Play the song
			$currentlyPlayingSong = searchResults[searchFocusIndex];
			searchModalOpen = false;
		}
	}

	$inspect(searchFocusIndex)
</script>
	...
	<div class="fixed inset-0 z-50 py-8 px-2 pointer-events-none">
		<div class="max-w-screen-md flex flex-col mx-auto w-full gap-8">
			...
			<div class="bg-background border border-border p-2 rounded-2xl flex flex-row gap-4 items-center pointer-events-auto" transition:fly={{ duration: 300, y: '-100%' }}>
				<Input class="grow" type="text" name="search" placeholder="Search for a song" onkeydown={onSearchKeyDown} bind:value={searchValue} bind:this={searchInput} />
			...
			{#if searchResults.length > 0}
				...
				>
					{#each searchResults as s, index (s.id)}
						<Track song={s} onclick={() => (searchModalOpen = false)} data-focused={searchFocusIndex === index} />
					{/each}
				</div>
			{/if}
