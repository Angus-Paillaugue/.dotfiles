<script lang="ts">
  ...
  let fakeCursorPos = spring({ x: 0, y: 0 },
    {
      stiffness: 0.3,
      damping: 0.35,
    }
  );
  let fakeCursorNameTagPos = spring({ x: 0, y: 0 },
    {
      stiffness: 0.03,
      damping: 0.25,
    }
  ...
  onMount(() => {
    fakeCursorContainer?.addEventListener('mousemove', (e) => {
    ...
    fakeCursorContainer?.addEventListener('mouseleave', () => {
      fakeCursorOpacity = 0;
    });
  });
</script>
...
<div
  class="flex w-full flex-col items-center justify-center lg:justify-between rounded-2xl bg-secondary px-4 pt-12 md:px-6 md:pt-24 min-h-[calc(100dvh-4rem-1rem-50px)]"
>
  <div class="space-y-4 text-center text-wrap-balance md:w-2/3">
    <WordAnimate words="Transform Your Logs into Actionable Insights" />
    <h2 class="text-base md:text-xl font-base text-muted-foreground">
  ...
  <div class="grow w-full h-full flex flex-col max-lg:hidden">
    <div class="p-3 aspect-video w-full mt-auto translate-y-14 rounded-3xl border border-primary bg-background relative overflow-hidden">
      <div class="absolute inset-0 bg-primary/15 -z-10"></div>
      <div class="bg-background overflow-hidden rounded-2xl shadow size-full relative cursor-none hover:cursor-none" bind:this={fakeCursorContainer}>
        <div class="absolute z-10 transition-opacity flex flex-col" style="top: {$fakeCursorPos.y}px; left: {$fakeCursorPos.x}px; opacity: {fakeCursorOpacity};">
          <svg xmlns="http://www.w3.org/2000/svg" class="size-6 fill-background" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4.037 4.688a.495.495 0 0 1 .651-.651l16 6.5a.5.5 0 0 1-.063.947l-6.124 1.58a2 2 0 0 0-1.438 1.435l-1.579 6.126a.5.5 0 0 1-.947.063z"/></svg>
        </div>
        <div class="bg-primary text-sm font-medium px-2 absolute z-10 transition-opacity py-0.5 translate-x-5 translate-y-5 text-primary-foreground rounded" style="top: {$fakeCursorNameTagPos.y}px; left: {$fakeCursorNameTagPos.x}px; opacity: {fakeCursorOpacity};">You</div>
        {#if $mode === 'dark'}
          <enhanced:img src={DarkOverview} alt="Dark Overview" class="size-full object-cover" />
        {:else}
          <enhanced:img src={LightOverview} alt="Light Overview" class="size-full object-cover" />
        {/if}
      </div>
    </div>
  </div>
