...
export async function getServerById(userId: number, id: number): Promise<Server | null> {
  const [rows] = await db.query<RowDataPacket[]>('SELECT * FROM server WHERE id = ?', [id]);
  const [mailingEnabled] = await db.query<RowDataPacket[]>('SELECT enabled FROM emailing WHERE userId = ? AND serverId = ?', [userId, id]);
  console.log(mailingEnabled);
