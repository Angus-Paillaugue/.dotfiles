...
test.describe('Exercises Page', () => {
  test.beforeEach(async ({ page }) => {
		...
		await page.fill('form:nth-child(1) input[type="text"]', 'Angus');
		await page.fill('form:nth-child(1) input[type="password"]', 'APAILL40');
  ...
  test('should display the exercises page', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toHaveText('All exercises');
  });

  test('should display a list of exercises', async ({ page }) => {
    const exercises = page.locator('tbody tr');
    await expect(exercises).toHaveCountGreaterThan(0);
  });
  ...
  test('should filter exercises based on search input', async ({ page }) => {
    ...
    await expect(filteredExercises).toHaveCountGreaterThan(0);
