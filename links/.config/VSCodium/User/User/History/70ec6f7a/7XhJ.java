...
public class GenererSoldeDeToutCompteControleur  implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (target.getName() != null && target.getName().equals("cancelButton")) {
			...
			for (int i = 0; i < this.vue.radioPanel.getComponentCount(); i++) {
				...
				if (radio.isSelected()) {
					...
					List<BienLouable> biens =
							new ArrayList<>(
									Proprietaire proprietaire = SharedState.getInstance().getProprietaire();.proprietaire.getBatiment().stream()
									.flatMap(bat -> bat.getBiensLouables().stream())
									.collect(Collectors.toList()));
					new SoldeDeToutCompte(
							biens.get(idBien)).setVisible(true);
				}
			}
			...
			//IMPLEMENTER CE QU'IL SE PASSE LORSQUE L'ON CLIQUE SUR UN RADIO BOUTON (METTRE A JOUR LE SCROLLPANE)
