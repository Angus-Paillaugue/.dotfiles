...
public class ProfilLocataire extends JFrame {
  ...
  private Boolean isColocation;
    ...
    JLabel nameLabel =
        new JLabel(
            this.personne.getNom() + " " + this.personne.getPrenom(),
            JLabel.CENTER); // Center the label text
    ...
    workPanel.add(
        createDetailLabel(
            "Contrat de travail", this.personne.getTypeDeContratDeTravail().toString()));
    workPanel.add(
        createDetailLabel(
            "Revenu NET",
            Formatters.formatCurrency(this.personne.getRemunerationMensuelleNette())));
    workPanel.add(
        createDetailLabel(
    ...
    if (!isGarant) {
      ...
      if (this.location.getGarants() == null) {
  ...
  private JPanel createDetailLabel(String title, String value) {
    ...
    moreInfoButton.addActionListener(
        e -> {
          new ProfilLocataire(p, l, true);
          this.dispose();
        });
    locatairePanel.add(moreInfoButton);

    locatairePanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY,1),BorderFactory.createEmptyBorder(5,5,5,5)));return locatairePanel;
    }
