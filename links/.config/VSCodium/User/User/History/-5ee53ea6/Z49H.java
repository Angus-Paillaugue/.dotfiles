...
public class EditAddress {
  ...
  public void refreshTenants(JPanel locataires, JPanel colocataires) {
    ...
    if (bienLouable.isColocation())
      for (Personne _coloc : bienLouable.getColocation().getColocataires()) {
        JPanel _subcontainer = new JPanel();
        ComboList<Personne> _tenant =
            new ComboList<>(bienLouable.getColocation().getColocataires());
        _tenant.setEnabled(false);
  ...
  private void initialize() {
    ...
    JLabel pageTitle =
        new JLabel("Modifier les informations de " + bienLouable.getAdresse().toString());
    ...
    addTenantBtn.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            if (bienLouable.isColocation())
              bienLouable
                  .getColocation()
                  .addColoc((Personne) persons.getSelectedItem(), null, null, null, 0);
            else bienLouable.getLocation().addLocataire((Personne) persons.getSelectedItem());
            refreshTenants(locataires, colocataires);
          }
        });
