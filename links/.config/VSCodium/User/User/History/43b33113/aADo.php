    ...
    foreach ($rencontres['previous'] as $rencontre) {
      ...
      <div class='bg-white p-4 rounded-lg border border-neutral-300/50'>
        ...
        <div class='flex flex-row items-end justify-between'>
          <p class='text-neutral-600 text-lg font-semibold'>".$rencontre->getLieu()."</p>
          <div class='px-2 py-2 rounded-full ".$rencontre->getResultat() === 'Nul' ? 'bg-neutral-600 text-neutral-900' : ($rencontre->getResultat() == 'Victoire' ? 'bg-green-600 text-neutral-100' : 'bg-red-600 text-neutral-100')."'>".$rencontre->getResultat()."</div>
        </div>
      </div>";
    }
    echo "</div>";
