...
<div class="max-w-screen-xl w-full mx-auto p-4 rounded-xl border space-y-4 border-neutral-300/50">
  ...
  if(count($rencontres['previous']) > 0) {
    ...
    foreach ($rencontres['previous'] as $rencontre) {
      $pillBg = $rencontre->getResultat() === 'Nul' ? 'bg-neutral-600 text-neutral-900' : ($rencontre->getResultat() == 'Victoire' ? 'bg-green-600 text-neutral-100' : 'bg-red-600 text-neutral-100');
      echo "
      <div class='bg-white p-4 rounded-lg border border-neutral-300/50'>
        <div class='flex flex-row justify-between items-center'>
          <h4 class='text-2xl font-semibold'>".$rencontre->getEquipeAdverse()."</h4>
        ...
        <div class='flex flex-row items-end justify-between'>
          <p class='text-neutral-600 text-lg font-semibold'>".$rencontre->getLieu()."</p>
          <div class='px-2 py-1 text-base rounded-full ".$pillBg."'>".$rencontre->getResultat()."</div>
        </div>
      </div>";
    }
    echo "</div>";
