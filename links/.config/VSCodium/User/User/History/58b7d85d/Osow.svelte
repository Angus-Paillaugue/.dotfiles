...
<script lang="ts">
  ...
  function windowScrollHandler(offset: number, container: HTMLElement) {
    ...
    for (let i in headingScrolls) {
      console.log(headingScrolls[i], trigger);
    ...
    if (!activeHeadingInSidebar && headingScrolls.length > 1) return;
    if(headingScrolls.length === 1) {
      activeHeadingInSidebar = document.querySelector(`[href="#${Object.keys(headingScrolls)[0]}"]`) as HTMLElement;
    }
