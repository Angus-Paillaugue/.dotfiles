...
files="$(find . -name '*.sh'  -not -name install.sh -type f)"

# Install the scripts
for script in $files; do
  sudo cp $script /usr/local/bin/$(basename $script .sh)
  sudo chmod +x /usr/local/bin/$(basename $script .sh)
  echo -e " ${GREEN}✓${NC} Installed $(basename $script .sh)"
done

echo -e "\n ${GREEN}✓${NC} The compressor scripts have been installed successfully."
