...
def scrapeProduct(url):
  ...
  if response.status_code == 200:
    ...
    productTitle = soup.select_one('h1>span').getText()
    productImage = soup.find('meta', {'property': 'og:image'})
    if metaTag and productTitle and productImage:
      priceString = metaTag.get('content')
      productImage = urljoin(productImage.get('content'), urlparse(productImage.get('content')).path)
      ...
      if match:
        ...
        return {'price':price, 'currency':currency, 'imageUrl':productImage, 'tilte':productTitle }
    else:
      return None
  return None
...
if __name__ == '__main__':
    ...
    for productUrl in PRODUCTS:
        scrapedData = scrapeProduct(productUrl)
        zalandoId = extractProductId(productUrl)
        if scrapedData:
            productsData.append({
                'productName':scrapedData['tilte'],
                'currentPrice': f"{scrapedData['price']} {scrapedData['currency']}",
                'imageUrl': scrapedData['imageUrl'],
                'link': productUrl,
                'zalandoId': zalandoId
            })
        else:
            log('ERROR', 'Error while scraping product', {'URL': productUrl})

    send_message(productsData)
