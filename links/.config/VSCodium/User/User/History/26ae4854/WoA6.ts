...
class ServerMonitoringDAO {
  ...
  async getServersStatuses(): Promise<{ server: Server; online: boolean; errors?: string[] }[]> {
    const servers = await this.getAllMonitoredServers();
    const serverStatuses = await Promise.all(
      servers.map(async (server) => {
        if (!server.publicUrl || server.id === undefined || !server.monitored) return;
        const serverMonitoring = await this.findServerMonitoringByServerId(server.id, new Date(), new Date());
        if (!serverMonitoring) {
          return { server, online: false, errors: ['No monitoring data'] };
        }
        return { server, online: true, errors: (serverMonitoring.errors || []) };
      })
    ) as { server: Server; online: boolean; errors?: string[] }[];
