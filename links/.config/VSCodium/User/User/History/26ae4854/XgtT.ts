...
class ServerMonitoringDAO {
  ...
  async createServerMonitoring(serverId: number, {
    ...
    const sql = `
      INSERT INTO serverMonitoring (serverId, duration, hasTimedOut, error, timestamp)
      VALUES (?, ?, ?, ?, ?)
    `;
    console.log(serverId, duration, hasTimedOut, error, timestamp);
    const params = [serverId, duration, hasTimedOut, error, timestamp];
