<script lang="ts">
  import { Timer, Container, TriangleAlert, Info, OctagonAlert, Database } from 'lucide-svelte';
  ...
  const tableLogs: { id: number; status: LogLevelType; timestamp: string; message: string }[] = [
    {
      id: 1254,
      status: 'error',
  ...
  const regenerateLogs = () => {
    tableLogs.forEach((log) => {
      log.timestamp = getRandomRecentDate();
    });
  }
</script>
...
<div class="flex flex-col">
  ...
  <div class="mt-4 grid grid-cols-1 gap-8 lg:grid-cols-3" bind:this={bentoGrid}>
    ...
    >
      <div class="grid lg:grid-cols-3 gap-4 w-full relative">
        <Button
          class="absolute top-1/2 -translate-x-1/2 left-1/3 -translate-y-1/2 z-10"
        ...
        <!-- Table -->
        <div class="relative h-[400px] w-full grow overflow-hidden flex flex-col items-center justify-center lg:col-span-2">
          <div class="w-[90%] h-[89%] border border-border rounded-lg flex flex-col bg-background">
            <p class="font-medium text-base px-4 py-1">Latest Logs</p>
            <div class="px-4 py-1 border-t border-border grow h-full overflow-y-hidden">
              <table class="table-auto w-full">
                <thead>
                  <tr>
                    <td class="text-sm font-medium">ID</td>
                    <td class="text-sm font-medium">Status</td>
                    <td class="text-sm font-medium">Timestamp</td>
                    <td class="text-sm font-medium">Message</td>
                  </tr>
                </thead>
                <tbody>
                  {#each tableLogs as log, i}
                    <tr transition:fade={{ duration:500, delay: 1000 * i }}>
                      <td class="py-2 text-sm text-thin text-muted-foreground">{log.id}</td>
                      <td class="py-2 text-sm text-thin text-muted-foreground">
                        <LogLevel level={log.status} class="text-xs py-0.2 px-1" />
                      </td>
                      <td class="py-2 text-sm text-thin text-muted-foreground">{log.timestamp}</td>
                      <td class="py-2 text-sm text-thin text-muted-foreground">
                        {log.message}
                      </td>
                    </tr>
