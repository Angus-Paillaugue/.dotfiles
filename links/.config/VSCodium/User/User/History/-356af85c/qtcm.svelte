...
<div class={cn("relative size-full", className)} {...restProps}>
  ...
  {#if isSelected}
    <div class="absolute inset-0 z-10 bg-background/50 backdrop-blur-sm flex flex-col items-center justify-center @container">
      {#if isPlaying}
        <!-- For covers <= 100px wide -->
        <div class="@[100px]:hidden flex flex-row justify-around items-center size-full gap-[2px] p-2" out:scale={{ duration: 300, delay: 0, opacity: 0 }}>
        ...
        <!-- For covers > 100px wide -->
        <div class="size-full hidden @[100px]:flex flex-row justify-around items-center gap-[2px] p-12" out:scale={{ duration: 300, delay: 0, opacity: 0 }}>
