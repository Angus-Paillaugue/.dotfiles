<script lang="ts">
  ...
  let { song, isSongSelected = false, class: className, ...restProps } : Props = $props();
...
<div class={cn("relative size-full", className)} {...restProps}>
  ...
  {#if isSongSelected}
    <div class="absolute inset-0 z-10 bg-background/50 backdrop-blur-sm flex flex-col items-center justify-center @container">
      ...
      <div class="@[100px]:hidden flex flex-row justify-around items-center size-full gap-[2px] p-2">
      ...
      <!-- For covers > 100px wide -->
      <div class="size-full hidden @[100px]:flex flex-row justify-around items-center gap-[2px] p-12">
