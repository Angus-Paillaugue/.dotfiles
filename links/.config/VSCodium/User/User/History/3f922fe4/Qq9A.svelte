...
<div bind:this={cursor} class="w-full h-full z-50 transition-all fixed pointer-events-none max-md:hidden">
  <div class="relative h-full w-full pointer-events-none">
    <div class="absolute -translate-x-1/2 -translate-y-1/2 transition-colors rounded-full shadow-[0_0_50px_20px_]" style="box-shadow: 0 0 50px 20px rgba(200, 200, 200, 0.5); width: 0px; height: 0px; top: {$coords.y}px; left: {$coords.x}px;"></div>
  </div>
</div>
