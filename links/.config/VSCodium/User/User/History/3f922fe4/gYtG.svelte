...
<div class="relative h-full w-full pointer-events-none z-10 max-md:hidden" bind:this={cursor}>
  <div
    class="absolute -translate-x-1/2 -translate-y-1/2 rotate-12 transition-colors"
    style="top: {$coords.y}px; left: {$coords.x}px; scale: {$coords.scale}"
  >
    {#if $coords.scale < 1.25}
      <div in:scale>
        <Icon icon="material-symbols:dangerous-outline-rounded" class="size-6 text-red-600" />
      </div>
    {:else}
      <div in:scale>
        <Icon icon="material-symbols:back-hand-rounded" class="size-6 text-blue-600" />
      </div>
    {/if}
  </div>
</div>
...
<section class="flex h-screen flex-col items-center justify-center gap-4 cursor-none hover:cursor-none">
  <h1 class="font-extrabold text-8xl transition-all hover:text-shadow-[0px_0px_15px_rgba(0,_0,_0,_1)]">{$page.status}</h1>
