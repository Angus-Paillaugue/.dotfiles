	...
	>
		<div class="flex flex-row gap-2 p-2 relative">
			...
			<Button
				href="/app"
				class={cn(
					'size-14 p-2 bg-white',
					activeNavLink === 'home' && 'bg-neutral-800'
				)}
				aria-label="Home"
			>
			...
			<Button
				...
				class={cn('size-14 bg-white', $bionicReadingEnabled && 'bg-neutral-800')}
				aria-label="Search"
			>
			...
			<Button
				...
				class={cn(
					'size-14 p-2 bg-white',
					activeNavLink === 'bookmarks' && 'bg-neutral-800'
				)}
