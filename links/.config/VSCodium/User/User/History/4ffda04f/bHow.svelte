...
<ul
  class={root ? 'bg-[var(--inline-code-bg)] p-4 rounded-md' : 'border-l-2 border-neutral-700 pl-2.5 ml-1.5'}
>
  {#each tree as element}
    ...
    {:else}
      <!-- Is a file -->
      <li
        class="list-none px-1 w-fit m-0 mt-1 flex flex-row gap-2 items-center text-neutral-100 {element?.highlighted &&
