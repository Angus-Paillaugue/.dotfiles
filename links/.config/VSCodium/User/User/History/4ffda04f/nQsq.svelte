  ...
  {#each tree as element}
    {#if element?.children}
      ...
      <details open={element?.open ?? open}>
        <summary
          class="flex flex-row gap-2 items-center cursor-pointer text-base font-bold text-neutral-100 hover:text-neutral-400 transition-colors mt-1 {element?.highted && 'bg-primary-700/50 rounded'}"
        >
    ...
    {:else}
      <!-- Is a file -->
      <li
        class="list-none pl-2 m-0 mt-1 flex flex-row gap-2 items-center text-neutral-100 {element?.highted && 'bg-primary-700/50 rounded'}"
      >
        <div class="size-5">
