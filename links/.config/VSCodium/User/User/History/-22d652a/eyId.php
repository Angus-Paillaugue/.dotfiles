...
<div class="max-w-screen-xl w-full mx-auto p-4 rounded-xl border space-y-6 border-neutral-900">
  ...
  // Liste des joueurs
  echo "<div><h2>Joueurs</h2><div class='!m-0 max-h-[300px] overflow-y-auto'><table class='w-full table-auto text-sm rounded-lg overflow-hidden'><thead class='border-b sticky top-0 border-neutral-900 bg-primary-50'><tr><td scope='  col' class='sticky top-0 h-12 px-4 text-left align-middle font-medium text-neutral-600'>Nom</td><td scope=' col' class='sticky top-0 h-12 px-4 text-left align-middle font-medium text-neutral-600'>Prénom</td></tr></thead><tbody class='[&amp;_tr:last-child]:border-0'>";
  ...
  if (count($rencontres['next']) > 0) {
    ...
    foreach ($rencontres['next'] as $rencontre) {
      ...
      <a href='/dashboard/rencontre.php?id=" .
        $rencontre->getId() .
        "' class='bg-primary-50 transition-colors hover:bg-neutral-900 p-4 rounded-lg border border-neutral-900'>
