...
<div class="max-w-screen-xl w-full mx-auto p-4 rounded-xl border space-y-6 border-primary-100">
  ...
  // Liste des joueurs
  echo "<div><h2>Joueurs</h2><div class='!m-0 max-h-[300px] overflow-y-auto'><table class='w-full table-auto text-sm rounded-lg overflow-hidden'><thead class='border-b sticky top-0 border-primary-100 bg-neutral-50'><tr><td scope='  col' class='sticky top-0 h-12 px-4 text-left align-middle font-medium text-neutral-600'>Nom</td><td scope=' col' class='sticky top-0 h-12 px-4 text-left align-middle font-medium text-neutral-600'>Prénom</td></tr></thead><tbody class='[&amp;_tr:last-child]:border-0'>";
  foreach ($joueurs as $joueur) {
    echo "<tr class='border-b border-border transition-colors even:bg-neutral-50'><td class='px-4 align-middle'><a class='hover:underline' href='/dashboard/joueur.php?id=" . $joueur->getId() . "'>" .
      $joueur->getNom() .
  ...
  if (count($rencontres['next']) > 0) {
    ...
    foreach ($rencontres['next'] as $rencontre) {
      ...
      <a href='/dashboard/rencontre.php?id=" .
        $rencontre->getId() .
        "' class='bg-neutral-50 transition-colors hover:bg-neutral-100 p-4 rounded-lg border border-primary-100'>
