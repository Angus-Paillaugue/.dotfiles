<script lang="ts">
  ...
  let { class: className, debounce = 0, value = $bindable(''), children, label, id, ...restProps }: (SvelteHTMLElements['input'] & MyProps) = $props();
...
{#snippet input()}
  <input class={finalClasses} use:debounceFunc={{ callback: handleDebounce, delay: debounce }} bind:value {id} name={id} {...restProps} />
{/snippet}
