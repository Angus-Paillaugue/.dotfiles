...
export const actions = {
	logIn: async ({ cookies, request }) => {
		...
		if (!password || password.length < 6) return fail(400, { error: m.error_messages_log_in_incorrect_password() });

		const user = await findUserByUsername(username);

		// If user does not exist, return error
		if (!user) return fail(400, { error: m.error_messages_log_in_no_account_with_provided_username() });
	...
	signUp: async ({ cookies, request }) => {
		...
		if (usernameIsAlreadyUsed) return fail(400, { error: m.error_messages_sign_up_username_already_exists() });
