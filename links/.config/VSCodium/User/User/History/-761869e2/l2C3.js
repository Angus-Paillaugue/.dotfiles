...
test('should show a spinner when saving personal info', async ({ page }) => {
  const usernameInput = page.locator('input#username');
  await usernameInput.fill('newUsername');
  ...
  const spinner = page.locator('button:has-text("Save") .size-5');
  await expect(spinner).toBeVisible();
});

test('should show a spinner when changing password', async ({ page }) => {
  const oldPasswordInput = page.locator('input#oldPassword');
  ...
  const spinner = page.locator('button:has-text("Change") .size-5');
  await expect(spinner).toBeVisible();
});
