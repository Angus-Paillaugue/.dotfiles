...
function auth(token: string): Promise<User | null> {
...
app.post('/logs', authenticateToken, async (req: Request, res: Response) => {
  const logs = req.body;
...
app.get('/getLogsOverviewStatistics', authenticateToken, async (req: Request, res: Response) => {
  try {
