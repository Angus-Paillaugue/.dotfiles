...
class Contact {
  ...
  public function __construct($nom, $prenom, $adresse, $telephone, $codePostal, $ville) {
    ...
    $insertedRow = $this->conn->run_query('INSERT INTO contacts (nom, prenom, adresse, telephone, codePostal, ville) VALUES (?, ?, ?, ?, ?, ?);', $nom, $prenom, $adresse, $telephone, $codePostal, $ville);
    $this->id = $insertedRow['id'];
  }

  public function getNom() {
    return $this->nom;
    $this->conn->run_query('UPDATE contacts SET nom=? WHERE id=?;', $nom, $this->id);
  }

  public function getPrenom() {
    return $this->prenom;
    $this->conn->run_query('UPDATE contacts SET prenom=? WHERE id=?;', $prenom, $this->id);
  }

  public function getAdresse() {
    return $this->adresse;
    $this->conn->run_query('UPDATE contacts SET adresse=? WHERE id=?;', $adresse, $this->id);
  }

  public function getTelephone() {
    return $this->telephone;
    $this->conn->run_query('UPDATE contacts SET telephone=? WHERE id=?;', $telephone, $this->id);
  }
