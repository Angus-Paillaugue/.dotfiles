<script lang="ts">
  ...
  let addCardModalFormValues = $state({ title: '', description: '', type: 'table', colSpan: '1', request: '' });
  ...
  async function addCard(event: Event) {
    ...
    await saveDashboard();
    addCardModalFormValues = { title: '', description: '', type: 'table', colSpan: '1', request: '' };
  ...
  $inspect(addCardModalFormValues.description)
</script>
  ...
  <Dialog.Content>
    ...
    <form onsubmitcapture={addCard} class="space-y-4">
      <div class="flex flex-col gap-1">
        ...
        <Input type="text" name="title" placeholder="Title" bind:value={addCardModalFormValues.title} />
      </div>

      <div class="flex flex-col gap-1">
        <Label for="description">Card description</Label>
        <Input type="text" name="description" placeholder="Description" bind:value={addCardModalFormValues.description} />
      </div>
