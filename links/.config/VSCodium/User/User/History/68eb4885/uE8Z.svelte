<script lang="ts">
  ...
  let addCardModalFormValues = $state({ title: '', description: '', type: 'table', colSpan: '1' });
  ...
  export async function saveDashboard(d?: Dashboard) {
    if (!d) d = dashboard;
    isSaving = true;
...
<div class="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
  ...
  <Grid bind:cards={dashboard.cards} {onSwap} {deleteCard} />
</div>
