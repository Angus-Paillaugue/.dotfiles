<script>
  ...
  let { pages = [], root = false, style = 'details' } = $props();
  const pathname = $derived(decodeURIComponent($page.url.pathname));

  const addOpenStates = (pages) => {
    pages.forEach((page) => {
      page.open = developSidebar;
      if (page.children) addOpenStates(page.children);
    });
  };
  if (root) addOpenStates(pages);
  $inspect(pages);
  /**
...
{#if style === 'details'}
  ...
  >
    {#each pages as page}
      {#if isNested(page)}
      <div>
        <button onclick={() => (page.open = !page.open)} class={twMerge(itemBaseClasses, itemColors, 'flex flex-row items-center gap-4 w-full')}>
          {page.name}
          <span class="arrow transition-all">
            <Icon
              icon="material-symbols:arrow-forward-ios-rounded"
              class="size-3 transition-transform {page.open && 'rotate-90'}"
            />
          </span>
        </button>
        <div use:accordion={page.open}>
