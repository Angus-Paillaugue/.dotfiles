...
export function headingTransformer() {
  ...
  const getHttpKeyword = (text) => httpKeywords.find(keyword => text.toUpperCase().startsWith(keyword));
  const getEndOfKeyword = (text) => text.slice(text.indexOf(' ') + 1);
  return (tree) => {
    visit(tree, 'heading', (node) => {
      for(const child of node.children) {
        const text = child.value;
        const matchingKeyword = getHttpKeyword(text);

        if (matchingKeyword) {
          child.value = text.replace(
            matchingKeyword,
            `<span class="http-keyword" data-keyword="${matchingKeyword}">${matchingKeyword}</span> ${getEndOfKeyword(text)}`
          );
        }
      }
    });
