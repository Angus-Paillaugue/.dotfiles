...
export async function editMetadata(song: Song) {
	console.log(path.join(songsDir, getSongFileName(song)));
	const metadata = await nodeId3.read(path.join(songsDir, getSongFileName(song)));
	...
	await nodeId3.update(
		{
			...
			year: song.year,
			image: path.join(coverDir, song.id + '.png')
		},
