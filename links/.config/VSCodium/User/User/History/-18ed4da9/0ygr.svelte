...
<PaneGroup direction="horizontal" class="grow max-h-full" autoSaveId="adminAddExercisePage">
	<!-- Left part -->
	<Pane defaultSize={50} class="flex flex-col grow gap-4">
		...
		<div class="flex flex-col max-h-full grow bg-neutral-900 rounded-xl overflow-hidden">
			<div class="p-4 grow max-h-full flex flex-col overflow-y-auto">
				...
				<div class="flex flex-col grow max-h-full mt-4">
					<div class="flex flex-row shrink-0 gap-4 mb-2">
						<button class={cn("flex flex-row gap-2 items-center rounded-xl px-2 py-1 transition-colors", !isPreviewingDescription && "bg-neutral-800")} onclick={() => (isPreviewingDescription = false)}>
							Text
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5"><path d="M17 6.1H3"/><path d="M21 12.1H3"/><path d="M15.1 18H3"/></svg>
						</button>
						<button class={cn("flex flex-row gap-2 items-center rounded-xl px-2 py-1 transition-colors", isPreviewingDescription && "bg-neutral-800")} onclick={() => (isPreviewingDescription = true)}>
							Preview
					...
					<div id="pageContainer" class="grow flex flex-col overflow-y-auto max-h-full">
