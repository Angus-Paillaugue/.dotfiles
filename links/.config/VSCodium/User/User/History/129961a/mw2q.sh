...
download() {
  ...
  # Start the spinner
  spinner "Downloading $file_title (calculating time...)" &
  spinner_pid=$!

  # Download the file
  start_time=$(date +%s)
  if curl -L --silent --output "$output_file_path" --write-out "%{size_download} %{time_total}" "$file_url" > curl_output.txt; then
    end_time=$(date +%s)
    download_time=$((end_time - start_time))
    file_size=$(du -h "$output_file_path" | cut -f1)
    remaining_files=$((remaining_files - 1))
    remaining_time=$(awk '{print int(($2 * '"$remaining_files"') / 1)}' curl_output.txt)

    # Convert remaining time to readable format
    if (( remaining_time >= 3600 )); then
      remaining_time_display=$(printf "%d hour(s) %d minute(s)" $((remaining_time / 3600)) $((remaining_time % 3600 / 60)))
    elif (( remaining_time >= 60 )); then
      remaining_time_display=$(printf "%d minute(s) %d second(s)" $((remaining_time / 60)) $((remaining_time % 60)))
    else
      remaining_time_display=$(printf "%d second(s)" $remaining_time)
    fi

    # Kill the spinner
    kill $spinner_pid
    wait $spinner_pid 2>/dev/null
    echo -ne "\r✓ $file_title : $file_size (${remaining_files} to go, ${remaining_time_display} left)\n"
  else
