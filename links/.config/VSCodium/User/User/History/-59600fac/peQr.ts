...
class Logger {
  ...
  #registered: FinalizationRegistry = false;
  ...
  static #finalizationRegistry = new FinalizationRegistry(async (heldValue: Logger) => {
    await heldValue.#flush();
  });

  debug(...messages: any[]) {
    const message = Logger.#formatMessage(messages);
    console.log(`${Logger.#COLORS.get("bgWhite")}${Logger.#COLORS.get("black")}[ DEBUG ]${Logger.#COLORS.get("reset")} ${message}`);
  ...
  warn(...messages: any[]) {
    const message = Logger.#formatMessage(messages);
    console.warn(`${Logger.#COLORS.get("bgYellow")}[ WARN ]${Logger.#COLORS.get("reset")} ${message}`);
    this.#sendLogImmediately({ level: "warn", message, timestamp: new Date() });
  }

  error(...messages: any[]) {
    const message = Logger.#formatMessage(messages);
    console.error(`${Logger.#COLORS.get("bgRed")}[ ERROR ]${Logger.#COLORS.get("reset")} ${message}`);
    this.#sendLogImmediately({ level: "error", message, timestamp: new Date() });
  }

  fatal(...messages: any[]) {
    const message = Logger.#formatMessage(messages);
    console.error(`${Logger.#COLORS.get("red")}[ FATAL ]${Logger.#COLORS.get("reset")} ${message}`);
    this.#sendLogImmediately({ level: "fatal", message, timestamp: new Date() });
  }
  ...
  constructor(config: Config) {
    ...
    Logger.#finalizationRegistry.register(this, this);
  }
