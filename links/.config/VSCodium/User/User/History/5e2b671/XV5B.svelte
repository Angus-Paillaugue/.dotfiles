<script>
	...
	let editExerciseModalVisible = $state(true);
</script>
		...
		>
			...
			<div class="max-h-[500px] flex flex-col gap-2 overflow-y-auto">
				{#each allExercises as exercise}
					<div class="bg-neutral-700 p-4 rounded-xl flex flex-row gap-4 items-center">
						<p class="text-lg font-semibold text-neutral-100">{exercise.name}</p>
					</div>
				{/each}
			</div>
			<div class="grid grid-cols-2 gap-4">
				<button
					type="button"
					class="text-base font-medium md:font-semibold w-full rounded-xl bg-neutral-700 px-4 py-2 text-neutral-100 outline-0 placeholder:text-neutral-400 focus:outline-0"
					onclick={() => (editExerciseModalVisible = false)}
				>
					Cancel
				</button>
				<button
					class="text-base font-medium md:font-semibold w-full rounded-xl bg-neutral-900 px-4 py-2 text-neutral-100 outline-0 placeholder:text-neutral-400 focus:outline-0 flex flex-row items-center justify-center gap-2"
				>
					Create
				</button>
			</div>
