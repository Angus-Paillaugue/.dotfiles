  ...
  <div class="flex flex-row gap-2 items-center max-md:hidden">
    {#each Object.entries(socials) as [name, url]}
      <a href={url} class="h-fit p-1">
        <Icon icon="material-symbols:{name.toLocaleLowerCase()}-line" class="size-6" />
      </a>
    {/each}
    ...
    >
      {#if $mode === 'dark'}
        <div in:scale>
          <Icon icon="material-symbols:sun" class="size-6" />
        </div>
      {:else}
        <div in:scale>
          <Icon icon="material-symbols:moon" class="size-6" />
        </div>
      {/if}
    </button>
