...
<div class="shrink-0 flex flex-row items-center justify-center mx-auto gap-px rounded-xl overflow-hidden mt-4">
  {#each {length:items.totalPages} as _, i}
    <button onclick={() => {fetchItems(i+1)}} class={cn("flex items-center font-mono text-base font-semibold justify-center w-8 h-8 bg-card text-neutral-100", currentPage === i+1 && 'bg-neutral-700')}>{i + 1}</button>
  {/each}
</div>
