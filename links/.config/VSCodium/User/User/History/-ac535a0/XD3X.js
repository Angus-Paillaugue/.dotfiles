...
(async () => {
  ...
  console.log(theme.default.tokenColors.find((color) => color.scope === 'markup.inserted.diff'));
  ...
  const lineAddedBg = hexToRgb(
		theme.default.tokenColors.find((color) => color.scope === 'markup.added.diff').settings
			.foreground
	);
