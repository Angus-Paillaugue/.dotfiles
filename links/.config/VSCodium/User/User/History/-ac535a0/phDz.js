...
(async () => {
  ...
  console.log(typeof theme.default.colors);
  const lineRemovedBg = hexToRgb(
		theme.default.colors.find((color) => color.scope === 'markup.inserted.diff').settings.foreground
	);
  const lineAddedBg = hexToRgb(
		theme.default.colors.find((color) => color.scope === 'markup.added.diff').settings.foreground
	);
