...
(async () => {
  ...
  console.log(theme.tokenColors.find((color) => color.scope === 'markup.added.diff'));
  const lineRemovedBg = hexToRgb(theme.tokenColors.find(
		(color) => color.scope === 'markup.inserted.diff'
	).settings.foreground);
  const lineAddedBg = hexToRgb(theme.tokenColors.find(
