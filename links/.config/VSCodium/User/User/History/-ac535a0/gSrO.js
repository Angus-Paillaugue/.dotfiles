...
(async () => {
  ...
  try {
    console.log(theme.default.colors['editor.findMatchHighlightBackground']);
		const lineHighlightedBg = hexToRgb(theme.default.colors['editor.findMatchHighlightBackground'].slice(0, 6));
    if(!lineHighlightedBg)
      throw new Error("No line highlighted background color found");
		let codeCSSFile = fs.readFileSync(`./src/code.css`, 'utf8');
		codeCSSFile = codeCSSFile.replace(
			/--line-highlighted-bg: .+;/,
