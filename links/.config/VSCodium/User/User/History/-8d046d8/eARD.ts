...
export async function getSongFromId(id: Song['id']) {
	const query = `
		...
		WHERE s.id = ?;`;
	const [row] = await db.execute<RowDataPacket[]>(query, [id]);

	const song =  normalizeSongPaths(row[0] as Song);
	// Get the artist associated to the song
	song.artist = await getArtist(row[0].artistId);
...
export async function getAllSongs() {
	...
	const songs = (rows as Song[]).map(normalizeSongPaths);

	// Get artist associated with each song
	await Promise.all(
		songs.map(async (song) => {
			song.artist = await getArtist(song.artist.id);
		})
	);
	return songs;
}
