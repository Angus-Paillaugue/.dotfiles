...
<div class="max-w-screen-xl w-full mx-auto p-4 rounded-xl border space-y-4 border-neutral-300/50">
  ...
  <div class="flex flex-row gap-8 flex-wrap">
    <div class="flex flex-row gap-2 items-center px-4 py-2 border rounded-lg w-fit border-neutral-300/50">
      <span class="text-neutral-600"><?php echo Components::Icon([ 'icon' => 'birthday', 'class' => 'size-5' ]); ?></span>
      <time><?php echo formatters::formatDate($joueur->getDateNaissance()); ?></time>
    </div>
    <div class="flex flex-row gap-2 items-center px-4 py-2 border rounded-lg w-fit border-neutral-300/50">
      <span class="text-neutral-600"><?php echo Components::Icon([ 'icon' => 'weight', 'class' => 'size-5' ]); ?></span>
      <span><?php echo Formatters::formatWeight($joueur->getPoids()); ?></span>
    </div>
    <div class="flex flex-row gap-2 items-center px-4 py-2 border rounded-lg w-fit border-neutral-300/50">
      <span class="text-neutral-600"><?php echo Components::Icon([ 'icon' => 'status', 'class' => 'size-5' ]); ?></span>
      <span><?php echo $joueur->getStatut(); ?></span>
    </div>
  </div>
  <?php
    if (count($joueur->getCommentaires()) == 0) {
      echo "<p class='text-neutral-600'>Aucun commentaire</p>";
    }else {
      ...
      foreach($joueur->getCommentaires() as $commentaire) {
        ...
        echo "<time class='text-sm text-neutral-600'>" . Formatters::formatDateTime($commentaire->getDate()) . "</time>";
        ...
        Components::Button([
          ...
          'variant' => 'danger',
          'class' => 'rounded-none',
          'href' => '/dashboard/delete-commentaire.php?id=' . $commentaire->getId() . '&redirect=' . urlencode($_SERVER['REQUEST_URI']),
        ]);
        echo "</div>";
        echo "</div>";
      }
