...
function cleanUpImages(filename, username) {
  ...
  writeFileSync(`${username}/photos.json`, JSON.stringify(output, null, 2));
}

function cleanUpVideos(filename, username) {
  // Create the directory
  mkdirSync(username, { recursive: true });
  // Read the har file
  const har = JSON.parse(readFileSync(filename, 'utf8'));
  ...
  writeFileSync(`${username}/videos.json`, JSON.stringify(output, null, 2));
}
...
function download() {
  ...
  for (const photo of photos) {
    exec(`wget -O ${photo.title}.jpg ${photo.url}`);
    const timestamp = new Date(photo.postedAt).toISOString().replace(/[^0-9]/g, '');
    // exec(`touch touch -a -m -t ${timestamp} ${photo.title}.jpg`);
  }
  // Loop through the videos
  for (const video of videos) {
    exec(new Date(photo.postedAt).toISOString().replace(/[^0-9]/g, '');`wget -O ${video.title}.mp4 ${video.url}`);
  }
}
