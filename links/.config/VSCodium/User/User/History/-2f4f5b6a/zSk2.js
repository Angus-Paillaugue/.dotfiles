...
function download() {
  ...
  for (const photo of photos) {
    ...
    // [[CC]YY]MMDDhhmm[.ss]  -> 201512180130.09
    const timestamp = photo.postedAt.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})/, '$1$2$3$4.$5');
    // exec(`touch -a -m -t ${timestamp} ${photo.title}.jpg`);
  }
  // Loop through the videos
  // for (const video of videos) {
  //   exec(`wget -O ${video.title}.mp4 ${video.url}`);
  // }
}
