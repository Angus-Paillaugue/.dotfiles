<script lang="ts">
  ...
  function onWindowClick(e: MouseEvent) {
    if(e.target && !(e.target as HTMLElement).closest('.title')) {
      saveConversation();
    }
  }
</script>
...
{#if isEditing || conversation.title === ''}
  <Input placeholder="Conversation name" bind:value={conversation.title} bind:ref={input} onkeyup={onKeyUp} onblur={() => {isEditing = false; saveConversation()}} />
{:else}
