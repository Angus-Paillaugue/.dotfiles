package dao;

import java.util.Arrays;
import java.util.List;
import java.util.Map;
import java.util.stream.Collectors;

import jdbc.Connector;
import modele.Charges;
import utils.Logger;

public class ChargesDAO implements GenericDAO<Charges> {

  public int create(Charges c) {
    return 0;
  }

  public int create(Charges c, int idBienLouable) {
    try {
      Connector db = Connector.getInstance();
      String query =
          "INSERT INTO Charges (ordureMenageres, entretienPartiesCommunes, eclairagePartiesCommunes, eauPartieFixe, provisionSurCharge, id_1) VALUES (?, ?, ?, ?, ?, ?)";
      int chargeId =
          db.executeUpdate(
              query,
              Arrays.asList(
                  c.getOrduresMenageres(),
                  c.getEntretienPartiesCommunes(),
                  c.getEclairagePartiesCommunes(),
                  c.getEauPartieFixe(),
                  c.getProvisionSurCharge(),
                  idBienLouable));

      return chargeId;
    } catch (Exception e) {
      Logger.error(e);
    }

    return -1;
  }

  @Override
  public Charges find(int id) {
    try {
      Connector db = Connector.getInstance();
      String query = "SELECT * FROM Charges WHERE id = ?";
      Map<String, Object> result = db.getFirst(query, Arrays.asList(Integer.toString(id)));
      if (result == null) {
        return null;
      }

      return new Charges(
          (int) result.get("id"),
          (float) result.get("ordureMenageres"),
          (float) result.get("entretienPartiesCommunes"),
          (float) result.get("eclairagePartiesCommunes"),
          (float) result.get("eauPartieFixe"),
          (float) result.get("provisionSurCharge"));
    } catch (Exception e) {
      Logger.error(e);
    }

    return null;
  }

  @Override
  public void update(Charges c) {
    // This is used to keep a record of the old values in the database
    int createdId = create(c);

    return createdId;
  }

  @Override
  public int update(Charges c) {
    // This is used to keep a record of the old values in the database
    int createdId = create(c);

    return createdId;
  }

  @Override
  public void delete(int id) {
    try {
      Connector db = Connector.getInstance();
      String query = "DELETE FROM Charges WHERE id = ?";
      db.executeUpdate(query, Arrays.asList(Integer.toString(id)));
    } catch (Exception e) {
      Logger.error(e);
    }
  }

  @Override
  public List<Charges> findAll() {
    try {
      Connector db = Connector.getInstance();
      String query = "SELECT * FROM Charges";
      List<Map<String, Object>> results = db.executeQuery(query, null);
      if (results == null) {
        return null;
      }

      return results.stream()
          .map(
              result ->
                  new Charges(
                      (int) result.get("id"),
                      (float) result.get("ordureMenageres"),
                      (float) result.get("entretienPartiesCommunes"),
                      (float) result.get("eclairagePartiesCommunes"),
                      (float) result.get("eauPartieFixe"),
                      (float) result.get("provisionSurCharge")))
          .collect(Collectors.toList());
    } catch (Exception e) {
      Logger.error(e);
    }

    return null;
  }

  public List<Charges> findByBienLouable(int bienLouableId) {
    try {
      Connector db = Connector.getInstance();
      String query = "SELECT * FROM Charges WHERE id_1 = ?";
      List<Map<String, Object>> results =
          db.executeQuery(query, Arrays.asList(Integer.toString(bienLouableId)));
      if (results == null) {
        return null;
      }

      return results.stream()
          .map(
              result ->
                  new Charges(
                      (int) result.get("id"),
                      (float) result.get("ordureMenageres"),
                      (float) result.get("entretienPartiesCommunes"),
                      (float) result.get("eclairagePartiesCommunes"),
                      (float) result.get("eauPartieFixe"),
                      (float) result.get("provisionSurCharge")))
          .collect(Collectors.toList());
    } catch (Exception e) {
      Logger.error(e);
    }

    return null;
  }
}
