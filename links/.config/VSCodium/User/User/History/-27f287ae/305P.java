...
public class ChargesDAO implements ChargesDAODef {
  ...
  public Charges create(Charges c, int idBienLouable) {
    try {
      ...
      String query =
          "INSERT INTO Charges (ordureMenageres, entretienPartiesCommunes, eclairagePartiesCommunes, eauPartieFixe, provisionSurCharge, indexEau, prixEau, dateRevision idBienLouable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      int chargeId =
          db.executeUpdate(
              query,
              Arrays.asList(
  ...
  public Charges update(Charges c, int idBienLouable) {
    // This is used to keep a record of the old values in the database
    c = create(c, idBienLouable);
