...
<div class="overflow-x-auto w-full max-w-fit no-scrollbar">
  ...
  <div
    onmouseleave={() => (showingDetailsTooltip = false)}
    class="uptimeGraph flex h-11 items-center justify-between gap-1"
  >
    {#each filledMonitoring as data, i}
      ...
      {#if data.filled}
        ...
        <div
          class={cn('flex shrink-0 flex-row items-center justify-center transition-all', height)}
          onclick={() => {
        ...
        >
          <div
            class={cn(
              ...
              data.filled ? 'bg-secondary' : data.errors.length > 0 ? 'bg-red-600' : 'bg-green-600'
            )}
          ></div>
        </div>
      {/if}
  ...
  <div
    class={cn(
      'relative flex h-6 w-full flex-row items-center justify-between overflow-hidden transition-all duration-300',
      showingDetailsTooltip && 'h-0 opacity-0'
    )}
  >
    <div class="absolute left-0 text-sm text-muted-foreground whitespace-nowrap" style="min-width: max-content;">
      {NUMBER_OF_COLUMNS} days ago
    </div>
    <div class="absolute right-0 text-sm text-muted-foreground whitespace-nowrap" style="min-width: max-content;">
      Today
    </div>
  </div>
</div>
...
{#if showingDetailsTooltip && isHoveringIndex > -1 && filledMonitoring[isHoveringIndex]}
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div
    class="uptimeGraphTooltip pt-4"
    onmouseleave={() => (showingDetailsTooltip = false)}
    onmouseenter={() => (showingDetailsTooltip = true)}
    transition:slide={{ axis: 'y', duration: 300 }}
  >
    <div class="z-10 grid w-full grid-cols-1 gap-8 lg:grid-cols-3">
      <!-- Date card -->
