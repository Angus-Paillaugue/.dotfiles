...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
  {#if isLoadingOutfits}
    ...
    {#each {length:6} as _, i}
      <Card>
        <!-- Title -->
        <div class="animate-pulse mx-auto bg-neutral-800 w-1/2 h-6 rounded-full"></div>
        <!-- Items list -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          {#each { length: randomIntFromInterval(2, 4) } as _item}
            <div class="flex flex-col gap-2">
              <!-- Image -->
              <div class="animate-pulse bg-neutral-800 h-[200px] w-full rounded-lg"></div>
              <!-- Item name -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          {#each outfit.elements as item}
            <div class="flex flex-col">
              <img src={getImageUrl(item)} alt="" class="size-full object-cover rounded-lg" />
