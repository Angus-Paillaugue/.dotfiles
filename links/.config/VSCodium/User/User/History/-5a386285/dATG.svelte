<script>
  ...
  const { type = "warning", class:className, children, style="s", ...restProps } = $props();
...
{#if Object.keys(typeClasses).join(", ")}
...
{#if Object.keys(typeClasses[style]).includes(type)}
  <div class={twMerge(baseClasses[style], typeClasses[style][type], className)} {...restProps} role="alert">
    ...
    {:else}
      <p>Invalid note style. Should be either {Object.keys(typeClasses).join(", ")}</p>
    {/if}
  </div>
{:else}
