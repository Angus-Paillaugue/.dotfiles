...
public class VuesUtils {
	...
	public static Integer toInt(String value) {
		try {
			String[] splitText = value.split(" "); // Step 1
			int[] numbers = new int[splitText.length]; // Step 2
			for (int i = 0; i < splitText.length; i++) { // Step 3
				numbers[i] = Integer.parseInt(splitText[i]); // Step 3.1
			}
