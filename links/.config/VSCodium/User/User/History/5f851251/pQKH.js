...
function parseMeta(meta) {
  ...
  let copyCode = metaArray && !metaArray.some((item) => item.startsWith('no-copy'));
  ...
  return { name, icon, lineNumbers, copyCode, metaArray };
}
...
async function highlighter(code, lang, meta) {
  ...
  const { name, icon, lineNumbers, copyCode, metaArray } = parseMeta(meta);
  ...
  if (!meta) {
    ...
    html = highlighter.codeToHtml(code, {
      ...
      meta: { __raw: meta, meta: metaArray }
    });
  }
