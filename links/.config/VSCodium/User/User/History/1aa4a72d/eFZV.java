...
public class ProprietaireDAO implements GenericDAO<Proprietaire> {
  ...
  public Proprietaire find(int id) {
    try {
      Connector db = Connector.getInstance();
      String query = "SELECT P.*, A.* FROM Proprietaire P JOIN Adresse A ON P.idAdresse = A.id WHERE P.id = ?;";
      ...
      for (Logement l : logements) {
        Batiment b = batimentDAO.find(l.getIdBatiment());
  ...
  public Proprietaire findByMail(String mail) {
    try {
      Connector db = Connector.getInstance();
      String query =
          "SELECT P.id FROM Proprietaire P WHERE P.mail = ?;";
