	...
	const services = await Promise.all(
		cards.map(async (card) => {
			...
			const status = await Promise.all(
				urls.map(async (url) => {
					const { isUp } = await getServiceStatus(url);
					return { isUp };
				})
			);

			return { ...card, isUp: status.some((s) => s.isUp) };
		})
	);
	// if(!db) {
		return { services, disks: [], databases: { isUp: false, message: 'Cannot connect to your MySQL instance.', list: [] } };
	// }
