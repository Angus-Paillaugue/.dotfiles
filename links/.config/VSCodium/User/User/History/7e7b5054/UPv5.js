...
/**
 * Cleans up the given har file and extracts the photos and videos information.
...
function cleanup(filename, username) {
  ...
  for (const entry of har.log.entries) {
    ...
    if(request.url.includes("medias?limit=")) {
      ...
      for (const media of content.list) {
        // Get the text
        const text = slugify(media?.text?.slice(0, 50)) || randomUUID();
  ...
  writeFileSync(`download/${username}/photos.json`, JSON.stringify(images, null, 2));
