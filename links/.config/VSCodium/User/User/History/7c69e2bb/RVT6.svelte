...
<aside
  class={cn(
    'shrink-0 flex-col rounded-xl border border-border bg-card p-2 pt-3 transition-all duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 mr-2 max-lg:min-w-[300px] lg:w-[16rem] relative',
    mobileMenuOpened
...
>
  ...
  <Tooltip class={cn("absolute top-2 max-lg:hidden transition-all duration-1000", collapsed ? '-right-10 top-0 mb-2' : 'right-2')} content={collapsed ? m.app_home_layout_nav_sidebar_expand() : m.app_home_layout_nav_sidebar_collapse()} position='right'>
    <Button variant="square secondary" onclick={() => {collapsed = !collapsed}} class="p-1.5 w-fit">
