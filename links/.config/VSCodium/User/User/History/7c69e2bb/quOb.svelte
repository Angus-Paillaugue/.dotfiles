...
<aside
	class={cn(
		'relative mr-2 shrink-0 flex-col rounded-xl border border-border p-2 pt-3 transition-all duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 max-lg:min-w-[300px] lg:w-[16rem]',
		mobileMenuOpened
			? ' max-lg:ml-2 max-lg:translate-x-0'
			: 'hidden max-lg:-ml-2 max-lg:-translate-x-full lg:flex',
		collapsed && '-ml-[16.5rem]',
		isExercisePage ? 'bg-body' : 'bg-card'
	)}
>
