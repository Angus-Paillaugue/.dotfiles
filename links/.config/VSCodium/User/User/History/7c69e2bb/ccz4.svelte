<script>
	import { Shuffle, List, Route, ChevronRight, Undo2 } from 'lucide-svelte';
...
<button class="absolute top-2 left-[300px]" onclick={() => {collapsed = !collapsed}}>Collapse</button>
<!-- Mobile navbar backdrop -->
{#if mobileMenuOpened}
	<button
		type="button"
		class="max-lg:bg-body/50 max-lg:fixed max-lg:inset-0 max-lg:z-30 max-lg:backdrop-blur-lg"
...
<aside
  class={cn(
    'shrink-0 flex-col rounded-xl border border-border bg-card p-2 pt-3 transition-all duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 mr-2 max-lg:min-w-[300px] lg:w-[16rem]',
    mobileMenuOpened
