...
import { hasPermission, PERMISSIONS as P } from '@shared/roles';
...
const routesPermissions: { [key: string]: string | string[] } = {
  ...
  '/app/logs/:id': [P.READ_LOG, P.DELETE_LOG],
  '/app/servers': [P.READ_SERVER, P.CREATE_SERVER],
  '/app/servers/:id': [P.READ_SERVER, P.UPDATE_SERVER, P.DELETE_SERVER],
  '/app/log-out': P.ANY,
};
  ...
  for (const route in routesPermissions) {
    if (isPathMatching(pathname, route)) {
      ...
      if (Array.isArray(routesPermissions[route])) {
        console.log('route', route, 'permissions', routesPermissions[route]);
        return hasPermission(user?.role, ...routesPermissions[route]);
      }
