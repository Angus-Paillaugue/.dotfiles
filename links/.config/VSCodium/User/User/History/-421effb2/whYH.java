...
public class GarantDAOTests {
  ...
  public void setUp() throws SQLException {
    ...
    this.location =
        new Location(123, p, new LinkedList<Personne>(), LocalDate.of(2022, 02, 5), adresse, 500.f);
  ...
  public void testAjouterGarant() {
    ...
    this.location = garantDAO.ajouterGarant(this.personne, this.location, false);
    assertEquals(1, this.location.getGarants().size());
  }
  ...
  public void testSupprimerGarant() {
    ...
    locationDAO.create(location, logement);
    this.location = garantDAO.ajouterGarant(this.personne, this.location, false);
    this.location = garantDAO.supprimerGarant(this.personne, this.location, false);
    assertEquals(0, this.location.getGarants().size());
  }
  ...
  public void testSupprimerTousLesGarants() {
    ...
    locationDAO.create(location, logement);
    this.location = garantDAO.ajouterGarant(this.personne, this.location, false);
    this.location = garantDAO.supprimerTousLesGarants(this.location, false);
    assertEquals(0, this.location.getGarants().size());

    Location databaseLocation = locationDAO.find(this.location.getId());
    assertEquals(0, databaseLocation.getGarants().size());
  }
}
