...
public class Proprietaire {
  ...
  public List<String> adressebiensLouables() {
	...
	public Map<Integer,Personne> getLocataires() {
		Map<Integer, Personne> locataires = new HashMap<Integer, Personne>();
		for (Batiment bat : this.getBatiment()) {
			for (BienLouable bien : bat.getBiensLouables()) {
				if(bien.isColocation()) {
					for(Personne personne : bien.getColocation().getColocataires().keySet()) {
						locataires.put(personne.getIdLocataire(), personne);
					}
				} else {
					for(Personne personne : bien.getLocation().getLocataires()) {
						locataires.put(personne.getIdLocataire(), personne);
					}
				}
			}
