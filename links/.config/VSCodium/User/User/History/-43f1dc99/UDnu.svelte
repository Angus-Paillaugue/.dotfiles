<script lang="ts">
  ...
  async function deleteLogs() {
    const localLogIds = Array.from(document.querySelectorAll('.log-row-checkbox')).filter(checkbox => (checkbox as HTMLInputElement).checked);
    const req = await fetch(API_URL+'/logs', { method: 'DELETE', body: JSON.stringify({ logIds:localLogIds }) });
    if(req.status !== 200) return;
    logs = logs.filter(log => !localLogIds.includes(log.id));
    logIds = new Set([...localLogIds]);
  }
...
<div class="flex flex-row items-start justify-between">
  ...
  {#if deleteRowsButtonVisible}
    <button class="btn" onclick={() => console.log('Delete rows')}>Delete selected rows</button>
  {/if}
</div>
