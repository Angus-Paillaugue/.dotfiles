<script lang="ts">
  ...
  let numberOfSelectedRows: number = $state(0);
  ...
  function onclick(e: MouseEvent) {
    ...
    if(e.target && (e.target as HTMLElement).closest('.log-row-checkbox')) {
      ...
      numberOfSelectedRows = checked.length;
    }
  }
</script>
...
<div class="flex flex-row items-start justify-between">
  ...
  {#if numberOfSelectedRows > 0}
    <button class="btn" onclick={deleteLogs}>Delete {numberOfSelectedRows} row{numberOfSelectedRows > 1 ? 's' : ''}</button>
  {/if}
</div>

<!-- Context menu -->
<div class="absolute transition-opacity z-20 flex flex-col overflow-hidden rounded-lg border border-neutral-800 bg-neutral-950/30 backdrop-blur-md" bind:this={contextMenu} style="opacity: 0;">
