<script lang="ts">
  ...
  function onclick(e: MouseEvent) {
    ...
    if(e.target && !(e.target as HTMLElement).closest('.log-row-checkbox')) {
      if(e.shiftKey) {
        const start = logs.findIndex(log => log.id === parseInt((e.target as HTMLElement).closest('.log-row')?.getAttribute('data-log-id')));
        const end = logs.findIndex(log => log.id === parseInt((e.target as HTMLElement).closest('.log-row')?.getAttribute('data-log-id')));
      }
    }
  }
</script>
