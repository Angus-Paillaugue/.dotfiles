...
test('should display started learning paths', async ({ page }) => {
  ...
  const startedPathsCount = await page.locator('div:has-text("Continue Learning") + div a').count();
...
test('should display not started learning paths', async ({ page }) => {
  ...
  const notStartedPathsCount = await page.locator('h1:has-text("Continue Learning") + div a').count();
...
test('should display progress bar for started paths', async ({ page }) => {
  const progressBar = page.locator('h1:has-text("Continue Learning") + div a div.relative.h-2');
  await expect(progressBar).toBeVisible();
});

test('should display correct progress text for started paths', async ({ page }) => {
  const progressText = page.locator(
		'h1:has-text("Continue Learning") + div a div.flex-row span.text-lg'
	);
