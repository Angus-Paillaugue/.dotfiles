...
<div class="mx-auto max-w-lg w-full boreder rounded-2xl mt-10 flex flex-col gap-4">
  ...
  <div class="flex flex-col gap-4">
    {#each tasks as task, i}
      <div class="flex flex-row gap-2" in:fly={{ easing:backOut, duration: 500, y:'-100%' }} out:fly={{ easing:backOut, duration: 500, y:'-100%' }}>
        <input type="checkbox" bind:checked={task.checked}>
        <p>{task.value}</p>
        <button onclick={() => {tasks.splice(i, 1)}}>Remove</button>
      </div>
    {/each}
  </div>
</div>
