...
We have created packages in multiple languages : (Javascript/Typescript and python).
 ...
 type Config = {
   ...
   bufferSize?: number; // An optional value (defaults to 10) that is used to cache non-essential logs (fatal, error and warning are omitted). When reaching this size, the batched logs will be sent as a batch. Set to 0 to ignore ans send logs imediately and individually.
   serverId: number
 };
 ```
