...
const handleBookmarkSettings = (index: number) => {
  ...
  deleteBookmarkButtonEl.addEventListener("click", () => {
    ...
    const moveBookmarkUpButtonEl = document.getElementById(`bookmark-${index}-move-bookmark-up`) as HTMLButtonElement
    moveBookmarkUpButtonEl.onclick = () => moveBookmark(index, "up");
};
...
const moveBookmark = (index: number, direction: "up" | "down") => {
  ...
  if (direction === "down" && index === totalBookmarks - 1) return;

  const bookmarkToMove = document.getElementById(`bookmark-user-defined-item-${index}`) as HTMLDivElement;
  const bookmarkToSwap = document.getElementById(`bookmark-user-defined-item-${direction === "up" ? index - 1 : index + 1}`) as HTMLDivElement;

  const uselessTitleToMove = document.getElementById(`bookmark-${index}-user-defined-useless-title`) as HTMLSpanElement;
  const uselessTitleToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-user-defined-useless-title`) as HTMLSpanElement;

  const nameInputElToMove = document.getElementById(`bookmark-${index}-name-input`) as HTMLInputElement;
  const urlInputElToMove = document.getElementById(`bookmark-${index}-url-input`) as HTMLInputElement;
  const colorInputElToMove = document.getElementById(`bookmark-${index}-color-input`) as HTMLInputElement;
  const iconTypeInputElToMove = document.getElementById(`bookmark-${index}-icon-type-input`) as HTMLInputElement;
  const iconColorInputElToMove = document.getElementById(`bookmark-${index}-icon-color-input`) as HTMLInputElement;

  const nameInputElToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-name-input`) as HTMLInputElement;
  const urlInputElToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-url-input`) as HTMLInputElement;
  const colorInputElToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-color-input`) as HTMLInputElement;
  const iconTypeInputElToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-icon-type-input`) as HTMLInputElement;
  const iconColorInputElToSwap = document.getElementById(`bookmark-${direction === "up" ? index - 1 : index + 1}-icon-color-input`) as HTMLInputElement;

  const collapsibleContentElToMove = document.getElementById(`bookmark-${index}-collapsible-content`) as HTMLDivElement;
  const collapsibleContentElToSwap
    ...
    <div id="bookmark-user-defined-item-${index}" class="bg-neutral-800 p-2 rounded-md grid grid-flow-row gap-4">
      <div class="grid grid-cols-[auto_max-content_max-content]">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button id="bookmark-${index}-move-bookmark-up" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer">
            <i class="text-white ri-arrow-up-line"></i>
          </button>
          <button id="bookmark-${index}-move-bookmark-down" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer">
