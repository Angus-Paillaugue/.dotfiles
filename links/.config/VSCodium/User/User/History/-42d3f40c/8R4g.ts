...
const handleBookmarkSettings = (index: number) => {
  ...
  deleteBookmarkButtonEl.addEventListener("click", () => {
    ...
    // The first bookmark can't be moved up
    if(index > 0) {
    ...
    // The last bookmark can't be moved down
    if (index < bookmarksUserDefinedList.children.length - 1) {
      const moveBookmarkDownButtonEl = document.getElementById(`bookmark-${index}-move-bookmark-down`) as HTMLButtonElement
      moveBookmarkDownButtonEl.onclick = () => moveBookmark(index, "down");
    }
};

const moveBookmark = (index: number, direction: "up" | "down") => {
  console.log("Moving bookmark", index, "(" + bookmarksUserDefinedList.children[index] + ")", direction);
  ...
  if (totalBookmarks > 0) {
    const bookmarkToSwap = document.getElementById(`bookmark-user-defined-item-${newIndex}`) as HTMLDivElement;
    bookmarksUserDefinedList.insertBefore(
      direction === "up" ? bookmarkToMove : bookmarkToSwap,
      direction === "up" ? bookmarkToSwap : bookmarkToMove
    );
    const firstButton = document.getElementById(
      `bookmark-0-move-bookmark-down`
    ) as HTMLButtonElement;
    firstButton.disabled = true;

    const lastButton = document.getElementById(
      `bookmark-${totalBookmarks - 1}-move-bookmark-down`
    ) as HTMLButtonElement;
    lastButton.disabled = true;
  }
    const moveBookmarkUpButtonEl = document.getElementById(
      `bookmark-${index}-move-bookmark-up`
    ) as HTMLButtonElement;
  const moveBookmarkDownButtonEl = document.getElementById(`bookmark-${index}-move-bookmark-down`) as HTMLButtonElement;
  moveBookmarkUpButtonEl.disabled = newIndex === 0;
  moveBookmarkDownButtonEl.disabled = newIndex === totalBookmarks - 1;
  bookmarksUserDefinedList.insertBefore(bookmarkToMove, direction === "up" ? bookmarkToMove.nextElementSibling : bookmarkToMove.previousElementSibling);
}
    ...
    <div id="bookmark-user-defined-item-${index}" class="bg-neutral-800 p-2 rounded-md grid grid-flow-row gap-4">
      <div class="grid grid-cols-[auto_max-content_max-content]">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button ${
            index === 0 ? "disabled" : ""
          } id="bookmark-${index}-move-bookmark-up" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-300 disabled:cursor-not-allowed">
            <i class="text-white ri-arrow-up-line"></i>
          </button>
          <button ${
            index === bookmarksUserDefinedList.children.length - 1 ? "disabled" : ""
          } id="bookmark-${index}-move-bookmark-down" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-300 disabled:cursor-not-allowed">
