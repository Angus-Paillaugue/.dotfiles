...
export const fillBookmarksInputs = (config: Config) => {
  ...
  const addMoveButtonsEventListeners = (index: number) => {
    const moveUpButton = document.getElementById(
      `bookmark-${index}-move-up-button`
    ) as HTMLButtonElement;
    const moveDownButton = document.getElementById(
      `bookmark-${index}-move-down-button`
    ) as HTMLButtonElement;

    moveUpButton.addEventListener("click", () => moveBookmark(index, "up"));
    moveDownButton.addEventListener("click", () => moveBookmark(index, "down"));
  };
  ...
  config.bookmarks.userDefined.forEach((bookmark, index) => {
    ...
    addMoveButtonsEventListeners(index);
  });
...
const moveBookmark = (index: number, direction: "up" | "down") => {
  const bookmarksList = document.getElementById("bookmarks-user-defined-list") as HTMLDivElement;
  const bookmarkElements = Array.from(bookmarksList.children) as HTMLDivElement[];

  console.log(`Attempting to move bookmark at index ${index} ${direction}`);

  if (direction === "up" && index > 0) {
    console.log(`Moving bookmark at index ${index} up`);
    bookmarksList.insertBefore(bookmarkElements[index], bookmarkElements[index - 1]);
  } else if (direction === "down" && index < bookmarkElements.length - 1) {
    console.log(`Moving bookmark at index ${index} down`);
    bookmarksList.insertBefore(bookmarkElements[index + 1], bookmarkElements[index]);
  } else {
    console.log(`Cannot move bookmark at index ${index} ${direction}`);
  }

  // Update the indices and IDs of the bookmarks
...
const updateBookmarkIndices = () => {
  const bookmarksList = document.getElementById("bookmarks-user-defined-list") as HTMLDivElement;
  const bookmarkElements = Array.from(bookmarksList.children) as HTMLDivElement[];

  bookmarkElements.forEach((bookmarkEl, newIndex) => {
    bookmarkEl.id = `bookmark-${newIndex}-container`;
    bookmarkEl.querySelectorAll("input, button").forEach((inputEl) => {
      const idParts = inputEl.id.split("-");
      idParts[1] = `${newIndex}`;
      inputEl.id = idParts.join("-");
    });
  });
};
...
const reattachEventListeners = () => {
  ...
  bookmarkElements.forEach((bookmarkEl, index) => {
    // Clone the element to remove existing event listeners
    const clonedBookmarkEl = bookmarkEl.cloneNode(true) as HTMLDivElement;

    // Replace the original element with the cloned one
    bookmarksList.replaceChild(clonedBookmarkEl, bookmarkEl);

    // Attach new event listeners to the cloned element
    const moveUpButton = clonedBookmarkEl.querySelector(
      `#bookmark-${index}-move-up-button`
    ) as HTMLButtonElement;
    const moveDownButton = clonedBookmarkEl.querySelector(
      `#bookmark-${index}-move-down-button`
    ) as HTMLButtonElement;
...
const handleBookmarkSettings = (index: number) => {
  ...
  deleteBookmarkButtonEl.addEventListener("click", () => {
    ...
    for (let i = index + 1; i < totalBookmarks; i++) {
      ...
      const oldColorContainerEl = document.getElementById(
        `bookmark-${i}-color-container`
      ) as HTMLInputElement;
    ...
    <div id="bookmark-user-defined-item-${index}" class="bg-neutral-800 p-2 rounded-md grid grid-flow-row gap-4">
      <div class="grid grid-cols-[auto_max-content_max-content]">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button ${
            index === 0 ? "disabled" : ""
          } id="bookmark-${index}-move-up-button" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-300 disabled:cursor-not-allowed">
            <i class="text-white ri-arrow-up-line"></i>
          </button>
          <button ${
            index === bookmarksUserDefinedList.children.length - 1 ? "disabled" : ""
          } id="bookmark-${index}-move-down-button" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-300 disabled:cursor-not-allowed">
        ...
        <div class="grid gap-2">
          ...
          <div
            ...
            <input id="bookmark-${index}-name-input" type="text" autocomplete="off"
              class="outline-none bg-transparent text-white placeholder-neutral-500" placeholder="input name..." value="${
                bookmark.name
              }">
          </div>
        </div>
        <div class="grid gap-2">
          ...
          <div
            ...
            <input id="bookmark-${index}-url-input" type="text" autocomplete="off"
              class="outline-none bg-transparent text-white placeholder-neutral-500" placeholder="input url..." value="${
                bookmark.url
              }">
          </div>
        </div>
        <div class="grid gap-2">
          ...
          <div
            ...
            <input id="bookmark-${index}-color-input" type="text" autocomplete="off"
              class="outline-none bg-transparent text-white placeholder-neutral-500" placeholder="input color..." value="${
                bookmark.color
              }">
          </div>
        </div>
        <div class="grid gap-2">
          ...
          <div
            ...
            <input id="bookmark-${index}-icon-type-input" type="text" autocomplete="off"
              class="outline-none bg-transparent text-white placeholder-neutral-500" placeholder="input icon type..." value="${
                bookmark.iconType
              }">
          </div>
        </div>
