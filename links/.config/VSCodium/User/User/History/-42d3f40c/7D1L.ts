...
const handleBookmarkSettings = (index: number) => {
  ...
  deleteBookmarkButtonEl.addEventListener("click", () => {
    ...
    if(index > 0) {
      ...
      const moveBookmarkUpButtonEl = document.getElementById(`bookmark-${index}-move-bookmark-up`) as HTMLButtonElement
      moveBookmarkUpButtonEl.setAttribute("disabled", "true");
    }
    ...
    if (index < bookmarksUserDefinedList.children.length - 1) {
      ...
      const moveBookmarkDownButtonEl = document.getElementById(`bookmark-${index}-move-bookmark-down`) as HTMLButtonElement
      moveBookmarkDownButtonEl.setAttribute("disabled", "true");
    }
};
...
const moveBookmark = (index: number, direction: "up" | "down") => {
  ...
  bookmarksUserDefinedList.insertBefore(bookmarkToMove, bookmarkToSwap);
}
    ...
    <div id="bookmark-user-defined-item-${index}" class="bg-neutral-800 p-2 rounded-md grid grid-flow-row gap-4">
      <div class="grid grid-cols-[auto_max-content_max-content]">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button disabled=${
            index === 0
          } id="bookmark-${index}-move-bookmark-up" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-200">
            <i class="text-white ri-arrow-up-line"></i>
          </button>
          <button disabled=${
            index === bookmarksUserDefinedList.children.length - 1
          } id="bookmark-${index}-move-bookmark-down" class="bg-neutral-500 hover:bg-neutral-600 transition aspect-square rounded-md cursor-pointer disabled:bg-neutral-200">
