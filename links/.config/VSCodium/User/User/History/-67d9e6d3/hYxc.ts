...
export async function logsWs(ws: WebSocket, req: Request) {
  ...
  const logsDeleted = async (logs: Log[]) => {
    ws.send(JSON.stringify({ action: 'logsDeleted', success: true, logs }));
  };

  // When the lient requests data
  ws.on('message', async (msg: WebSocket.MessageEvent) => {
    const { action, page = 0, pageSize = 30, logIds, realTime = true } = JSON.parse(msg as unknown as string);
    ...
    switch (action) {
      ...
      case 'deleteLogs': {
        ...
        try {
          ...
          const logs = await LogDAO.getLogs(pageSize, page);
          logEventEmitter.emit('logsDeleted');
          ws.send(JSON.stringify({ action: 'newLogs', success: true, logs }));
        } catch (error) {
