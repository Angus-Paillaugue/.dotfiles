...
export async function logsWs(ws: WebSocket, req: Request) {
  ...
  ws.on('message', async (msg: WebSocket.MessageEvent) => {
    const { action, page, limit, level = 'all', id, logIds } = JSON.parse(msg as unknown as string);

    switch (action) {
      case 'fetchLogs': {
        try {
          const logs = await LogDAO.getLogs(limit || MAX_LOGS_SENT_PER_REQUEST, page || 0, level);
          ...
          ws.send(
            JSON.stringify({
              ...
              ...logs,
              success: true,
              offset: (page || 0) + logs.logs.length
            })
          );
        } catch (error) {
