	...
	<div class={cn("fixed left-0 right-0 top-4 flex flex-row items-start justify-center", className)}>
		<div class="w-fit relative flex flex-col items-center justify-start">
			{#each $toasts.slice(0, 1) as toast (toast.id)}
				...
				<div
					class="group w-[250px] md:w-[300px] flex cursor-pointer flex-row items-center justify-between gap-4 rounded-xl bg-body-dark dark:bg-neutral-800 p-3 text-text-heading-dark transition-all hover:h-52 hover:w-[500px] h-12 md:h-14 relative shake shadow-lg overflow-hidden"
					data-id={toast.id}
				...
				>
					...
					<div
						...
						class="flex grow flex-col justify-between overflow-hidden opacity-0 duration-300 group-hover:opacity-100 h-full transition-all"
					>
					...
					<h3 class="text-xl m-0 font-bold absolute top-1/2 -translate-y-1/2 duration-200 right-3 left-10 text-end overflow-hidden group-hover:translate-x-full transition-all {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
						{toast.title}
					</h3>
				</div>
			{/each}
