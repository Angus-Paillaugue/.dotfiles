...
export const actions = {
	async changePassword({ request, locals }) {
		...
		if (newPassword !== confirmPassword) return fail(400, { error: m.app_account_settings_actions_errors_passwords_dont_match() });
		...
		try {
			...
			if (!dbUser) return fail(400, { error: m.app_account_settings_actions_errors_user_not_found() });

			const compare = await bcrypt.compare(oldPassword, dbUser[0].password_hash);
			if (!compare) return fail(400, { error: m.app_account_settings_actions_errors_incorrect_password() });
			// Update password
	...
	async saveInfos({ request, locals, cookies }) {
		...
		try {
			...
			if (usernameIsTakenValue) return fail(400, { error: m.app_account_settings_actions_username_is_taken() });
	...
	async saveProfilePicture({ request, locals }) {
		...
		if (picture.size > 1024 * 1024) return fail(400, { error: m.app_account_settings_actions_image_is_too_large() });
