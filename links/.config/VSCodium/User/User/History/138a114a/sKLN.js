...
export const actions = {
	...
	async saveProfilePicture({ request, locals }) {
		...
		try {
			...
			const publicPicturePath = `/profile_picture/${user.id}${imageExtension}`;
			// Save picture
			// await writeFile(
			// 	`${env.PWD}/uploads/profile_pictures/${user.id}${imageExtension}`,
			...
			await resizedImage.toFile(`${env.PWD}/uploads/profile_pictures/${user.id}${imageExtension}`);
