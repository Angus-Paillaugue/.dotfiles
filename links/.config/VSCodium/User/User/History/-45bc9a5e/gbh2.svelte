<script lang="ts">
  ...
  function exportQueryResults() {
    ...
    if(exportLogsAmount === 'all') {
      const res = fetch('/api/logs/query', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ query: queryInputValue, lastQueryValue, currentPage, all: true })
      });
    }
    const exportCSV = () => {
...
<!-- Export logs modal -->
<Dialog.Root
  bind:open={exportLogsModalOpen}
>
  <Dialog.Content>
    <Dialog.Header>
      <Dialog.Title>Export logs</Dialog.Title>
      <Dialog.Description>
        Export logs in your favorite format: JSON, CSV and more.
      </Dialog.Description>
    </Dialog.Header>
    <div class="flex flex-col">
      <h2 class=" text-lg fornt-semibold">Amount</h2>
      <RadioGroup.Root bind:value={exportLogsAmount} class="grid grid-cols-2 gap-2">
      ...
      <h2 class="mt-4 text-lg fornt-semibold">Format</h2>
    ...
    <Dialog.Footer>
      <Button type="submit" disabled={!exportLogsFormat} onclick={exportQueryResults}>Export</Button>
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
      ...
      <div class="no-scrollbar max-h-full overflow-auto">
        ...
        <div class="flex flex-row items-center justify-between gap-6">
          <div class="flex flex-row items-center">
            <Button
              onclick={() => (exportLogsModalOpen = true)}
            >Export</Button>
          </div>
          <div class="flex flex-row gap-6 items-cente">
