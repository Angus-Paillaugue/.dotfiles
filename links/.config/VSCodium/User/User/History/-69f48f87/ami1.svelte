<script lang="ts">
  ...
  let { card = $bindable(), onDragStart, onDragEnd, deleteCard, onEditCard }: { card: Card; onDragStart: (card: Card) => void; onDragEnd: () => void; deleteCard: (card:Card) => void; onEditCard: (card: Card) => void } = $props();
  ...
  <Dialog.Content>
    ...
    <form onsubmitcapture={editCard} class="space-y-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <Input type="text" name="description" placeholder="Description" bind:value={editCardData.description} />
      </div>
      ...
      <div class="flex flex-col gap-1">
        ...
        <Textarea name="query" bind:value={card.request} />
      </div>
...
<ContextMenu.Root>
  ...
  <ContextMenu.Content>
    ...
    <ContextMenu.Item variant="destructive" class="flex flex-row gap-2 items-center" onclick={() => (deleteCardConfirmModalOpen = true)}>
