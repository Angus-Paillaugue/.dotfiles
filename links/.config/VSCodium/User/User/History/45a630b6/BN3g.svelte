	...
	>
		{#each projects[$locale] as project, index}
			...
			>
				...
				></div>
				<p
					class="absolute z-20 top-4 right-4 text-neutral-200 bg-white/30 text-sm px-2 py-1 rounded-full"
				>
					{project.date}
				</p>
				<div class="px-4 mt-auto z-10 h-1/4 justify-evenly flex flex-col {project.bgColor}">
					...
					<div class="flex flex-nowrap overflow-auto no-scrollbar gap-4 flex-row">
						{#each project.tags as tag}
							<span class="mix-blend-difference gray bg-white/30 text-xs px-2 py-1 rounded-full">{tag}</span>
						{/each}
					</div>
				</div>
