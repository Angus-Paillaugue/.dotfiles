...
<section id="projects" class="max-w-screen-xl mx-auto p-4">
	...
	>
		{#each projects[$locale ?? 'en'] as project, index}
		<!-- Projects card -->
			<div
	...
	<div class="flex flex-row-items-center justify-end gap-8 mt-4">
		...
		<button
			...
			onclick={() => {
				carouselIndex = Math.min(carouselIndex + 1, projects[$locale ?? 'en'].length);
			}}
			disabled={projects[$locale ?? 'en'].length - displayedElements <= carouselIndex}
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
