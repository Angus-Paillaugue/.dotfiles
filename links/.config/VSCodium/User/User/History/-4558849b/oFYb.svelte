<script>
  ...
  onMount(() => {
    const sideBarScrollContainer = document.querySelector('#sidebar div.lenis-prevent');
    const gradientTop = sideBarScrollContainer.querySelector('.bg-gradient-to-b');
    const gradientBottom = sideBarScrollContainer.querySelector('.bg-gradient-to-t');

    sideBarScrollContainer.addEventListener('scroll', (e) => {
      const tocContainer = e.target;
      const scrollPosition = tocContainer.scrollTop;
      gradientTop.style.opacity = scrollPosition > 0 ? 1 : 0;
      gradientBottom.style.opacity = scrollPosition < tocContainer.scrollHeight - tocContainer.clientHeight ? 1 : 0;
      gradientTop.style.top = `${scrollPosition}px`;
      gradientBottom.style.bottom = `${tocContainer.scrollHeight - scrollPosition + tocContainer.clientHeight}px`;
      const scrollPercentage = (scrollPosition / (tocContainer.scrollHeight - tocContainer.clientHeight)) * 100;
...
>
  ...
  <div class="lenis-prevent grow overflow-y-auto relative">
    <div class="absolute top-0 pointer-events-none left-0 transition-opacity right-0 h-20 bg-gradient-to-b from-body dark:from-body-dark to-transparent"></div>
    <div class="absolute bottom-0 pointer-events-none left-0 transition-opacity right-0 h-20 bg-gradient-to-t from-body dark:from-body-dark to-transparent"></div>
