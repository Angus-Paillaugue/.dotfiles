<script lang="ts">
  ...
  function handleTouchEnd(event: TouchEvent) {
    ...
    if (dropTarget && dropTarget.dataset.id !== item.id.toString() && ondrop) {
...
<div
  class={cn("flex flex-row gap-1 group h-8 items-center relative before:absolute before:h-full before:bg-primary before:left-0 [.drop-target]:before", isDragging ? 'opacity-50' : '' ,isDropTarget && 'before:w-1')}
  data-note-item
  data-id={item.id}
