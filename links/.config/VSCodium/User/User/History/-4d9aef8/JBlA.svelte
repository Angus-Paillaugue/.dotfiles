<script lang="ts">
  ...
  function handleDrop(event: DragEvent) {
    event.preventDefault();
    (event.currentTarget as HTMLElement).classList.add('drop-target'); // Reset before dispatch
    const draggedId = event.dataTransfer?.getData("text/plain");
...
<div
  class={cn("flex flex-row gap-1 group h-8 items-center relative before:absolute before:h-full before:bg-primary before:left-0", isDragging ? 'opacity-50' : '' ,isDropTarget && 'before:w-1')}
  data-note-item
  data-id={item.id}
