...
import { eq, sql, and, jsonArrayAgg } from 'drizzle-orm';
		...
		const exercises = await db
			.select({
				...
				tests: db
					...
					.from(exerciseTests)
					.where(eq(exerciseTests.exerciseId, exercises.id)),
				submissions: db
					.select(
						jsonArrayAgg(
							jsonObject({
					...
					.where(
						and(
							eq(submissions.exerciseId, exercises.id),
							eq(submissions.userId, user.id)
						)
					)
					.having(count(submissions.id).gt(0))
			})
			.from(exercises)
			.leftJoin(
				submissions,
			...
			.leftJoin(languagesTable, eq(exercises.languageId, languagesTable.id))
			.where(eq(exercises.id, identifier.id))
			.groupBy(
				exercises.id,
				exercises.title,
				exercises.description,
		...
		const exercises = await db
			.select({
				...
				submissions: db
					...
					.from(submissions)
					.where(and(eq(submissions.exerciseId, exercises.id), eq(submissions.userId, userId)))
					.having(count(submissions.id).gt(0))
			})
			.from(exercises)
			.leftJoin(
			...
			.where(eq(exercises.slug, identifier.slug))
			.groupBy(
				exercises.id,
				exercises.title,
				exercises.description,
