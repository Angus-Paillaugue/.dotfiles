<script lang="ts">
  ...
  let selectedLog: Record<string, Log> | null = $state(null);

  async function refresh() {
    await getResults({ pageNumber: currentPage, pageSize: pageSize });
  }
...
<div class={cn("flex flex-col w-full overflow-x-auto", className)}>
  <!-- Table heading -->
  ...
  <!-- Table footer -->
  {#if rows.length > 0}
    <div class="flex flex-wrap flex-row gap-2 items-center justify-between p-2">
