<script lang="ts">
  ...
  async function refresh() {
    await getResults({ pageNumber: currentPage, pageSize: resultsPerPage });
  }
...
<div class={cn("flex flex-col gap-4", className)}>
  {@render children?.()}
  <div class="flex flex-row items-center pb-4 pt-2 px-2">
  ...
  {#if rows.length > 0}
    <!-- Table footer -->
    <div class="flex flex-row items-center justify-between pt-4 pb-2 px-2">
