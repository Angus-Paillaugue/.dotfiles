...
installBasePrograms() {
  ...
  sudo apt update -y
  sudo apt upgrade -y
  sudo apt install git gh curl wget zsh bat fzf gnome-tweaks chrome-gnome-shell gnome-shell-extensions dconf-editor openjdk-17-jdk software-properties-common apt-transport-https jq python3 python3-pip dconf-cli uuid-runtime tree sassc lowdown cava pipx cmatrix -y
  pipx ensurepath
  chsh -s $(which zsh)
