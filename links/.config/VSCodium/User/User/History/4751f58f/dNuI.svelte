<script>
  ...
  const styleClasses = {
    "ios": "",
    "default": "fixed right-2 bottom-2 flex flex-col-reverse"
  };
</script>
...
{#each $toasts.slice(0, 1) as toast, i (toast.id)}
  ...
  <div
    class={cn("group/toast w-[250px] md:w-[300px] flex cursor-pointer flex-row items-center justify-between gap-4 rounded-[1.75rem] bg-body-dark dark:bg-neutral-800 p-3 text-text-heading-dark transition-all hover:h-52 hover:w-[500px] h-12 md:h-14 relative shake shadow-lg", styleClasses[style])}
    data-id={toast.id}
