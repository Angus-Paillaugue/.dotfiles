...
{#each $toasts.slice(0, 1) as toast, i (toast.id)}
  ...
  <div
    class="group/toast w-[250px] md:w-[300px] flex cursor-pointer flex-row items-center justify-between gap-4 rounded-[1.75rem] bg-body-dark dark:bg-neutral-800 p-3 text-text-heading-dark dark:text-text-heading transition-all hover:h-52 hover:w-[500px] h-12 md:h-14 relative shake shadow-lg"
    data-id={toast.id}
