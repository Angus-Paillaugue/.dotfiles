...
async function checkTranslations() {
	...
	if (PARAMS.checkEmptyFields) {
		translations.forEach((translation, index) => {
			const emptyFields = Object.entries(translation).filter(([key, value]) => value === '');
			if (emptyFields.length > 0) {
				hasErrors.push(RED('✖') + ` Empty fields in ${translationFiles[index]}:`);
				emptyFields.forEach(([key, value]) => console.log(` - ${key}`));
			}
		});
	}
