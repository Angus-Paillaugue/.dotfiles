...
export async function updateUser(id: number, { username, role} : { username: string; role: Role}): Promise<ResultSetHeader> {
  const [rows] = await db.execute('UPDATE user SET username = ? WHERE id = ?', ['username', 1]);
  return rows as ResultSetHeader;
}
