...
export async function POST({ request, locals }) {
	...
	const initialCategoriesMap = new Map(initialCategories.map((c) => [c, c]));

	const categoriesToInsert = newCategories.filter((c) => !initialCategoriesMap.has(c));
	const categoriesToDelete = initialCategories.filter(
		(c) => !newCategories.some((nc) => nc === c)
	);
