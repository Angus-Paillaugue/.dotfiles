...
export async function POST({ request, locals }) {
	...
	const categoriesToInsert = newCategories
		.filter((id) => !initialCategories.some((category) => category.id === id));

	// Categories to delete (initialCategories but not in newCategories)
	const categoriesToDelete = initialCategories.filter(
		(category) => !newCategories.includes(category.id)
	);
