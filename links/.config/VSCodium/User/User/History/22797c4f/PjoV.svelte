<script>
  ...
  function setTabIndex() {
    ...
    const detailsElements = details.querySelectorAll('a, button, input, select, textarea');

    detailsElements.forEach((el) => {
      if (open) {
        el.setAttribute('tabindex', el.getAttribute('data-tabindex') ?? -1);
      } else {
        el.setAttribute('data-tabindex', el.getAttribute('tabindex') ?? -1);
