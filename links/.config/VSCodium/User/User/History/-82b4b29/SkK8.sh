...
originalDirSize=$(du -h "$inputPath" | cut -f1)
...
for file in $files; do
  ...
  if [ -f "$file" ]; then
    ...
    rm "$file"
    mv "$outputName" "$file"
    echo -e " $GREEN✓$NC Compressed $fileName ($RED$baseSize$NC → $GREEN$newSize$NC)"
  fi
done
...
finalDirSize=$(du -h "$inputPath" | cut -f1)
originalSize=$(du -s -h "$inputPath" | cut -f1)
compressionRate=$(echo "scale=2; ($originalSize - $(du -s -h "$inputPath" | cut -f1)) / $originalSize * 100" | bc)
echo -e " $GREEN✓$NC Reduced the directory size : $originalDirSize → $finalDirSize (-$compressionRate%)"
