<script>
	...
	import { Check, AlignLeft, RotateCcw, Plus, Minus, WrapText, Save, AlignJustify } from 'lucide-svelte';
...
<div class="flex h-full grow flex-col overflow-hidden">
	...
	>
		...
		<Dropdown align="start">
			<Dropdown.Trigger variant="none" class="w-fit gap-2 rounded-lg px-3 py-1 text-base text-neutral-100 hover:bg-neutral-700/50">File</Dropdown.Trigger>
			{#snippet items()}
				<!-- Save -->
				<Dropdown.Item aria-label="Save" onclick={saveTryValue}>
					...
					Save
					<div class="ml-auto">
						<kbd class="px-1 text-xs py-0.5">Ctrl</kbd>
						<kbd class="px-1 text-xs py-0.5">S</kbd>
					</div>
				</Dropdown.Item>
				<!-- Format -->
				<Dropdown.Item onclick={formatCode} aria-label="Format code" disabled={formatted}>
					...
					Format
					<div class="ml-auto">
						<kbd class="px-1 text-xs py-0.5">Alt</kbd>
						<kbd class="px-1 text-xs py-0.5">Shift</kbd>
						<kbd class="px-1 text-xs py-0.5">F</kbd>
					</div>
				</Dropdown.Item>
				<!-- Word wrap -->
				<Dropdown.Item aria-label="Toggle word wrap" onclick={() => (wordWrap = !wordWrap)}>
					...
					<div>Turn
						{#if wordWrap}
							<div class="inline-block" in:slide={{ axis: 'y' }}> off </div>
						{:else}
							<div class="inline-block" in:slide={{ axis: 'y' }}> on </div>
						{/if}
						word wrap
					</div>
					<div class="ml-auto">
						<kbd class="px-1 text-xs py-0.5">Alt</kbd>
						<kbd class="px-1 text-xs py-0.5">Z</kbd>
					</div>
				</Dropdown.Item>
				<!-- Reset -->
				{#if defaultValue}
