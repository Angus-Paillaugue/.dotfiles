	...
	>
		...
		<Dropdown align="start">
			...
			{#snippet items()}
				...
				<!-- Format -->
				<Dropdown.Item onclick={formatCode} aria-label={m.components_editor_dropdown_format()} disabled={formatted}>
					{#if formatted}
						<span in:scale>
							<Check class="size-5" />
						</span>
				...
				<!-- Word wrap -->
				<Dropdown.Item aria-label={m.components_editor_dropdown_toggle_word_wrap()} onclick={() => (wordWrap = !wordWrap)}>
					{#if wordWrap}
						<WrapText class="size-5" />
					{:else}
						<AlignJustify class="size-5" />
					{/if}
					<p>{wordWrap ? m.components_editor_dropdown_turn_off_word_wrap() : m.components_editor_dropdown_turn_on_word_wrap()}</p>
					<div class="ml-auto">
						<kbd class="px-1 py-0.5 text-xs">Alt</kbd>
				...
				<!-- Reset -->
				{#if defaultValue}
					<Dropdown.Item onclick={resetEditor} aria-label={m.components_editor_dropdown_reset_label()}>
