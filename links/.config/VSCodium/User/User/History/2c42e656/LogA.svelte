<script>
  ...
  import { writable, syncKeyStore } from 'svelte/store';
  ...
  if(syncKey) {
    syncKeyStore = writable({...$syncKeyStore, [syncKey]: 0});
  }

  $effect(() => {
    if(!syncKey) return;

    syncKeyStore.update((value) => {
      value[syncKey] = selectedIndex;
      return value;
    });
  })

  $inspect($syncKeyStore)
</script>
