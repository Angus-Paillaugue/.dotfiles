  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    <div class="bg-neutral-900 transition-colors flex flex-row justify-between p-4 rounded-lg border border-neutral-900">
      ...
      <div class="flex flex-col items-end justify-between">
        <button class="px-3 py-1 rounded bg-green-600 font-mono group flex flex-row text-start no-scale">
        ...
        <button class="px-3 py-1 rounded bg-red-600 font-mono group flex flex-row text-start no-scale">
        ...
        <button class="px-3 py-1 rounded bg-gray-600 font-mono group flex flex-row text-start no-scale">
        ...
        <?php foreach ($playersStats as $joueur): ?>
          <tr class='even:bg-neutral-900 text-base font-base'>
            <td class='px-4 py-1 align-middle'>
              <a class='hover:underline' href='/dashboard/joueur.php?id=<?php echo $joueur['id']; ?>'>
                <?php echo $joueur['prenom'] . ' ' . $joueur['nom']; ?>
              </a>
            </td>
            <td class='px-4 py-1 align-middle'><?php echo $joueur['poste_max_victoire']; ?></td>
            <td class='px-4 py-1 align-middle'><?
              echo $joueur['nbTitulaire'];
            ?></td>
            <td class='px-4 py-1 align-middle'><?php echo $joueur['nbRemplacant']; ?></td>
            <td class='px-4 py-1 align-middle'><?php echo $joueur['moyenne_notes'] ? number_format($joueur['moyenne_notes'], 1) : ''; ?></td>
            <td class='px-4 py-1 align-middle'><?php echo $joueur['pourcentage_victoire_titulaire'] ? number_format($joueur['pourcentage_victoire_titulaire'], 1) : ''; ?></td>
          </tr>
        <?php endforeach ?>
      </tbody>
    </table>
