<script>
  ...
  const { name, description, definition } = $props();
</script>


<div class="rounded-md border border-neutral-300 dark:border-neutral-800">
  <div class="bg-primary-700/50 text-white w-fit p-2 rounded-tl-md rounded-br-md border-2 border-primary-600">
      ...
      <tbody>
        {#each definition.content as prop}
          <tr class="border-b border-neutral-200 dark:border-neutral-900">
            <td class="p-2">
              <p class="font-bold px-1 rounded bg-primary-700/50 border border-primary-600 text-white w-fit text-sm">{prop.name}</p>
            </td>
            <td class="p-2">
              <p class="font-bold px-1 rounded bg-[var(--inline-code-bg)] text-white w-fit text-sm">{prop?.default ?? " - "}</p>
            </td>
            <td class="p-2 flex flex-col gap-2">
              {#if prop.type}
                <p class="font-bold px-1 rounded bg-[var(--inline-code-bg)] text-white w-fit text-sm m-0">{prop.type}</p>
              {/if}
              {#if prop.description}
