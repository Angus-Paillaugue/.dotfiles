        ...
        {#each definition.content as prop}
          <tr class="border-b border-neutral-200 dark:border-neutral-900">
            ...
            {#each Object.entries(prop) as [key, value]}
              <td class="p-2">
                {#if prop.href}
                  <a href={prop.href} class="font-bold px-1 rounded bg-primary-800/50 border border-primary-600 text-white w-fit text-sm">{value}</a>
                {:else}
                  <p class="font-bold px-1 rounded bg-primary-800/50 border border-primary-600 text-white w-fit text-sm">{value}</p>
                {/if}
              </td>
            {/each}
          </tr>
