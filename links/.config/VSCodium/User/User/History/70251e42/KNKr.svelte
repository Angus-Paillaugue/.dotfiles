<script>
  ...
  const getStyles = () => {
    if (position === "top") {
      return `top: ${tooltipCoords.y}px; left: ${tooltipCoords.x}px;`;
...
<button tabindex="0" class="inline-block cursor-pointer underline underline-offset-2 decoration-dashed decoration-primary-600/50 dark:decoration-primary-600/70 font-semibold" bind:this={tooltip}>
  {title}
</button>
...
{#if isHovered}
  ...
  >
    <div class="text-sm dark:bg-neutral-950 bg-neutral-200 text-neutral-500 dark:text-neutral-400 p-2 rounded-md relative">
      {@render children()}
      <svg xmlns="http://www.w3.org/2000/svg" class="size-5 absolute text-neutral-200 dark:text-neutral-950 {arrowPositionClasses[position]}" viewBox="0 0 40 40">
