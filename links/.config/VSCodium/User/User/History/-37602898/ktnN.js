...
async function login(page, origin) {
	...
	const emailInput = await page.$('#logIn input[type="text"]');
	await emailInput.type('Angus');
	const passwordInput = await page.$('#logIn input[type="password"]');
	await passwordInput.type('APAILL40');
	await Promise.all([page.$eval('#logIn', (form) => form.submit()), page.waitForNavigation()]);
}
