...
<section class="relative isolate transform-gpu h-screen flex flex-col md:pb-[300px] md:pt-40" id="home">
  <!-- Grid -->
  <svg
    ...
    {#if $mode}
      <div class="md:row-span-2 max-sm:hidden max-md:self-start overflow-hidden border border-neutral-300/50 dark:border-neutral-700/50 relative rounded-lg" style="animation: float 5s ease-in-out infinite;">
        <picture>
          ...
          <img src="/homepage/mockup/{$mode}/homepagePreview_1920.webp" alt="website mockup" class="w-full rounded-md object-contain h-full">
        </picture>
      </div>
    {/if}
  </div>
