...
public class PersonneDAO implements PersonneDAODef {
  ...
  public Personne create(Personne p, String villeNaissance) {
    try {
      Connector db = Connector.getInstance();

      String query =
          "INSERT INTO Personne (id, nom, prenom, dateNaissance, situationFamiliale, profession, employeur, typeContratDeTravail, remunerationMensuelleNette, autreRevenus, email, telephone, idAdresseResidence, villeNaissance) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      int idPersonne =
          db.executeUpdate(
              query,
              Arrays.asList(
    ...
    try {
      ...
      Personne p =
          new Personne(
              ...
              (String) result.get("villeNaissance"),
  ...
  public Personne update(Personne p, String villeNaissance) {
    try {
      Connector db = Connector.getInstance();
      AdresseDAO adresseDAO = new AdresseDAO();
      adresseDAO.update(p.getAdresse());
      String query =
          "UPDATE Personne SET nom = ?, prenom = ?, dateNaissance = ?, profession = ?, employeur = ?, typeContratDeTravail = ?, remunerationMensuelleNette = ?, autreRevenus = ?, email = ?, telephone = ?, IdAdresseResidence = ?, villeNaissance = ? WHERE id = ?";
      db.executeUpdate(
          query,
          Arrays.asList(
              p.getNom(),
