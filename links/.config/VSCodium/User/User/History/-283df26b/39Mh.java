...
public class PersonneDAO implements PersonneDAODef {
  ...
  public Personne update(Personne p, String adresseNaissance) {
    try {
      ...
      String query =
          "UPDATE Personne SET nom = ?, prenom = ?, dateNaissance = ?, profession = ?, employeur = ?, typeContratDeTravail = ?, remunerationMensuelleNette = ?, autreRevenus = ?, email = ?, telephone = ?, IdAdresseResidence = ?, adresseNaissance = ? WHERE id = ?";
      db.executeUpdate(
          query,
          Arrays.asList(
              p.getNom(),
