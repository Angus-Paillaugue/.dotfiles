...
public class PersonneDAO implements GenericDAO<Personne> {
  ...
  @Override
  public Personne create(Personne p, int inParent) {
    try {
      ...
      String query =
          "INSERT INTO Personne (nom, prenom, dateNaissance, profession, employeur, typeContratDeTravail, remunerationMensuelleNette, autreRevenus, email, telephone, idLocation, idCoLocation, idAdresseResidence, idAdresseNaissance) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      int idPersonne =
          db.executeUpdate(
              query,
              Arrays.asList(
  ...
  @Override
  public Personne update(Personne p, int inParent) {
    try {
      Connector db = Connector.getInstance();
      AdresseDAO adresseDAO = new AdresseDAO();
      adresseDAO.update(p.getAdresse());
      String query =
          "UPDATE Personne SET nom = ?, prenom = ?, dateNaissance = ?, profession = ?, employeur = ?, typeContratDeTravail = ?, remunerationMensuelleNette = ?, autreRevenus = ?, email = ?, telephone = ?, idLocation = ?, idCoLocation = ?, idAdresseResidence = ?, idAdresseNaissance = ? WHERE id = ?";
      db.executeUpdate(
          query,
          Arrays.asList(
              p.getNom(),
