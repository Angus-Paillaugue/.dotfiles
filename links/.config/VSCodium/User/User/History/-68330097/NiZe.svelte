<script lang="ts">
  ...
  function onPlayerMouseMove(e: MouseEvent) {
    if (!isPlayerMouseDown) return;
    const progressX = e.offsetX - isPlayerMouseDown;
...
{#if isPlayerFullScreen && $currentlyPlayingSong && player?.song}
	...
	<div class="absolute inset-0 z-20 bg-background" transition:fly={{ y: '100%', duration: 300 }} onmousedown={onPlayerMouseDown} onmouseup={onPlayerMouseUp} onmousemove={onPlayerMouseMove} ondrag={onPlayerMouseMove}>
		<div
