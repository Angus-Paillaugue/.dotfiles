...
class JoueurDAO
{
  ...
  public function getStatistics($joueur) {
    $id = $joueur->getId();
    $data = $this->conn->run_query('SELECT poste, COUNT(poste) as count FROM feuille_match WHERE id_joueur = ? GROUP BY poste ORDER BY count DESC LIMIT 1;', $id);
    $poste = $data[0]['poste'];

    $data = $this->conn->run_query('SELECT COUNT(id) as titularisations FROM feuille_match WHERE id_joueur = ?;', $id);
    return [
      'poste' => $poste,
    ];
  }
