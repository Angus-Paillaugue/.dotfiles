...
class JoueurDAO {
  ...
  public function getAll() {
    ...
    $joueursArray = array();
    foreach ($rows as $row) {
      $joueur = new Joueur($row['nom'], $row['prenom'], $row['numero_licence'], $row['date_naissance'], $row['taille'], $row['poids'], $row['statut']);
      $joueur->setId($row['id']);
      $commentaires = (new CommentaireDAO())->getAllForJoueur($joueur);
      $joueur->setCommentaires($commentaires);
  ...
  public function search($nom) {
    $rows =  $this->conn->run_query('SELECT * FROM joueur WHERE nom LIKE ?;', '%' . $nom . '%');
    $joueursArray = array();
    foreach ($rows as $row) {
      $joueur = new Joueur($row['nom'], $row['prenom'], $row['numero_licence'], $row['date_naissance'], $row['taille'], $row['poids'], $row['statut']);
      $joueur->setId($row['id']);
