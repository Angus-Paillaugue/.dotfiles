<script>
	...
	onMount(() => {
		...
		document.querySelector('main').addEventListener('scroll', (e) => {
			...
			if(scroll > 200) {
				occupation.classList.remove('translate-y-full');
			} else {
				occupation.classList.add('translate-y-full');
			}
		})
	})
	...
	async function onMountHandle() {
			...
			(entries) => {
				entries.forEach((entry) => {
					...
					sidebarIndicator.set({
						...
						width: activeSectionLink.clientWidth + 4
					});
				});
			},
			{ threshold: 0.5 }
		...
		if ($sections.length === 0) {
			...
			sidebarIndicator.set({
				y: goBackButton.offsetTop + 2,
				width: goBackButton.getBoundingClientRect().width + 2
			});
		}
	}
</script>
...
<div class="w-screen h-screen bg-primary">
	<div class="max-w-[2000px] h-full w-full mx-auto flex flex-row p-2 md:p-6 lg:p-8 gap-8">
		...
		>
			<a href="/" aria-label="Go home" id='logo'>
				<h2 class="font-base text-lg transition-transform translate-y-full ease-back-out duration-300">{$t('sidebar.occupation')}</h2>
				<h1 class="font-semibold text-xl bg-primary -translate-y-full">Angus Paillaugue</h1>
			</a>
