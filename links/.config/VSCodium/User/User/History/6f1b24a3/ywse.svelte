<script>
	...
	onMount(() => {
		...
		document.querySelector('main').addEventListener('scroll', (e) => {
			...
			if(scroll > 200) {
				occupation.classList.remove('lg:translate-y-full');
			} else {
				occupation.classList.add('lg:translate-y-full');
			}
		})
	})
	...
	<div class="max-w-[2000px] h-full w-full mx-auto flex flex-row p-2 md:p-6 lg:p-8 gap-8">
		...
		>
			<a href="/" aria-label="Go home" id='logo'>
				<h2 class="font-base text-lg transition-transform lg:translate-y-full ease-back-out duration-300">{$t('sidebar.occupation')}</h2>
