...
<div class={cn("w-full overflow-hidden relative pb-8", className)}>
  ...
  <div class="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-2 flex-row">
    {#each new Array(numberOfPages) as _, pageNumber}
      <button class={cn("size-3 rounded-full transition-colors", pageNumber === currentPage ? 'bg-foreground' : 'bg-secondary')} aria-label="Go to page {pageNumber}" disabled={pageNumber === currentPage} onclick={() => scrollToPage(pageNumber)}> </button>
    {/each}
  </div>
</div>
