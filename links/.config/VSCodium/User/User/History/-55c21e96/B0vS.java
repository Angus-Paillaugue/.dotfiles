...
public class ContractDeLocation extends JFrame {
  ...
  public ContractDeLocation(Location location, Personne personne) {
    if (location.isLocation() && !location.isColocation()) {
      // personne est nulle, on gère une location
      editContratDeLocation(location);
    } else if (!location.isLocation() && location.isColocation() && personne != null) {
      // personne est non nulle, on gère une colocation et un contrat unique pour une
      // personne
      editContratDeColocation(location, personne);
    }
  }

  public void editContratDeLocation(Location location) {
    ...
    for (Personne p : location.getLocation().getLocataires()) {
      infosContrat.append(p.getNom() + " " + p.getPrenom());
      infosContrat.append("\n");
      infosContrat.append(
          "Adresse email (facultatif) : "
              + location.getLocation().getLocataires().get(0).getEmail()
              + "\n");
      infosContrat.append(
          "Téléphone (facultatif) : "
              + location.getLocation().getLocataires().get(0).getTelephone()
              + "\n");
    }
    infosContrat.append("\n");
    infosContrat.append("Localisation du logement : " + location.getAdresse() + "\n");
    if(location.getLoyer() != null) infosContrat.append("Montant du loyer : " + location.getLoyer().getMontantHorsCharge() + "€\n");
    if (location.getCharges() != null) infosContrat.append(
        "Provision sur charges : " + location.getCharges().getProvisionSurCharge() + "\n");
