...
<svelte:head>
	...
	<meta
		...
		content={m.app_learning_paths_description()}
	/>
		...
		{#if path.solved_exercises > 0}
			<div class="flex flex-col gap-1">
				...
				<div class="flex flex-row items-center justify-between">
					<span class="text-sm capitalize text-neutral-400">{m.app_learning_paths_card_total_progress()}</span>
					<div class="block">
...
<div class="mx-auto mt-10 flex w-full max-w-screen-lg flex-col gap-2">
	...
	{#if notStartedPaths.length > 0}
		<h1 class="mt-10 text-2xl font-bold capitalize">{m.app_learning_paths_sections_not_started_paths()}</h1>
