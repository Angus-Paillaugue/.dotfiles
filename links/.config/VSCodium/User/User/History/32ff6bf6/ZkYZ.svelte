<script>
  ...
  onMount(() => {
    ...
    // Calculate rectangle sizes based on text
    const calculateWidth = d => Math.max(150, 20 + d3.max(d.columns.map(col => col.name.length + col.type.length)) * 7);
    const calculateHeight = d => 30 + d.columns.length * 20;
    ...
    const drawRelationships = () => {
      svg.selectAll('line.relationship').remove();
      ...
      relationships.forEach(rel => {
        ...
        const fromGroup = svg.selectAll('g.table').filter((_, i) => i === fromTablePos);
        const toGroup = svg.selectAll('g.table').filter((_, i) => i === toTablePos);

        const fromRect = fromGroup.select('rect');
        const toRect = toGroup.select('rect');

        const x1 = +fromGroup.attr('transform').split(/[(),]/)[1] + +fromRect.attr('width');
        const y1 = +fromGroup.attr('transform').split(/[(),]/)[2] + 40 + tables[fromTablePos].columns.findIndex(c => c.name === fromColumn) * 20;

        const x2 = +toGroup.attr('transform').split(/[(),]/)[1];
        const y2 = +toGroup.attr('transform').split(/[(),]/)[2] + 40 + tables[toTablePos].columns.findIndex(c => c.name === toColumn) * 20;
