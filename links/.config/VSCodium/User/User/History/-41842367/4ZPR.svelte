...
<svelte:window onkeydown={(e) => terminal.handleInput(e)} onclick={() => {input.focus();}} />
...
<div class="w-full h-full grow p-4">
  <div class="text-neutral-100 ligatures-normal font-semibold tracking-normal border border-neutral-100 rounded-lg p-4 w-full h-full">
    ...
    <!-- Prompt/Input -->
    <div class="relative">
      <input bind:this={input} class="whitespace-pre text-wrap font-mono text-base text-neutral-100 flex flex-row">{@html terminal.parseTerminalLine(terminal.prompt)} {terminal.input}</input>
      <!-- Cursor -->
