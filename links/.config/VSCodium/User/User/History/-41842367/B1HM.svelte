...
<div class="w-full h-full grow p-4">
  <div class="text-neutral-100 ligatures-normal overflow-y-auto font-semibold tracking-normal border border-neutral-100 rounded-lg p-4 w-full h-full">
    <div class="w-full text-base" id="terminalOutput">
      {#each terminal.terminalValue as line}
        {#if terminal.isCommand(line)}
          {@html terminal.parseTerminalLine(line.command + line.value)}
        {:else if line.reveal}
          {@const value = terminal.parseTerminalLine(line.value)}
          {@const reveal = new TextReveal({ text: value })}
          {@html reveal.value}
        {:else}
          {@html terminal.parseTerminalLine(line.value)}
        {/if}
      {/each}
    </div>
