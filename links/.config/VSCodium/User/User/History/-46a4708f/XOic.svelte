<script>
	...
	let activeSelectItem = $state(categories[0]);
	...
	async function fetchData() {
		...
		try {
			const response = await fetch(
				`${PROXY_URL}/get?url=${encodeURIComponent(categories[activeSelectItem])}`
			);
...
<InterestPicker bind:visible={interestPickerVisible} bind:categories={categories} />
...
<div class="h-full grow flex flex-col pb-28 overflow-hidden">
	<div class="shrink-0 p-4 max-w-md mx-auto w-full">
		<div class="flex flex-row justify-between items-center">
			<h2>News</h2>
			<button class="rounded-full border border-neutral-500/50 p-1" onclick={() => {interestPickerVisible = true}}>
