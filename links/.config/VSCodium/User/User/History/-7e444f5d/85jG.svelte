...
<div class="w-screen h-screen bg-blue-600">
	<div class="max-w-[2000px] h-full w-full mx-auto flex flex-row p-4 md:p-6 lg:p-8 gap-8">
		...
		<button
			...
			class={cn(
				'fixed bottom-8 left-1/2 -translate-x-1/2 z-40 text-white bg-blue-600 rounded-2xl p-2 lg:hidden transition-color duration-500 flex flex-row gap-4 items-center',
				isSidebarShown && 'bg-white text-black'
			)}
		>
			<div class="relative size-4">
				<div class="absolute w-full h-1 bg-white rounded-full top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
				<div class="absolute w-full h-1 bg-white rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
				<div class="absolute w-full h-1 bg-white rounded-full top-full left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
			</div>
			Menu
		</button>
