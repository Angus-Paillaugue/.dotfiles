<script lang="ts">
  ...
  function inview(node: HTMLElement, options) {
    let observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          node.dispatchEvent(new CustomEvent("inview_change", { detail: { inView: true } }));
        } else {
          node.dispatchEvent(new CustomEvent("inview_change", { detail: { inView: false } }));
        }
      });
    }, options);
