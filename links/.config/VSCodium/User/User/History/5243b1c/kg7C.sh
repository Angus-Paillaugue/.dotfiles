...
function compress() {
  ...
  local fileName="$(basename -- "$file")"
  local fileNameWithoutExtension="${fileName%.*}"
  local fileCreationTime=$(stat -c %Y "$file")
  ...
  # Create the output file with the same metadata as the input file
  # touch -r "$file" "$outputName" # Does not change the creation time
  touch -a -m -d @$fileCreationTime "$outputName" # Change the creation time to the original file's creation time
