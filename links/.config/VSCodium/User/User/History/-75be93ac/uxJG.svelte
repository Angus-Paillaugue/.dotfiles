...
<div class="flex h-screen w-full flex-row gap-2 bg-body lg:p-2">
	<nav
		class={cn(
			'shrink-0 flex-col rounded-xl border border-border bg-card p-2 transition-transform duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 pt-3 max-lg:min-w-[300px] lg:w-[16rem]',
	...
	>
		{#if path.split('/').length > 1}
			<div
				...
				class={cn(
					'flex-row flex items-center mb-6 gap-2 relative'
				)}
			>
