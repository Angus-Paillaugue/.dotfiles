...
<div class="flex h-screen w-full flex-row gap-2 bg-body lg:p-2">
	<nav
		class={cn(
			'shrink-0 flex-col rounded-xl border border-border bg-card p-2 transition-transform duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 max-lg:min-w-[300px] lg:w-[16rem]',
			mobileMenuOpened ? ' max-lg:translate-x-0 max-lg:ml-2' : 'max-lg:-ml-2 hidden max-lg:-translate-x-full lg:flex'
		)}
	>
		{#if path.split('/').length > 1}
			<div
				...
				class={cn(
					'flex-row flex items-center mt-2 mb-6 gap-2 relative'
				)}
			>
				<Button href={'/app/'+path.split('/').slice(0, -1).join('/')} variant="backButton" arial-label={m.app_home_layout_nav_go_back()}
				><Undo2 class="size-5" /></Button
				>
				<!-- {#if !isExercisePage} -->
					<span class={cn("text-lg font-medium absolute top-1/2 -translate-y-1/2 left-2 transition-all duration-300 linear", path.split('/').length > 1 && 'ml-10')}>{isExercisePage ? "Exercise" : pageName}</span>
				<!-- {/if} -->
			</div>
		{/if}

		<!-- Navigation -->
		<div class="flex grow mt-6 flex-col">
			<!-- Exercises -->
	...
	<div class="no-scrollbar flex h-full grow flex-col overflow-auto">
		...
		>
			...
			{/if}
			{#if !isExercisePage}
				<span class={cn("text-lg font-medium absolute top-1/2 -translate-y-1/2 left-2 transition-all duration-500 ease-in-out", path.split('/').length > 1 && 'ml-16')}>{pageName}</span>
			{/if}
			<div class="h-8"></div>
		</div>
		<!-- Open mobile nav button -->
		<Button
			class={cn("ml-auto max-lg:z-50 transition-transform fixed lg:hidden top-4 right-2", mobileMenuOpened ? 'scale-125' : '')}
			variant="backButton"
