...
<div class="flex h-screen w-full flex-row gap-2 bg-body lg:p-2">
	<nav
		class={cn(
			'shrink-0 flex-col gap-6 rounded-xl border border-border bg-card p-2 transition-transform duration-300 max-lg:fixed max-lg:bottom-2 max-lg:left-0 max-lg:top-2 max-lg:z-50 max-lg:flex max-lg:w-1/2 max-lg:min-w-[300px] lg:w-[16rem]',
			mobileMenuOpened ? ' max-lg:translate-x-0 max-lg:ml-2' : 'max-lg:-ml-2 hidden max-lg:-translate-x-full lg:flex'
		)}
	>
		<div
			class={cn(
				'flex-row flex mt-2 items-center gap-2 relative'
			)}
		>
			...
			{/if}
			{#if !isExercisePage}
				<span class={cn("text-lg font-medium absolute top-1/2 -translate-y-1/2 left-2 transition-all duration-500 ease-in-out", path.split('/').length > 1 && 'ml-10')}>{pageName}</span>
			{/if}
			<div class="h-8"></div>
		</div>
		<!-- Profile -->
