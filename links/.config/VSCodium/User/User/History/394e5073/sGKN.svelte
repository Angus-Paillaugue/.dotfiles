...
<div class="pt-10 flex flex-col items-center justify-center p-2">
	<form
		class="max-w-md w-full rounded-3xl p-6 flex flex-col gap-6"
		style="background-color: #{CARDS_COLORS[Math.floor]};"
		method="POST"
		use:enhance={() => {
			isSendingForm = true;
			return async ({ update }) => {
	...
	>
		...
		<button
			...
			type="submit"
			class="flex flex-row items-center justify-center gap-2 w-full px-4 py-4 bg-neutral-400 rounded-full text-text-heading-dark"
		>
			{#if isSendingForm}
				<Spinner />
			{/if}
