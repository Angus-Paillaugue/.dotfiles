export class Terminal {
	 ...
	 *
	 * @constructor
	 * @param {Object} commands - An object containing additional commands to be added to the terminal.
	 * @param {string} [prompt=this.#PROMPT] - The prompt string to be used in the terminal.
	 *
	 * @property {Object} COMMANDS - An object containing the default and additional commands for the terminal.
	 * @property {Object} COMMANDS.help - Command to display a list of available commands.
	 * @property {string} COMMANDS.help.description - Description of the help command.
	 * @property {Function} COMMANDS.help.callback - Callback function to execute the help command.
	 * @property {Object} COMMANDS.clear - Command to clear the terminal screen.
	 * @property {string} COMMANDS.clear.description - Description of the clear command.
	 * @property {Function} COMMANDS.clear.callback - Callback function to execute the clear command.
	 * @property {Object} COMMANDS.echo - Command to print a message to the terminal.
	 * @property {string} COMMANDS.echo.description - Description of the echo command.
	 * @property {Function} COMMANDS.echo.callback - Callback function to execute the echo command.
	 *
	 * @method addWelcomeMessage - Adds a welcome message to the terminal.
	 */
	...
	handleInput(e) {
		...
		if (key === 'Enter') {
			...
			if (e.ctrlKey || e.metaKey) {
				...
				this.#input =
					this.#input.slice(0, this.#caretPosition - 1) + this.#input.slice(this.#caretPosition);
			...
			if (e.ctrlKey || e.metaKey) {
				...
				this.#input =
					this.#input.slice(0, this.#caretPosition) + this.#input.slice(this.#caretPosition + 1);
			}
		} else if (!e.altKey && !e.ctrlKey && !e.metaKey && key.length === 1) {
			this.#input =
				this.#input.slice(0, this.#caretPosition) + key + this.#input.slice(this.#caretPosition);
			this.#caretPosition++;
		} else if (key === 'Tab') {
			e.preventDefault();
		} else if (key === 'ArrowUp') {
