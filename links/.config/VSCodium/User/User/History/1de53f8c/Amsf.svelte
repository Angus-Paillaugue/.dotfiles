...
>
  ...
  >
    ...
    <input
      type="text"
      class="w-full rounded-full bg-neutral-800 px-6 py-3 text-base font-medium text-text-heading-dark h-10 outline-0 placeholder:text-text-body focus:outline-0"
      name="username"
    ...
    <div class="flex flex-row relative">
      <input
        ...
        class="w-full rounded-l-full bg-neutral-800 px-6 py-3 text-base font-medium h-10 text-text-heading-dark outline-0 placeholder:text-text-body focus:outline-0"
        name="password"
        placeholder="Password"
      />
      <img src="/password-input-end.svg" alt="" class="h-10 -ml-px">
      <button class="absolute right-1 top-1 rounded-full bg-white w-8 h-8 text-neutral-800 p-1" type="button" onclick={() => (passwordVisible = !passwordVisible)}>
        {#if passwordVisible}
          <span in:scale>
