...
<div class="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
  <div class="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
    ...
    <Card.Root>
      <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
        <Card.Title class="text-sm font-medium">Errors</Card.Title>{(statistics?.warn ?? 0)}
        <OctagonAlert
          class={cn('text-muted-foreground size-4', (statistics?.warn ?? 0) > 0 && 'text-red-600')}
        />
      </Card.Header>
      <Card.Content>
    ...
    <Card.Root>
      <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Skull
          class={cn('text-muted-foreground size-4', (statistics?.warn ?? 0) > 0 && 'text-rose-600')}
        />
      </Card.Header>
      <Card.Content>
