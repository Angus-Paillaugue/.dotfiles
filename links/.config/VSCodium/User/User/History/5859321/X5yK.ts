...
export class Player {
	...
	changeSong(song: Song) {
		...
		this._playerElement.addEventListener('canplay', () => {
			this.play();
		}, { once: true });
		this._songIndex = $songs.map((s) => s.path).indexOf(song.path);
	}
	...
	next() {
		if (!this._song) return;
		const nextIndex = (this._songIndex + 1) % $songs.length;
	...
	previous() {
		if (!this._song) return;
		const previousIndex = (this._songIndex - 1 + $songs.length) % $songs.length;
		this.changeSong($songs[previousIndex]);
		this._playerElement.dispatchEvent(new Event('songChange'));
	}
