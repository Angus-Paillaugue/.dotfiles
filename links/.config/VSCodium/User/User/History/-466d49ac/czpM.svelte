<script lang="ts">
  ...
  async function search({ pageNumber, pageSize }: { pageNumber: number; pageSize: number }) {
    isSearching = true;
...
<div class="flex w-full flex-col">
  <div class="gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
    <Card.Root>
      ...
      <Card.Content>
        <form onsubmitcapture={() => {search(null)}}>
    ...
    <LogsTable {user} class="mt-6" getResults={search} bind:rows={results} bind:isLoading={isSearching} />
  </div>
</div>
