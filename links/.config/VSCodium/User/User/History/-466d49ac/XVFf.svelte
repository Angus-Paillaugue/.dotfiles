<script lang="ts">
  ...
  async function search({ pageNumber: number, pageSize, pageChange = false }) {
    if(!pageChange) {
      currentPage = 0;
    }
    isSearching = true;
    const res = await fetch(`/api/logs/search`, {
      ...
      body: JSON.stringify({ query: searchValue, page: currentPage, resultsPerPage: pageSize }),
    });
    if(!res.ok) {
...
<div class="flex w-full flex-col">
  <div class="gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
    ...
    <LogsTable {user} getResults={search} bind:rows={results} bind:isLoading={isSearching} bind:pageSize={resultsPerPage} bind:numberOfPages />
  </div>
</div>
