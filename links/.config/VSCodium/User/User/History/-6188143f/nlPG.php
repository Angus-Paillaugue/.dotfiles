...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($username) || empty($password)) {
    Error::setError('Veuillez remplir tous les champs');
  } else {
    $username = htmlspecialchars($username);
    $password = htmlspecialchars($password);
    $user = $conn->run_query('SELECT * FROM users WHERE username=?;', $username);
    if (count($user) == 0) {
      Error::setError("Cet utilisateur n'existe pas");
    } else {
      $user = $user[0];
      if (password_verify($password, $user['password_hash'])) {
        ...
        Error::setError('Mot de passe incorrect');
      }
    }
  }
...
<div class="max-w-xl mx-auto w-full p-4">
   ...
   $_SERVER['PHP_SELF']
    ...
    Components::Input([
      ...
      // if (Error::getError()) {
      //   Components::Alert(['text' => Error::getError(), 'variant' => 'danger']);
      // }
