<script lang="ts">
  ...
  let { cards = $bindable([]), onSwap, deleteCard }: { cards: Card[]; onSwap?: (cards: Card[]) => void; deleteCard: (card: Card) => void } =
    $props();
...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
  {#each cards as card, i (card.id)}
    ...
    <div animate:flip={{ duration: dragDuration }} ondragenter={() => swapWith(card)} ondragover={(e) => e.preventDefault()} style="grid-column: span {card.colSpan ?? 1} / span {card.colSpan ?? 1};">
      <CardComponent {onDragStart} {onDragEnd} bind:card={cards[i]} {deleteCard} />
    </div>
  {/each}
</div>
