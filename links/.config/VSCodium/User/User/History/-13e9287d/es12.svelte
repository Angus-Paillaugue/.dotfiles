<script lang="ts">
  ...
  let { cards = $bindable([]), onSwap, deleteCard, onCardsEdit }: { cards: Card[]; onSwap?: (cards: Card[]) => void; deleteCard: (card: Card) => void; onCardsEdit: () => void } =
    $props();
...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
  {#each cards as card, i (card.id)}
    ...
    <div animate:flip={{ duration: dragDuration }} ondragenter={() => swapWith(card)} ondragover={(e) => e.preventDefault()} class={cn(colSpans[card.colSpan ?? 1])}>
