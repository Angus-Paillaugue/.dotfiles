<script lang="ts">
  ...
  const skeletonBarHeight = new Array(5).fill(0).map(() => Math.random() * 100);
</script>

{#if !canShow}
  <!-- Skeleton loader -->
  <div class="w-fulll flex flex-roe items-end justify-between px-4 gap-8" style="height: {options.chart.height}px;">
    {#each Array(5) as _, i}
      <Skeleton class="grow" style="height: {skeletonBarHeight[i]}%;" />
    {/each}
  </div>
{/if}
