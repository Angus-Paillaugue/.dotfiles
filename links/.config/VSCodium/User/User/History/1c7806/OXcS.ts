...
export async function deleteLog(req: Request, res: Response) {
  ...
  if (!id && !ids) {
    res.status(200).send({ action: 'deleteLog', success: false, message: 'id or ids are required' });
    return;
  }
  ...
  try {
    if(ids) {
      ...
      if (!deletedLogs) {
        ...
        res.status(200).send({ action: 'deleteLogs', success: false, error: 'Logs not found' });
      }
      logEventEmitter.emit('logsDeleted', ids);
      res.status(200).send({ action: 'deleteLogs', success: true, ids });
    }else {
