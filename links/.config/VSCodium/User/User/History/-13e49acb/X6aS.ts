...
export async function ask(messages: BuildingMessage[]) {
  ...
  const messagesForModel: MessageForModel[] = [contextMessage, ...messages].slice(0, -1).map((m) => ({ // Adds the system prompt with project context. The slice is here because on the page, we create an empty message 
    role: m.role,
    content: m.content
  }));
