...
export async function ask(messages: BuildingMessage[]) {
  ...
  let messagesForModel: MessageForModel[] = [];
  ...
  if (messages.length <= 2) {
    ...
    messagesForModel = [contextMessage, ...messages.slice(0, -1)];
  } else {
    messagesForModel = messages.slice(0, -1);
  }

  messagesForModel = messagesForModel.map((m) => ({
    role: m.role,
    content: m.content
  }));
