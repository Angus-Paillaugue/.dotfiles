...
export class Terminal {
	...
	private _terminalValue = $state<Line[]>([]);
	...
	getCommand(): string {
		...
		return this.stripLine(commands[this._lookingForCommandIndex] || commands.at(-1) || this._input);
	}
	...
	terminalPush(value: string = ' ', reveal: boolean = true): void {
		const line: Line = {
			value: this.parseTerminalLine(value),
			reveal,
			command: this.isCommand({ value, reveal: false, command: null }) ? value : null
		}
		this.terminalValue.push(line);
	}
