...
export class Terminal {
	...
	constructor(commands: Command[], prompt: string = this._PROMPT) {
		this._PROMPT = prompt;
		this.COMMANDS = {
			help: {
	...
	handleInput(e) {
		...
		if (key === 'Enter') {
			...
			if (e.ctrlKey || e.metaKey) {
				const index = this._input.lastIndexOf(' ', this._caretPosition - 1);
				if (index === -1) {
					this._input = this._input.slice(this._caretPosition);
					this._caretPosition = 0;
				} else {
					this._input = this._input.slice(0, index) + this._input.slice(this._caretPosition);
					this._caretPosition = index;
				}
			} else {
				this._input =
					this._input.slice(0, this._caretPosition - 1) + this._input.slice(this._caretPosition);
				this._caretPosition = Math.max(0, this._caretPosition - 1);
			}
		} else if (key === 'Delete') {
			if (this._input.length === 0) return;
			if (e.ctrlKey || e.metaKey) {
				const index = this._input.indexOf(' ', this._caretPosition);
				if (index === -1) {
					this._input = this._input.slice(0, this._caretPosition);
				} else {
					this._input = this._input.slice(0, this._caretPosition) + this._input.slice(index + 1);
				}
			} else {
				this._input =
					this._input.slice(0, this._caretPosition) + this._input.slice(this._caretPosition + 1);
			}
		} else if (!e.altKey && !e.ctrlKey && !e.metaKey && key.length === 1) {
			this._input =
				this._input.slice(0, this._caretPosition) + key + this._input.slice(this._caretPosition);
			this._caretPosition++;
		} else if (key === 'Tab') {
			e.preventDefault();
		} else if (key === 'ArrowUp') {
