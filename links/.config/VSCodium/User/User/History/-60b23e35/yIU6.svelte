<script>
  ...
  import { scrollPercentage } from '$lib/stores';
  ...
  onMount(() => {
    ...
    lenis.on('scroll', (event) => {
      ...
      const scrollValue = event.animatedScroll;
      const maxScrollValue = event.dimensions.scrollHeight - event.dimensions.height;
      const percentage = (scrollValue - minScrollValue) / (maxScrollValue - minScrollValue) * 100;
