...
<aside
  class="h-screen p-2 max-lg:pt-0 flex flex-col gap-0 lg:w-[300px] max-lg:top-[3.7rem] pb-[58px] overflow-y-auto pl-0 z-40 fixed lg:sticky max-lg:inset-0 bg-white dark:bg-neutral-900 transition-transform top-0 border-r border-neutral-300/50 dark:border-neutral-800 shrink-0 {open
...
>
  ...
  <div class="flex flex-row gap-2 items-center justify-end lg:hidden sticky bottom-0 left-0 right-0 bg-inherit p-2">
...
<button
  class="h-12 aspect-square flex flex-col items-center justify-center lg:hidden fixed z-50 top-[0.5rem] left-[0.5rem]"
  onclick={() => (open = !open)}
>
