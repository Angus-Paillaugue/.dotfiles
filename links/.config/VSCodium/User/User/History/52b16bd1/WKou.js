export default function imageZoomPlugin() {
  ...
  allImages.forEach((image) => {
    image.addEventListener('click', () => {
      ...
      imageZoom.classList.add(...'inset-0 fixed z-50 bg-neutral-300/50 dark:bg-neutral-800/50 bg-center bg-no-repeat bg-contain transition-all scale-0'.split(" "));
      imageZoom.style.backgroundImage = `url(${image.src})`;
      document.body.appendChild(imageZoom);

      setTimeout(() => {
        imageZoom.classList.remove('scale-0');
        imageZoom.classList.add('scale-100');
      }, 0);
