    ...
    <div class={cn("demoContainer rounded-xl border border-main dark:border-main-dark h-[20rem] overflow-y-auto lg:h-[28rem] lenis-prevent", code && 'rounded-tl-none')}>
        ...
        >
          {#await component?.component()}
            <div class="flex p-4 rounded flex-row items-center justify-center gap-4">
      ...
      {#if code}
        <Pre
          class={cn(selectedIndex === 1 ? 'block h-full rounded-tl-none min-h-full h-max' : 'hidden')}
        >
        <div class="flex w-full min-h-full h-max flex-row items-center justify-center">
