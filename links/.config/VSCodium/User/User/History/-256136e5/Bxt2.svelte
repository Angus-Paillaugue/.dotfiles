<script>
  ...
  const { name, lang = 'svelte', code = false } = $props();
  ...
  $effect(() => {
    ...
    const left = selectedItem.offsetLeft;true
  ...
  <div class="demo" bind:this={commandsContainer}>
    ...
    {#if code}
      <div class="relative w-fit">
        ...
        <div class="absolute left-0 right-0 bg-body -bottom-px h-px"></div>
      </div>
    {/if}

    <!-- Preview/Code -->
    <div class="demoContainer rounded-xl rounded-tl-none border border-main dark:border-main-dark bg-neutral-50 dark:bg-neutral-950 h-[20rem] overflow-y-auto lg:h-[28rem] lenis-prevent">
        <!-- Preview -->
        <div
