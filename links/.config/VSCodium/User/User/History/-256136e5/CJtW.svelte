  ...
  <div class="demo flex flex-col gap-4" bind:this={commandsContainer}>
    ...
    {#if code}
      <div class="relative">
        ...
        ></span>
        <ul
          class="flex flex-row border-b border-main py-2 dark:border-main-dark"
        >
    ...
    <!-- Preview/Code -->
    <div class="demoContainer rounded-xl border border-main dark:border-main-dark bg-neutral-50 dark:bg-neutral-950 h-[20rem] overflow-y-auto lg:h-[28rem] lenis-prevent">
      <!-- Preview -->
      <div
        class={cn("component-preview flex flex-col items-center justify-center h-full p-4", selectedIndex === 0 ? 'flex' : 'hidden')}
      >
      ...
      {#if code}
        <Pre
          class={cn(selectedIndex === 1 ? 'block h-full' : 'hidden')}
          copyCode='false'
        >
          {#await component?.raw()}
