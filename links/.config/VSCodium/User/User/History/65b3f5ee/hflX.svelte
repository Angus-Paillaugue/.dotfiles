<script lang="ts">
  ...
  import { Brain, Send , Settings} from 'lucide-svelte';
  ...
  async function askQuestion(e: SubmitEvent) {
    ...
    isAIWriting = false;
    saveLatestMessage();
    generateTitle(); // Called every time but returns if it's not the first message
  }
...
<div class="grow flex flex-col h-full w-full">
  <div class="flex flex-row items-center justify-between border-y border-border shrink-0 p-2 gap-4">
    <Title {conversation} />
    <Button href="/ai" variant="outline" class="size-10 p-0">
