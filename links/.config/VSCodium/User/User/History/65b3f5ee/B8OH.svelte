  ...
  <Dialog.Content>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-row gap-2 items-center justify-end">
        <Button variant="outline" onclick={() => {settingsModalVisible = false; editedConversation = conversation}}>Cancel</Button>
        <Button loading={isSavingConversation} disabled={isSavingConversation} onclick={saveConversation}>Save</Button>
      </div>
    </div>
  </Dialog.Content>
</Dialog.Root>
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        This action cannot be undone. This will permanently delete this conversation and all of it's messages.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
      <AlertDialog.Cancel>Cancel</AlertDialog.Cancel>
      <Button onclick={deleteConversation} loading={isDeletingConversation} disabled={isDeletingConversation}>Continue</Button>
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
...
<div class="grow flex flex-col h-full w-full">
  <div class="flex flex-row items-center justify-between border-y border-border shrink-0 p-2 gap-4">
    ...
    <Title bind:conversation {saveConversation} canEdit={hasPermission(user?.role, PERMISSIONS.UPDATE_AI_CONVERSATION)} />
    {#if hasAtLeastOnePermission(user?.role, PERMISSIONS.UPDATE_AI_CONVERSATION, PERMISSIONS.DELETE_AI_CONVERSATION)}
      <DropdownMenu.Root>
        <DropdownMenu.Trigger class={buttonVariants({ variant: "outline", class: 'size-10 p-0 ml-auto' })}>
        ...
        <DropdownMenu.Content>
          <DropdownMenu.Group>
            ...
            {#if hasPermission(user?.role, PERMISSIONS.DELETE_AI_CONVERSATION)}
              <DropdownMenu.Item variant="destructive" onclick={() => {deleteConversationConfirmVisible = true}}>
                <Trash2 class="size-4"/>
                Delete
              </DropdownMenu.Item>
            {/if}
