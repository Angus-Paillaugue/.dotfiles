<script lang="ts">
  ...
  import { Brain, Send , ChevronLeft, Settings } from 'lucide-svelte';
  import MessageComponent from './message.svelte';
  import { fade } from "svelte/transition";
  import TitleInput from "./input.svelte";
...
<div class="grow flex flex-col h-full w-full">
  <div class="flex flex-row items-center justify-between border-y border-border shrink-0 p-2 gap-4">
    ...
    <Button onclick={() => (settingsModalVisible = true)} variant="outline" class="size-10 p-0 ml-auto">
  ...
  <form onsubmit={askQuestion} class="flex flex-row gap-2 p-2 bg-sidebar shrink-0">
    <TitleInput bind:value={message} onsubmit={askQuestion} placeholder="Message OwnLogs's AI" />
