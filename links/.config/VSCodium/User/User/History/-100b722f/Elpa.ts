...
function transformData(data: { date: Date; name: LogLevel; count: number }[]): {
  dates: Date[];
  series: { date: Date; data: { name: LogLevel; count: number }[] }[];
} {
  const dates: Date[] = [];
  const series: { date: Date[]; data: [{ name: string; count: number }] }[] = [];
  ...
  data.forEach((entry) => {
    ...
    const existing = series.find((item) => item.date === entry.date);
    if (existing) {
