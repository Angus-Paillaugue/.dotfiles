...
class LogDAO {
  ...
  async getLogStatisticsByDay(): Promise<{ logDate: Date; level: LogLevel; logCount: number }[]> {
    const sql = `
    SELECT DATE(timestamp) AS logDate, level, COUNT(*) AS logCount
    FROM logs
    GROUP BY logDate, level;
    `;
    const rows = await DB.query<any[]>(sql);
    return rows as { logDate: Date; level: LogLevel; logCount: number }[];
  }
}
