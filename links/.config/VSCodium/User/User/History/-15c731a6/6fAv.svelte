...
<div class="mt-auto grid grid-cols-2 items-end justify-between gap-4 py-4">
  {#if previousPage}
    <a
      ...
      class="flex h-full flex-row items-center justify-start gap-2 rounded-md border border-main p-2 transition-all hover:gap-4 md:p-4"
    >
      <Icon
  ...
  {#if nextPage}
    <a
      href={nextPage.url}
      class="col-start-2 flex h-full flex-row items-center justify-end gap-2 rounded-md border border-main p-2 transition-all hover:gap-4 md:p-4"
    >
