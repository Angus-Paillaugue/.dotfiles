...
public class BatimentDAO implements BatimentDAODef {
  ...
  public Batiment create(Batiment batiment) {
    try {
      ...
      String query = "INSERT INTO Batiment (typeBatiment, numeroContrat, idAdresse, numeroContrat) VALUES (?, ?, ?, ?)";
      int insertedRowId =
          db.executeUpdate(
              query,
              Arrays.asList(batiment.getTypeBatiment(), batiment.getNumeroFiscal(), batiment.getAdresse().getId(), batiment.getNumeroFiscal()));
