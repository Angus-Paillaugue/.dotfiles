<script lang="ts">
  ...
  let highlighter = $state< | null>(null);
...
{:else}
  ...
  >
    ...
    {:else}
      <div
        class={cn(
          'flex flex-col whitespace-pre-wrap rounded-xl border-border bg-secondary p-4 text-base font-normal',
          message.role === 'system' ? 'rounded-tl-none message' : 'rounded-tr-none'
        )}
      >
