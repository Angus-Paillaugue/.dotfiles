...
test('home page has has sign-up and log-in form', async ({ page }) => {
	...
	expect(signUpFormFields[0].getAttribute('type')).toBe('text');
	expect(signUpFormFields[1].getAttribute('type')).toBe('password');
	...
	expect(logInFormFields[0].getAttribute('type')).toBe('text');
	expect(logInFormFields[1].getAttribute('type')).toBe('password');
});
