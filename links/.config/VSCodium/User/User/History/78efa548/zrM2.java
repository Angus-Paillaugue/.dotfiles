...
public class LogementDAO implements LogementDAODef {
  ...
  public Logement create(Logement l, int idBatiment) {
    try {
      System.out.println("From create: "+idBatiment);
  ...
  public Logement find(int id) {
    try {
      Connector db = Connector.getInstance();
      String query =
          "SELECT L.*, BL.idAdresse, BL.refCadastrale, BL.bati, BL.numeroFiscal, BL.taxeFonciere, BL.typeLogement, BL.idBatiment, BL.numeroContrat, BL.surfaceTotale, BL.surfaceHabitable, BL.nbPieces, BL.typeLogement FROM Logement L, Bien_louable BL WHERE BL.id = L.id AND L.id = ? ORDER BY L.id DESC";
      ...
      System.out.println("From find: "+(int) result.get("idBatiment"));
      logement.setNbPieces((int) result.get("nbPieces"));
      String typeLogement = (String) result.get("typeLogement");
      if(typeLogement.equals("Location")) {
        logement.setIsLocation();
        logement.addLocation(findLocationByLogement(id));
      }else if(typeLogement.equals("Colocation")){
        Colocation colocation = findCoLocationByLogement(logement);
        if(colocation != null) {
  ...
  public void update(Logement l) {
    System.out.println("From update: "+l.getIdBatiment());
    try {
      Connector db = Connector.getInstance();
      String query =
  ...
  public Colocation findCoLocationByLogement(Logement logement) {
    return new ColocationDAO().findByLogement(logement);
  }
}
