...
public class LogementDAO implements LogementDAODef {
  ...
  public Logement create(Logement l, int idBatiment) {
    try {
      ...
      new AssuranceDAO().create(l.getAssurance());
      String query =
          "INSERT INTO Bien_louable (idAdresse, refCadastrale, bati, numeroFiscal, taxeFonciere, idBatiment, surfaceTotale, surfaceHabitable, typeLogement, nbPieces, numeroContrat) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      int idBienLouable =
          db.executeUpdate(
              query,
              Arrays.asList(
                  ...
                  l.getNbPieces(),
                  l.getAssurance().getNumeroContrat()));
      // Creating adresse
  ...
  public Logement find(int id) {
    try {
      ...
      Logger.log(result.get("numeroContrat"));
      Assurance assurance = new AssuranceDAO().find((int) result.get("numeroContrat"));
      Logement logement =
          new Logement(
              adresse,
      ...
      logement.setAssurance(assurance);
