...
<div class="rounded-full fixed z-50 pointer-events-none transition-colors ring-black ring-4" style="top: {$extCirclePos.y}px; left:{$extCirclePos.x}px; width: {$extCirclePos.width}px; height: {$extCirclePos.height}px;" bind:this={cursor}></div>
<div class="rounded-full fixed z-50 pointer-events-none transition-colors bg-black size-2" style="top: {$interiorCirclePos.y}px; left:{$interiorCirclePos.x}px;" ></div>
