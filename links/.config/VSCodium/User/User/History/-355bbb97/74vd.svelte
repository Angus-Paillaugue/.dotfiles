<script>
  ...
  function handleMouseMove(e) {
    ...
    if(hoveringActiveElement) {
      cursor.classList.add(...'ring-2 bg-transparent'.split(' '))
      cursor.classList.remove(...'bg-black'.split(' '))
    }else {
      cursor.classList.remove(...'ring-2 bg-transparent'.split(' '))
      cursor.classList.add(...'bg-black'.split(' '))
    }
  }
</script>


<div class="rounded-full fixed z-50 pointer-events-none transition-colors ring-black bg-black" style="top: {$extCirclePos.y}px; left:{$extCirclePos.x}px; width: {$extCirclePos.width}px; height: {$extCirclePos.height}px;" bind:this={cursor}></div>
