<script>
  ...
  function handleMouseMove(e) {
    ...
    const width = hoveringActiveElement ? hoveringActiveElement.clientWidth : initialSize;
    const height = hoveringActiveElement ? hoveringActiveElement.clientHeight : initialSize;
    const rect = hoveringActiveElement?.getBoundingClientRect()
    const x = hoveringActiveElement ? rect.x : mouseX - initialSize / 2;
    const y = hoveringActiveElement ? rect.y : mouseY - initialSize / 2;
