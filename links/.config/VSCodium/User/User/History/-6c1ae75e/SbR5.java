...
public class LoyerDAOTests {
  ...
  public void testUpdate() {
    int idBienLouable = 1;
    int idLoyer = loyerDAO.create(this.testLoyer, idBienLouable);
    Loyer updatedLoyer = new Loyer(
        800.0F, 10, LocalDate.of(2024, 8, 11), LocalDate.of(2025, 9, 11), ModeDePaiment.CB);
    int newIdLoyer = loyerDAO.update(updatedLoyer, idLoyer);
    Loyer loyer = loyerDAO.find(newIdLoyer);
    assertNotNull(loyer);
    Loyer expected = new Loyer(2, 800.0F, 10, LocalDate.of(2024, 8, 11), LocalDate.of(2025, 9, 11),
        ModeDePaiment.CB, idBienLouable);
