...
public class LoyerDAO implements GenericDAO<Loyer> {
  ...
  @Override
  public Loyer create(Loyer l, int inParent) {
    try {
  ...
  @Override
  public Loyer update(Loyer l, int inParent) {
  ...
  public List<Loyer> findAllForLogement(Logement logement) {
    try {
      Connector db = Connector.getInstance();
      String query = "SELECT id FROM Loyer WHERE idBienLouable = ?";
  ...
  public Loyer findLatestForLogement(int id) {
    try {
      Connector db = Connector.getInstance();
      Logger.log("Finding latest loyer for logement with id: " + id);
      String query =
