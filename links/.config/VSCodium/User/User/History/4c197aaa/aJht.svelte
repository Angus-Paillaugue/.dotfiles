<script lang="ts">
  ...
  let checkedListItems = $derived(note.items.filter(item => item.checked).filter((i) => i).sort((a, b) => a.position - b.position));
  let uncheckedListItems = $derived(note.items.filter(item => !item.checked).filter((i) => i).sort((a, b) => a.position - b.position));
