<script lang="ts">
  ...
  let checkedListItems = $derived(note.items.filter(item => item.checked));
  let uncheckedListItems = $derived(note.items.filter(item => !item.checked));
  ...
  function addListItem() {
    note.items.push({
      ...
      item: '',
      checked: false,
      position: uncheckedListItems.length
    });
    save();
  }
