<script lang="ts" generics="TData, TValue">
  ...
  let socket: WebSocket | undefined | 'closed' = $state();
  let table = $state();
  let pageSize = $state(30);
  let numberOfPages = $state(0);
  let toastId: number | string = $state(0);
  ...
  function webSocket() {
    toastId = toast.info('Connecting to the server...');
    ...
    socket.addEventListener('open', () => {
      fetchLogs();
      toast.info('Connected to the server', { id: toastId });
    });
...
<div class="sm:px-6 sm:pb-6">
  ...
  <div class="rounded-md border">
    <LogsTable {user} getResults={fetchLogsPage} bind:this={table} bind:pageSize bind:rows bind:numberOfPages>
      <!-- Real-time checkbox -->
