...
class Components
{
  ...
  public static function Button($params = [])
  {
    ...
    $baseClasses = "cursor-pointer transition-all duration-200 ease-in-out px-4 py-2 font-medium rounded-lg disabled:opacity-50 disabled:cursor-not-allowed items-center justify-center flex flex-row gap-2 text-center";
    if($href) {
      echo "<a href='$href' ".($id && 'id='.$id)." class='$baseClasses $variantClasses $class' $disabled type='$type'>$label</a>";
    }else {
      echo "<button type='submit' ".($id && 'id='.$id)." class='$baseClasses $variantClasses $class' $disabled type='$type'>$label</button>";
    }
  }
  ...
  public static function Link($params = [])
  {
    ...
    $id = Components::merge($params, 'id');
    $disabled = Components::merge($params, 'disabled', '', 'disabled');
    echo "<a href='$href' ".($id && 'id='.$id)." class='cursor-pointer font-medium disabled:opacity-50 disabled:cursor-not-allowed items-center justify-center inline-flex flex-row gap-2 text-neutral-900 hover:text-primary-600 underline transition-colors $class' $disabled>$label</a>";
  }
  ...
  public static function Alert($params = [])
  {
    ...
    echo "<div ".($id && 'id='.$id)." class='rounded-lg px-4 py-2 flex flex-row border gap-4 items-center before:size-10 before:bg-cover before:bg-center before:bg-no-repeat before:block $variantClasses[$variant] $class'><p class='p-0 m-0'>$text</p></div>";
  }
}
