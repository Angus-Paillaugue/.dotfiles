...
public class AddRentalVue extends JFrame {
    ...
    public AddRentalVue() {
        ...
        addSeparator(mainPanel, gbc);

        JPanel formPanel = createFormPanel();
        JPanel rentPanel = createRentPanel();
        JPanel fileInputsPanel = createFileInputsPanel();

        addPanelsToMain(mainPanel, gbc, formPanel, rentPanel, fileInputsPanel);
        addBottomButtons(mainPanel, gbc);
    ...
    private JPanel createFormPanel() {
        ...
        addFormFields(formPanel, formGbc);

        return formPanel;
    }

    private void addFormFields(JPanel panel, GridBagConstraints gbc) {
        addLabelAndTextField(panel, gbc, "Adresse", 0);
        addLabelAndComboBox(panel, gbc, "Type de bâtiment", new String[] { "Appartement", "Maison", "Studio" }, 1);
        addLabelAndTextField(panel, gbc, "Référence cadastrale", 2);
        addLabelAndTextField(panel, gbc, "Référence logement", 3);
        addLabelAndFormattedTextField(panel, gbc, "Numéro fiscal", 4);
        addLabelAndTextFieldWithUnit(panel, gbc, "Superficie", "m²", 5);
        addLabelAndComboBox(panel, gbc, "Note énergétique", noteEnergetique.getNames(), 6);
        addLabelAndComboBox(panel, gbc, "Emissions gaz à effet de serre", noteEnergetique.getNames(), 7);
        addLabelAndRadioButtons(panel, gbc, "Bâti", new String[] { "Oui", "Non" }, 8);
        addLabelAndRadioButtons(panel, gbc, "Meublé", new String[] { "Oui", "Non" }, 9);
        addLabelAndComboBox(panel, gbc, "Type de logement", new String[] { "Location", "Co-location" }, 10);
        addLabelAndCustomField(panel, gbc, "Montant taxe foncière",
                new TextField(null, null, "euro-16x16-body.png").build(), 11);
        addLabelAndCustomField(panel, gbc, "Date permis de construire", new DateField(null).build(), 12);
        addLabelAndCustomField(panel, gbc, "Date construction bâtiment", new DateField(null).build(), 13);
        addLabelAndRadioButtons(panel, gbc, "Type de porte", new String[] { "Gauche", "Droite" }, 14);
        addLabelAndTextField(panel, gbc, "Access technologique", 15);
    }
    ...
    private JPanel createFileInputsPanel() {
        ...
        addFileInputs(fileInputsPanel, fileInputsGbc);

        return fileInputsPanel;
    }

    private void addFileInputs(JPanel panel, GridBagConstraints gbc) {
        addLabelAndFilePicker(panel, gbc, "Constat amiante", 0);
        addLabelAndFilePicker(panel, gbc, "Constat plomb", 1);
        addLabelAndFilePicker(panel, gbc, "Constat électricité", 2);
    }

    private void addPanelsToMain(JPanel mainPanel, GridBagConstraints gbc, JPanel formPanel, JPanel rentPanel,
            JPanel fileInputsPanel) {
        gbc.gridx = 0;
        gbc.gridy = 2;
        gbc.gridwidth = 1;
        gbc.weightx = 0.5;
    ...
    private void addBottomButtons(JPanel mainPanel, GridBagConstraints gbc) {
