...
<div class="flex flex-col h-screen gap-2 p-2">
	<!-- Top part -->
	<header class="flex shrink-0 justify-between items-center h-10">
		<div class="flex flex-row items-center gap-4">
			...
			<!-- Button group -->
			<div class="group rounded-lg overflow-hidden flex flex-row gap-px h-10">
				<a href="/app/exercises" class="flex flex-row gap-2 items-center text-base transition-colors group-hover:bg-neutral-900 hover:!bg-neutral-800 h-full px-2">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5"><path d="M3 12h.01"/><path d="M3 18h.01"/><path d="M3 6h.01"/><path d="M8 12h13"/><path d="M8 18h13"/><path d="M8 6h13"/></svg>
					Problem list
				</a>
				<a href="/app/exercises/random" class="flex flex-row gap-2 items-center text-base transition-colors group-hover:bg-neutral-900 hover:!bg-neutral-800 h-full px-2" aria-label="Go to a random exercise">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-4"><path d="M2 18h1.4c1.3 0 2.5-.6 3.3-1.7l6.1-8.6c.7-1.1 2-1.7 3.3-1.7H22"/><path d="m18 2 4 4-4 4"/><path d="M2 6h1.9c1.5 0 2.9.9 3.6 2.2"/><path d="M22 18h-5.9c-1.3 0-2.6-.7-3.3-1.8l-.5-.8"/><path d="m18 14 4 4-4 4"/></svg>
				</a>
			</div>
		</div>
		<div class="grid grid-cols-2 items-center gap-px">
			<!-- Run button -->
			<Tooltip content="Run <kbd>CTRL</kbd> <kbd>Enter</kbd>" delay={100} class="w-fit ml-auto" position="bottom">
				<button
					class="px-2 py-2 text-base rounded-l-lg font-medium text-neutral-100 bg-neutral-800 flex flex-row items-center gap-2"
					onclick={runCode}
				>
	...
	<!-- Main content -->
	<PaneGroup direction="horizontal" class="grow" autoSaveId="exercisePage">
		<!-- Left part -->
		<Pane defaultSize={50} class="flex flex-col grow gap-4">
			<!-- Error -->
			{#if error}
				<div
					transition:scale
			...
			<div class="flex flex-col grow justify-center bg-neutral-900 h-10 rounded-xl overflow-hidden">
		...
		<PaneResizer class="relative flex w-2 items-center justify-center bg-background">
			<div class="z-10 flex h-7 w-1 items-center justify-center rounded-sm bg-neutral-800">
			</div>
		</PaneResizer>
		<!-- Right part -->
		<Pane defaultSize={50} class="flex w-full flex-col">
			...
			<div class={cn("flex flex-col", editorFullScreen ? "fixed inset-2" : "")}>
				<!-- Full screen button -->
				<div class="flex flex-row items-center px-2 h-10 rounded-t-xl bg-neutral-800">
					<Tooltip content="Toggle Full Screen" position="left" class="ml-auto" delay={100}>
						 <button onclick={() => fullScreen = !fullScreen} class="flex flex-roe gap-2 items-center" aria-label="Toggle full screen">
							 {#if fullScreen}
								 <svg in:scale xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5"><path d="M8 3v3a2 2 0 0 1-2 2H3"/><path d="M21 8h-3a2 2 0 0 1-2-2V3"/><path d="M3 16h3a2 2 0 0 1 2 2v3"/><path d="M16 21v-3a2 2 0 0 1 2-2h3"/></svg>
							 {:else}
								 <svg in:scale xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg>
							 {/if}
						 </button>
					</Tooltip>
				</div>
			</div>
			<Editor language="javascript" defaultValue={exercise.content} bind:value bind:this={editor} onRunCodeShortcut={runCode} />
			<!-- Tests cases -->
