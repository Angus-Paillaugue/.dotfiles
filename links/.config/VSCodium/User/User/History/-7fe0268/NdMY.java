...
public class GarantDAO implements GarantDAODef {
  ...
  @Override
  public Location ajouterGarant(Personne garant, Location location, Colocation colocation) {
    try {
      Connector db = Connector.getInstance();
      String query = "INSERT INTO CoGarantir (idColocation, idCoGarant) VALUES (?, ?)";
      db.executeUpdate(query, Arrays.asList(colocation.get(), garant.getIdLocataire()));
      colocation.addGarant(garant);
      return colocation;
    } catch (SQLException e) {
      Logger.error(e);
    }
  ...
  public Location supprimerTousLesGarants(Location location) {
    try {
      ...
      query =
          "DELETE FROM Personne WHERE id IN (SELECT idGarant FROM Garantir WHERE idLocation = ?)";
      db.executeUpdate(query, Arrays.asList(location.getId()));
