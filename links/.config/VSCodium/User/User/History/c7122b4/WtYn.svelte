...
<aside
  class="fixed top-0 z-50 flex max-h-screen shrink-0 flex-col gap-0 overflow-y-auto border-r border-neutral-300/50 bg-white p-2 max-xl:pb-[58px] pl-0 transition-transform dark:border-neutral-700/50 dark:bg-neutral-900 max-xl:inset-0 max-xl:top-[3.8rem] max-xl:pt-0 xl:sticky xl:w-[300px] {open
    ? 'max-xl:translate-x-0'
    : 'max-xl:-translate-x-full'}"
>
  <div class="pl-4 pt-4 flex flex-col">
    <h3 class="m-0">{siteName}</h3>
    <span class="grow mx-2 mt-2 border-b border-neutral-300/50 dark:border-neutral-700/50"></span>
  </div>
