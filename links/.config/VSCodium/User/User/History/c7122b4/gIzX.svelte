...
<aside
  class="fixed top-0 z-20 flex max-h-screen shrink-0 flex-col justify-between gap-0 overflow-y-auto border-r border-neutral-300/50 bg-white p-2 max-xl:pb-[58px] pl-0 transition-transform dark:border-neutral-700/50 dark:bg-neutral-900 max-xl:inset-0 max-xl:top-[3.8rem] max-xl:pt-0 xl:sticky xl:w-[300px] {open
...
<button
  class="fixed left-[0.5rem] top-[0.5rem] z-10 flex aspect-square h-12 flex-col items-center justify-center xl:hidden"
  onclick={() => (open = !open)}
  name="search"
>
