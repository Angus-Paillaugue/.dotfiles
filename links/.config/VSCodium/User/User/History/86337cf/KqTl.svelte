<script>
	...
	import { Play, CloudDownload, Pause } from 'lucide-svelte';
	import { scale, slide, draw } from 'svelte/transition';
...
<div class="z-20 absolute max-lg:bottom-4 lg:top-2 w-52 left-1/2 -ml-[6.5rem]">
	<div class="grid grid-cols-2 items-center gap-px relative">
		...
		<div class="absolute -top-full max-lg:-translate-x-1/2 max-lg:-mt-2 max-lg:left-1/2 lg:left-full lg:ml-2 lg:top-0 rounded-lg overflow-hidden">
			<div class="flex flex-row gap-px">
				...
				>
					<button
						...
						class="flex flex-col relative items-center justify-center size-10 text-neutral-300 bg-neutral-900 lg:bg-neutral-800"
						onclick={() => {
							if (timerInterval) clearTimer();
							else startTimer();
						}}
					>
						<!-- <AlarmClock class="size-6" /> -->
						<div class="absolute rotate-45 h-[3px] w-2/3 bg-neutral-300 rounded-full"></div>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6">
							...
							{#if timer != null}
								<!-- <path d="m2 2 20 20" transition:draw={{ duration: 5000, delay: 500, easing: quintOut }} /> -->
							{/if}
							<path d="M12 9v4l2 2"/>
							<path d="M5 3 2 6"/>
							<path d="m22 6-3-3"/>
				...
				{#if timer !== null}
					<div class="flex flex-row gap-px w-fit" transition:slide={{ axis: 'x', duration:500 }}>
						<!-- Time display -->
						<div class="p-2 text-neutral-400 h-10 bg-neutral-900 lg:bg-neutral-800">
							{formatTime(timer)}
						</div>
						<!-- Play/Pause button -->
						<button
							class="flex size-10 bg-neutral-900 text-neutral-300 lg:bg-neutral-800 flex-col items-center justify-center"
