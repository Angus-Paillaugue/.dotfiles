	...
	<div class="grid grid-cols-2 items-center gap-px relative">
		...
		<div class="absolute -top-full max-lg:-translate-x-1/2 max-lg:-mt-2 max-lg:left-1/2 lg:left-full lg:ml-2 lg:top-0 rounded-lg overflow-hidden">
			<div class="flex flex-row gap-px">
				...
				{#if timer !== null}
					<div class="flex flex-row gap-px w-fit" transition:slide={{ axis: 'x', duration:5000 }}>
						<!-- Time display -->
						<div class="flex flex-col items-center px-2 justify-center text-center text-neutral-400 h-10 bg-neutral-900 lg:bg-neutral-800">
							{formatTime(timer)}
						</div>
						<!-- Play/Pause button -->
						<button
