<script>
  ...
  onMount(() => {
    if (!slider) return;
    slider.addEventListener('mousemove', move,handleMouseMove, false);
...
<div class={cn("flex flex-row gap-8 flex-no-wrap overflow-x-auto no-scrollbar", className)} bind:this={slider} {...restProps}>
  {@render children()}
</div>
