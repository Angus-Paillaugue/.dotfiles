...
function createNewRelease(release) {
  ...
  const lastVersionArray = lastVersion.version.slice(1).split('.');
  const newVersionArray = release.version.slice(1).split('.');

  for (let i = 0; i < lastVersionArray.length; i++) {
    if (newVersionArray[i] > lastVersionArray[i]) {
      break;
    }
    if (newVersionArray[i] < lastVersionArray[i]) {
      throw new Error('Version must be greater than the last version');
    }
  }
