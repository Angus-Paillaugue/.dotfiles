  ...
  onMount(async () => {
    window.addEventListener('keydown', (e) => {
      if (event.ctrlKey && event.key === 'k') {
        ...
        const focusClasses = "outline-none ring-2 ring-primary-500 dark:ring-primary-400 ring-offset-2";
        const addFocusClass = (el) => el.classList.add(focusClasses.split(' '));
        const removeFocusClass = (elArr) => elArr.forEach(el => el.classList.remove(focusClasses.split(' ')));
        // Input is focused
        if (e.key === 'ArrowDown') {
        ...
        removeFocusClass(Array.from(resultsContainer.children));
        addFocusClass(resultsContainer.children[currentlyFocusedSearchResult]);
      }
    });
    createPagesIndex();
    search = 'ready';
