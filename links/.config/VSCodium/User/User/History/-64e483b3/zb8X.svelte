...
{#if $searchModalShown}
  ...
  ></div>
  <div
    class="bg-neutral-100 z-[52] fixed max-md:top-0 max-md:left-0 max-md:right-0 md:top-1/2 md:left-1/2 md:-translate-y-1/2 md:-translate-x-1/2 dark:bg-neutral-800 md:rounded-t-md w-full max-w-screen-md md:rounded-md dark:text-white text-neutral-950 overflow-hidden"
    ...
    {#if results.length > 0}
      <div class="grid grid-cols-1 gap-2 p-4 max-h-[50vh] overflow-y-auto no-scrollbar">
        {#each results as result (result.url)}
          <a
            href={result.url}
            class="list-none w-full p-2 rounded-md border border-neutral-300/50 dark:border-neutral-700/50 flex flex-col gap-2 hover:bg-neutral-400 dark:hover:bg-neutral-700 transition-colors"
          >
            <h2 class="m-0">
              {@html result.name}
            </h2>
