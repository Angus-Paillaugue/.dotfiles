  ...
  onMount(async () => {
    window.addEventListener('keydown', (e) => {
      if (event.ctrlKey && event.key === 'k') {
        ...
        const addFocusClass = (i  ndex) => {
          resultsContainer.children[index].classList.add('focus:ring-2', 'focus:ring-primary-500');
        };
        // Input is focused
        if (e.key === 'ArrowDown') {
          currentlyFocusedSearchResult = Math.min(currentlyFocusedSearchResult + 1, results.length - 1);

          console.log(currentlyFocusedSearchResult, resultsContainer);
