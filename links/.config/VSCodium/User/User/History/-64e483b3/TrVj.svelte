      ...
      {#if results.length > 0}
        <div class="grid grid-cols-1 gap-2 p-4 max-h-[50vh] overflow-y-auto no-scrollbar">
          {#each results as result (result.url)}
            <a animate:flip={{ duration: 500 }} transition:fade href={result.url} class="list-none w-full p-2 rounded-md border border-neutral-300 dark:border-neutral-800 flex flex-col gap-2 hover:bg-neutral-400 dark:hover:bg-neutral-700 transition-colors">
              <h2 class="m-0">
                {@html result.name}
              </h2>
