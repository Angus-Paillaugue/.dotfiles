...
export async function logsOverviewWs(ws: WebSocket, req: Request) {
  ...
  const intervalId = setInterval(async () => {
    ...
    console.log('Sending logs overview statistics update');

    try {
      const logsOverviewStatistics = await LogDAO.getLogsOverviewStatistics();
      ws.send(
        JSON.stringify({
          action: 'logsOverviewStatisticsUpdate',
          logsOverviewStatistics,
          serverStatistics,
          success: true
        })
  ...
  logEventEmitter.on('newLogs', logChanged);

  ws.on('close', () => {
    clearInterval(intervalId);
    logEventEmitter.removeListener('newLogs', logChanged);
  });
}
