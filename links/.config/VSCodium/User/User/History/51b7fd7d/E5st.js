...
const jsDocParser = (jsDocComment) => {
  ...
  lines.forEach((line) => {
    ...
    if (paramMatch) {
      ...
      const defaultValueMatch = paramMatch[2].match(/\[(\w+)\s*=\s*([^\]]+)\]/);
      console.log(defaultValueMatch);

      if (defaultValueMatch) {
        param.name = defaultValueMatch[1]; // Update name to the part before the '='
        param.default = defaultValueMatch[2]; // Capture the default value
      }
