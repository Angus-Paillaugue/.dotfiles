...
public class ${testScriptName} {
	...
	try {
		const output = await new Promise((resolve, reject) => {
			exec(
				`sudo docker run --rm --memory=50m --cpus="0.5" -v ${testFilePath}:/app/${testScriptName}.java -v ${userInputScriptPath}:/app/${className}.java openjdk:17-slim /bin/sh -c "cd /app && javac ${testScriptName}.java ${className}.java && java ${testScriptName}"`,
				(error, stdout, stderr) => {
					if (error) {
						reject({ error: stderr || stdout });
					} else {
		...
		return {
			message:
				error?.error.split('\n')[0] ?? m.run_tests_filed_to_run_tests(),
			consoleOutput: null,
			ok: false
		};
	} finally {
