...
public class ${testScriptName} {
	...
	try {
		const output = await new Promise((resolve, reject) => {
			exec(
				`sudo docker run --rm --memory=50m --cpus="0.5" -v ${testFilePath}:/app/${testScriptName}.java openjdk:17-slim /bin/sh -c "cd /app && javac ${testScriptName}.java ${testScriptName}.java && java ${testScriptName}"`,
				(error, stdout, stderr) => {
					if (error) {
						reject({ error: stderr || stdout });
					} else {
