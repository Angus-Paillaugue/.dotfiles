...
public class ${testScriptName} {
	...
	try {
		const output = await new Promise((resolve, reject) => {
			exec(
				`sudo docker run --rm -v ${testFilePath}:/app/${testScriptName}.java -v ${userInputScriptPath}:/app/${className}.java openjdk:17-slim /bin/sh -c "cd /app && javac ${testScriptName}.java ${className}.java && java ${testScriptName}"`,
				(error, stdout, stderr) => {
					if (error) {
						reject({ error: stderr || stdout });
					} else {
		...
		const consoleOutput = parsedOutput.slice(0, -1);
		const results = parsedOutput.at(-1);
		console.log(results);
