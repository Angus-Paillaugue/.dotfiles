...
public class ${testScriptName} {
    ...
    public List<Map<String, Object>> runTests() {
			...
			try {
					...
					testResult.put("passed", result.equals(${test.expected_output}));
	...
	try {
		const output = await new Promise((resolve, reject) => {
			exec(
				`sudo docker run --rm --memory=20m --cpus="0.5" -v ${testFilePath}:/app/${testScriptName}.java -v ${userInputScriptPath}:/app/${className}.java openjdk:17-slim /bin/sh -c "cd /app && javac ${testScriptName}.java ${className}.java && java ${testScriptName}"`,
				(error, stdout, stderr) => {
					if (error) {
						reject({ error: stderr || stdout });
					} else {
		...
		const consoleOutput = output.split('\n').filter((e) => e).slice(0, -1);
		const results = JSON.parse(
			output
				...
				.replace(/,(\s*[\w"])/g, ',$1') // Ensure proper comma spacing
				.replace(/(\w+)=/g, '"$1":') // Replace `key=` with `"key":` again
		...
		return {
			message:
				error?.error[0] ?? m.run_tests_filed_to_run_tests(),
			consoleOutput: null,
			ok: false
		};
	} finally {
