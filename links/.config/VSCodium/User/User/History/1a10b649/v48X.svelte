<script>
  ...
  const baseToastClasses =
    'rounded-xl border bg-neutral-100 dark:bg-neutral-950 border-neutral-300 dark:border-neutral-800 p-4 relative text-neutral-600 dark:text-neutral-300';
  ...
  {#each $toasts.slice(0, 4) as toast (toast.id)}
    ...
    >
      ...
      <button class="absolute top-2 right-2" on:click={() => removeToast(toast.id)}>
        <Icon icon="ri:close-line" class="size-5" /><Icon icon="material-symbols:close-rounded" />
      </button>
    </div>
  {/each}
</div>
