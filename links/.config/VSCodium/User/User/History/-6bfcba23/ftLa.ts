export type WindowSate = 'closed' | 'minimized' | 'maximized' | 'default';
...
export class Draggable {
    ...
    const closeBtn = this._element.querySelector('.closeBtn') as HTMLButtonElement;
    const minimizeBtn = this._element.querySelector('.minimizeBtn') as HTMLButtonElement;
    const maximizeBtn = this._element.querySelector('.maximizeBtn') as HTMLButtonElement;
    ...
    const titleBar = this._element.querySelector('.titleBar') as HTMLElement;
		if (titleBar) {
	...
	elementDrag(e: MouseEvent) {
		if (this._visible === 'maximized' || !this._element) return;
		e.preventDefault();
	...
	setState(state: WindowSate) {
    if (this._visible === state || !this._element) return;
		switch (state) {
			case 'closed':
				this.hide();
				break;
