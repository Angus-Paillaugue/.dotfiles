<script>
  ...
  onMount(() => {
    ...
    window.onmousemove = (e) => {
      if (isHovered && !tooltipContent.contains(e.target)) {
        hideTooltip();
      }
    };
...
{#if isHovered}
  ...
  >
    <div
      class="relative rounded-lg flex items-center gap-2 p-2 text-sm border border-neutral-700 bg-neutral-800 text-text-body-dark"
    >
      {@html content}
    </div>
  </div>
