<script>
  ...
  import icons from 'websemantics/file-icons-js'

  const { commands, showIcons = true } = $props();
...
<div class="w-full">
      ...
      {#each commands as command, i}
        <li class="mb-0 list-none {selectedIndex === i && 'selected'}">
          <button
            ...
            class="px-4 py-2 relative overflow-visible cursor-pointer flex flex-row gap-2 items-center"
          >
            {#if showIcons}
              <span class={icons.getClassWithColor(command.name.toLowerCase())}></span>
            {/if}
            {command.name}
          </button>
        </li>
  ...
  <div class="border border-neutral-300 dark:border-neutral-800">
    ...
    >
      <code class="text-lg command">{commands[selectedIndex].command}</code>
