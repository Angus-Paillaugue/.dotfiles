<script>
  ...
  import { getHighlighter } from 'shiki'
	import ShikiMagicMove from '$lib/components/core/ShikiMagicMove.svelte';
  ...
  let curentCommand = $state(commands[0].command);
  const highlighter = getHighlighter({
		themes: [codeBlockTheme],
		langs: ['bash'],
	})

  $inspect(curentCommand);
    ...
    >
      {#each commands as command, i}
        <li class="mb-0 list-none {selectedIndex === i && 'selected'}">
          <button
            onclick={() => {selectedIndex = i; curentCommand = command.command}}
