<script lang="ts">
	...
	import { Menu, RefreshCcw } from 'lucide-svelte';
...
<div class="h-16 shrink-0 border-b border-border flex flex-row p-4">
	<button onclick={() => (downloadSongModalOpen = true)}>Download</button>
	<Button variant={["icon", "border"]} onclick={() => (sidebarOpen = !sidebarOpen)}>
		{#if sidebarOpen}
