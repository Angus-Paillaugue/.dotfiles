<script lang="ts">
	...
	async function downloadSong(songURL: URL, format: typeof songFormat) {
		...
		const reader = response.body.pipeThrough(new TextDecoderStream()).getReader();
		songDownloadingResponseStreamData = { event: 'songName', playlistName: '', songs: { total: -1, current: -1 } };
		while (true) {
			let { value, done } = await reader.read();
			if (done) break;
			if(!value) continue;
			...
			switch(parsedValue.event) {
				case 'songName':
					songDownloadingResponseStreamData.playlistName = parsedValue.data as string;
					break;
				case 'end':
					toast.success('Playlist downloaded successfully');
					downloadSongModalOpen = false;
