<script lang="ts">
	...
	let songDownloadingResponseStreamData = $state<{ event: 'songName' | 'end' | 'error'; songName: string; } | null>(null);
	...
	async function downloadSong(songURL: URL, format: typeof songFormat) {
		...
		const reader = response.body.pipeThrough(new TextDecoderStream()).getReader();
		songDownloadingResponseStreamData = { event: 'songName', songName: '' };
		while (true) {
			let { value, done } = await reader.read();
			if (done) break;
			if(!value) continue;
			const parsedValue: { event: typeof songDownloadingResponseStreamData['event']; data: string | Song} = JSON.parse(value);
			if(!parsedValue) continue;
			switch(parsedValue.event) {
				case 'songName':
					songDownloadingResponseStreamData.songName = parsedValue.data as string;
					break;
				case 'end':
					toast.success('Playlist downloaded successfully');
					downloadSongModalOpen = false;
					const s = parsedValue.data as Song;
					$songs.unshift(s);
	...
	async function download(e: Event) {
		...
		songDownloadingResponseStreamData = null;
		try {
			...
			if(isUrlAPlaylist(songURL)) {
				...
				// Download a song
				await downloadSong(songURL, format);
			}
		} catch (e) {
...
<Modal bind:open={downloadSongModalOpen}>
	...
	<form onsubmit={download} method="post" class="mt-4 flex flex-col gap-4">
		...
		{/if}
		{#if songDownloadingResponseStreamData}
			<div class="flex flex-row justify-between">
				<p>Downloading <b>{songDownloadingResponseStreamData.songName}</b></p>
			</div>
		{/if}
		{#if downloadError}
