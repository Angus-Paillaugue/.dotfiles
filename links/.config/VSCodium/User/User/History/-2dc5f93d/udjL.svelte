<script>
	...
	$effect(() => {
		...
		const MAX_DISTANCE = 100; // You can adjust this value as needed

		bookmarksMatchingSearch = bookmarks.filter((b) => {
			const titleDistance = levenshtein.get(b.title.toLowerCase(), searchInputValue.toLowerCase());
			const descriptionDistance = levenshtein.get(b.description.toLowerCase(), searchInputValue.toLowerCase());
			return titleDistance <= MAX_DISTANCE || descriptionDistance <= MAX_DISTANCE;
		});
	});
</script>
