<script>
	...
	$effect(() => {
		...
		console.log(fuseList.search(searchInputValue));

		bookmarksMatchingSearch = fuseList.search(searchInputValue);
	});
</script>
...
<div class="flex grow flex-col gap-8 items-center justify-start p-4 relative overflow-hidden">
	...
	{/if}
	{#each bookmarksMatchingSearch as article, i (article.id)}
		<button
			onclick={() => {
