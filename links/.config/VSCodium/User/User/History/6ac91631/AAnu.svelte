...
{:else}
  <svelte:element bind:this={element} this={tagName} class={cn(baseClasses, variantClasses.get(variant), className)} {...restProps}>
    {#if loading}
      <span in:fade class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <Spinner class="size-6" />
      </span>
    {:else}
      <div transition:slide={{ axis:'x', duration:500 }} class="flex flex-row items-center justify-center gap-2">
        {@render children()}
      </div>
    {/if}
  </svelte:element>
