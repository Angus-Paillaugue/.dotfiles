<script>
  ...
  const { children, class:className, variant:test="primary", loading=false, ...restProps } = $props();
  let variant = $effect(() => test.split(' '));
  ...
  let element = $state();
  console.log(...variants.map(v => variantClasses.get(v)));
