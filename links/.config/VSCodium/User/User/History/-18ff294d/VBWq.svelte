...
<!-- Databases -->
<h2 class="mt-10 scroll-m-20 pb-2 text-3xl font-semibold tracking-tight mb-4">Databases</h2>
				...
				<ScrollArea class="md:max-h-[300px] flex flex-col items-center w-full">
					{#each databases.list as database, index}
						...
						>
							...
							<h3 class="scroll-m-20 text-xl font-semibold tracking-tight">{database.name}</h3>
							<p class="ml-auto m-0 leading-7 [&:not(:first-child)]:mt-6">
								{database.size ?? 0} Mb
							</p>
						</button>
					{/each}
					...
					<tbody>
						{#if databases.list[databaseModalDatabaseIndex]}
							{#each Object.entries(databases.list[databaseModalDatabaseIndex]) as [key, value]}
								<tr class="border-y odd:bg-muted">
