...
class LogDAO {
  ...
  async getLogs(limit: number = 100, offset: number = 0, level: LogLevel | 'all'): Promise<{ logs:Log[]; total:number; hasMore: boolean, success:boolean; error?:unknown;}> {
    try {
      const getLogsSQL = `
        SELECT *
        FROM logs
      ...
      return { logs, total: totalNumberOfLogs[0].total, hasMore: totalNumberOfLogs[0].total > offset + limit, success: true };
    } catch (error) {
      console.error("Error getting logs:", error);
      return { logs: [], total: 0, hasMore: false, error: error, success: false };
    }
  }
