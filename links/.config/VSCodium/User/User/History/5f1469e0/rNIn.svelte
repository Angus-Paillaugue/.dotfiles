			...
			<div class="relative w-full overflow-x-auto">
				{#if submissions.length === 0}
					<div class="mx-4 mt-10 grow rounded-lg bg-neutral-700 p-4">
						<h2 class="text-base font-medium">{m.app_exercise_left_pane_solutions_tab_no_solutions()}</h2>
					</div>
				{:else}
					<table class="w-full table-auto text-left text-sm text-neutral-400 rtl:text-right">
						<thead class="text-xs uppercase text-neutral-400">
							<tr>
								<th scope="col" class="px-6 py-3"> {m.app_exercise_left_pane_solutions_tab_table_col_code()} </th>
								<th scope="col" class="px-6 py-3"> {m.app_exercise_left_pane_solutions_tab_table_col_date()} </th>
								{#if !submissions.every((s) => s.ram_usage === null)}
									<th scope="col" class="px-6 py-3"> {m.app_exercise_left_pane_solutions_tab_table_col_ram_usage()} </th>
								{/if}
							</tr>
						</thead>
						<tbody>
