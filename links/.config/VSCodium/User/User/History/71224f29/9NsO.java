    ...
    if (target.getName() != null && target.getName().equals("cancelButton")) {
      ...
      this.vue.dispose();
			...
			for(String[] row: values) {
				row[2] = "01/"+row[2];
				Loyer loyer = new Loyer(Float.parseFloat(row[3]), 1, VuesUtils.toLocalDate(LocalDate.parse(row[2])),
						VuesUtils.toLocalDate(LocalDate.parse(row[2])), ModeDePaiment.CHEQUE);
