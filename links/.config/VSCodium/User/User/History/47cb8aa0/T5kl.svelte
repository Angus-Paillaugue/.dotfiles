<script>
  ...
  function onWindowClickHandler(event) {
    if (!event.target.closest('.dropdown-trigger')) open = false;
  }
...
<div class={cn('dropdown relative w-fit', className)} {...restProps}>
  ...
  <div
    class={cn(
      'absolute z-30 w-52 rounded-xl border border-neutral-700/50 bg-neutral-800/50 backdrop-blur-md text-neutral-100',
