...
export class knownServerIdsCache {
  ...
  static lastChecked: Date;
  static MAX_CACHE_TIME = 1000 * 60; // 1 minute
  ...
  static async pruneCache() {
    if (knownServerIdsCache.lastChecked && knownServerIdsCache.lastChecked.getTime() + knownServerIdsCache.MAX_CACHE_TIME < Date.now()) {
      console.log('Pruning known server ids cache');
