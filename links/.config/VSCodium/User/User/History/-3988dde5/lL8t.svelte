...
<div class="relative left-0 right-0 top-4 flex flex-row items-start justify-center">
	<div class="w-fit relative flex flex-col items-center justify-start">
		{#each toasts.slice(0, 1) as toast, i (toast.id)}
			...
			<div
				class="group/toast w-[250px] md:w-[300px] flex cursor-pointer flex-row items-center justify-between gap-4 rounded-[1.75rem] bg-black p-3 text-white transition-all hover:h-52 hover:w-[500px] h-14 relative shake transform"
				data-id={toast.id}
