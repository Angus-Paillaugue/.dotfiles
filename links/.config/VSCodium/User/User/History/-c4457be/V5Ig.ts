...
export const GET: RequestHandler = async ({ url }) => {
	const playlistId = Number(url.searchParams.get('playlistId')) as number;
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			c.on('exit', async () => {
				...
				try {
					await createPlaylist(playlistName || playlistId.toString());
					const files = await readdir(incompleteDir);
					const songs: Song[] = [];
					...
					for (const file of files) {
						...
						if (file.endsWith(`.${format}`)) {
							try {
								...
								const songCoverPath = await saveSongCover(playlistId.toString());
