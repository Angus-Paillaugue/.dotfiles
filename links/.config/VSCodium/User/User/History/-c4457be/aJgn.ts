...
import { saveSongCover, getSongInfoFromId, incompleteDirName, coverDirName, songsDirName } from '$lib/songs';
		...
		start(controller) {
			...
			c.stdout.on('data', (data) => {
				...
				if (songIndex.current > -1) {
					...
					controller.enqueue(JSON.stringify({ event: 'songIndex', data: songIndex - 1 }));
				}
			});
			...
			c.on('exit', async () => {
				...
				try {
					await createPlaylist(playlistName || playlistId);
					const files = await readdir(incompleteDir);
					const songs: Song[] = [];
					...
					for (const file of files) {
						...
						if (file.endsWith(`.${format}`)) {
							try {
								...
								const song = await getSongInfoFromId(playlistId);
								const songCoverPath = await saveSongCover(playlistId);
								song.coverPath = songCoverPath;
								await addSong(song);
								songs.push(song);
							} catch (error) {
								console.error(error);
								controller.enqueue(JSON.stringify({ event: 'error', data: error instanceof Error ? error.message : error }));
							}
					...
					controller.enqueue(JSON.stringify({ event: 'end', data: songs }));
					controller.close();
					streamClosed = true;
				} catch (error) {
