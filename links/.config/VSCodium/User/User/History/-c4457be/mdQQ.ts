...
import { addSongToPlaylist, createPlaylist } from '$lib/db/playlist';
		...
		const stream = new ReadableStream({
			start(controller) {
				...
				c.on('exit', async () => {
					...
					try {
						await createPlaylist(playlistName || playlistId.toString(), playlistId.toString());
					} catch (error) {
					...
					for (const file of files) {
						...
						if (file.endsWith(`.${format}`)) {
							console.log('Processing song:', file);
							try {
								...
								console.error("Couldn't add song to database.", error);
								controller.enqueue(
									JSON.stringify({
										event: 'error',
