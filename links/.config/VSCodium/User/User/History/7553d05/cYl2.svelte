<script lang="ts">
  ...
  import type { Scroll } from 'lucide-svelte';
...
{#if searchModalShown}
  ...
  <div class="fixed inset-0 z-50 p-4 sm:p-6 md:p-12">
    <div
      class="no-scrollbar mx-auto max-h-[300px] w-full max-w-screen-md overflow-hidden overflow-y-auto rounded-2xl border border-main bg-neutral-100"
      transition:scale={{ start: 0.8, duration: 200 }}
      onscroll={onResultsScroll}
    >
      <div
        class="sticky top-0 z-10 w-full border-b border-transparent bg-neutral-100 p-2 transition-all"
        id="searchContainer"
      >
        <div class="relative">
          <Icon
          ...
          {#each results as result, index}
            ...
            >
              ...
              >
                ...
                {/if}
                {#if result?.component}
                  <p class="m-0 line-clamp-3 truncate">
              ...
              <Icon
                ...
                class="group-hocus:opacity-100 text-primary-400 opacity-0 transition-opacity group-focus:opacity-100 [.group.selected_&]:opacity-100"
              />
            </a>
          {/each}
        </div>
