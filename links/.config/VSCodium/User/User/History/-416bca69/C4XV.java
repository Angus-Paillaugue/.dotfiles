...
public class MesAppartements extends JFrame {
	...
	public MesAppartements(Proprietaire proprietaire) {
		...
		for (Logement logement : logements) {
			if (logement.getLocation() != null) {
				...
				// Est une colocation
				String locataires = logement.getColocation().getColocataires().keySet().stream()
	...
	private void createApartmentCard(Logement logement) {
		String tennantName;
		if(logement.getLocation() != null) {
			tennantName = logement.getLocation().getLocataires().stream()
					.map(p -> p.getNom() + " " + p.getPrenom())
					.collect(Collectors.joining(", "));
		} else if (logement.getColocation() != null) {
			tennantName = logement.getColocation().getColocataires().keySet().stream()
					.map(p -> p.getNom() + " " + p.getPrenom())
					.collect(Collectors.joining(", "));
		} else {
			tennantName = "Pas de locataire";
		}
