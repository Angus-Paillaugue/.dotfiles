...
public class MesAppartements extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(
				() -> {
					try {
						MesAppartements frame = new MesAppartements(Constants.proprietaire);
	...
	public MesAppartements(Proprietaire proprietaire) {
		...
		List<Logement> logements = batiments.stream()
	...
	private JPanel createNavbar() {
		...
		JButton addApartment = new Button("Ajouter un batiment", Constants.ICONS_PATH + "plus-16x16-heading.png").build();
		addApartment.addActionListener(
				e -> {
					new AjouterLogement().setVisible(true);
				});

		JButton addApartment = new Button("Ajouter un logement", Constants.ICONS_PATH + "plus-16x16-heading.png").build();
		addApartment.addActionListener(
				e -> {
					new AjouterLogement().setVisible(true);
				});

		JButton addTenant = new Button("Ajouter un locataire", Constants.ICONS_PATH + "plus-16x16-heading.png").build();
		...
		actionButtonsNav.add(addTenant);
	...
	private void createApartmentCard(Logement logement) {
		...
		if(logement.getLocation() != null) {
			tenantName = logement.getLocation().getLocataires().stream()
					.map(p -> p.getNom() + " " + p.getPrenom())
					.collect(Collectors.joining(", "));
		} else if (logement.getColocation() != null) {
			tenantName = logement.getColocation().getColocataires().keySet().stream()
					.map(p -> p.getNom() + " " + p.getPrenom())
					.collect(Collectors.joining(", "));
		} else {
