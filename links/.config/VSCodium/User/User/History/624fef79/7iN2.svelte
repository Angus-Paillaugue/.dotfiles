...
{#if mobileMenuOpen}
  ...
  <div class="fixed z-30 top-0 md:hidden bottom-0 left-0 w-2/3 flex flex-col gap-10 p-6 bg-background" transition:fly={{ x: '-100%' }}>
    ...
    <!-- CTA -->
     {#if hasARegisteredUser}
      <Button href="/log-in" class="mt-auto">
        Log-in
      </Button>
    {:else}
      <Button href="/register" class="mt-auto">
        Register
      </Button>
    {/if}
  </div>
...
<div class="fixed top-2 z-20 left-2 right-2">
  <div class="max-w-screen-xl mx-auto w-full">
    <nav class={cn("flex transition-all text-primary mx-auto flex-row duration-500 justify-between items-center px-3 h-16 rounded-2xl border", isNavActive ? 'md:w-2/3 w-[90%] bg-background shadow border-border' : 'w-full bg-sidebar border-sidebar')}>
      ...
      <Button variant="outline" class="md:hidden p-2 size-fit" onclick={() => (mobileMenuOpen = !mobileMenuOpen)}>
        <Menu class="size-6" />
      </Button>
    </nav>
  </div>
