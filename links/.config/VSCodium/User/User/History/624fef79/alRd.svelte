...
{#if mobileMenuOpen}
  <div class="fixed inset-0 bg-neutral-900/50 z-30" transition:fade></div>
  .fixed.z-30.top-0.bottom-0.left-0.w-full.max-w-screen.min-w-
{/if}

<div class="fixed top-2 left-2 right-2">
  <div class="max-w-screen-xl mx-auto w-full">
    <nav class={cn("flex transition-all z-20 text-primary mx-auto flex-row duration-500 justify-between items-center px-3 h-16 rounded-2xl border", isNavActive ? 'w-2/3 bg-background shadow border-border' : 'w-full bg-sidebar border-sidebar')}>
