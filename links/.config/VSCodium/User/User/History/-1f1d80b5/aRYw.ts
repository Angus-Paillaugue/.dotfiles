...
export async function createPlaylist(title: Playlist['title'], id: Playlist['id'] | null = null): Promise<Playlist['id']> {
...
export async function addSongToPlaylist(songId: Song['id'], playlistId: Playlist['id']): Promise<void> {
	const query = `INSERT INTO beInPlaylist (songId, playlistId) VALUES (?, ?)`;
	await db.execute(query, [songId, playlistId]);
}

export async function removeSongFromPlaylist(songId: Song['id'], playlistId: Playlist['id']): Promise<void> {
