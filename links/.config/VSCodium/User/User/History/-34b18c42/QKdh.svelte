<script lang="ts">
	...
	let updatedTrack = $state<Song>(JSON.parse(JSON.stringify(track)));
	let confirmSongDeletionModalOpen = $state<boolean>(false);

		$inspect(updatedTrack)
...
<Modal bind:open={confirmSongDeletionModalOpen}>
	...
	<div class="flex gap-4 items-center flex-row justify-between mt-4">
		<Button
			variant="border"
...
{#if contextMenuOpen}
	...
	>
		...
		<button
			onclick={() => {confirmSongDeletionModalOpen = true}}
