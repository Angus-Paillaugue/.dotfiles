<script lang="ts">
	...
	import { Download, ListMusic, LoaderCircle, PenIcon, Trash2 } from 'lucide-svelte';
...
{#if contextMenuOpen && track}
	...
	>
		...
		<a
			href="{track.filePath}"
			download="{track.artist.name} - {track.title}.{track.mediaType}"
			class="text-text flex w-full flex-row items-center gap-2 border-b border-border px-4 py-2 text-sm transition-colors hover:bg-secondary"
		>
