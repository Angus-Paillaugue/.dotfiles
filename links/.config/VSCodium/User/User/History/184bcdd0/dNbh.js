...
export function allPages() {
  ...
  function traverseAndAddMetadata(pages, baseUrl = '') {
    return pages.map((page) => {
      if (typeof page === 'string') {
        ...
        const slug = pageUrl.split('/').slice(2).join('/')?.replace('.md', '');
        return { name: page, ...metadata.metadata, component: metadata.default, url: pageUrl, slug };
      }  else {
  ...
  console.log(traverseAndAddMetadata(updatedPages, docsUrlStart));
