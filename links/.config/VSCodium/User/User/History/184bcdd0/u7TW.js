...
export async function pageBySlug(slug) {
  ...
  // Internal recursive function to traverse and find the post
  function traverseAndFindPost(pages) {
    for (const page of pages) {
      if (page.url === slug) return page;
      if (page.children) {
        const foundPost = traverseAndFindPost(page.children);
        if (foundPost) return foundPost;
      }
    }
  }

  return traverseAndFindPost(posts);
}
