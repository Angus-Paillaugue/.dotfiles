		...
		<div class="w-fit relative flex flex-col items-center justify-start">
			{#each $toasts.slice(0, 1) as toast (toast.id)}
				...
				>
					...
					<!-- {:else} -->
						<h3 class="text-xl m-0 font-bold text-end overflow-hidden group-hover/toast:translate-x-full transition-all {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
							{toast.title}
						</h3>
					<!-- {/if} -->
				</div>
