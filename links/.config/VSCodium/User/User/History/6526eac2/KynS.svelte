				...
				>
					...
					<!-- {#if toast.hover} -->
						<div
							...
							class="flex grow flex-col justify-between overflow-hidden h-0 group-hover/toast:h-full"
						>
					...
					<!-- {:else} -->
						<h3 class="text-xl m-0 font-bold w-4/3 overflow-hidden group-hover/toast:w-0 {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
							{toast.title}
						</h3>
					<!-- {/if} -->
				</div>
