			...
			{#each $toasts.slice(0, 1) as toast (toast.id)}
				...
				>
					...
					<div
						...
						class="flex grow flex-col justify-between overflow-hidden -translate-y-[calc(100%+1.5rem)] duration-500 group-hover/toast:translate-y-0 h-full transition-all"
					>
					...
					<h3 class="text-xl m-0 font-bold absolute top-1/2 -translate-y-1/2 duration-500 right-3 left-10 text-end overflow-hidden group-hover/toast:translate-x-full transition-all {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
						{toast.title}
					</h3>
				</div>
			{/each}
