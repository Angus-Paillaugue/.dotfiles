		...
		<div class="w-fit relative flex flex-col items-center justify-start">
			{#each $toasts.slice(0, 1) as toast (toast.id)}
				...
				>
					...
					<h3 class="text-xl m-0 font-bold absolute top-1/2 opacity-0 duration-300 group-hover/toast:opacity-100 right-3 left-10 text-end overflow-hidden transition-all {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
						{toast.title}
					</h3>
				</div>
			{/each}
