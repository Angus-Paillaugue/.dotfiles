				...
				>
					...
					<!-- {#if toast.hover} -->
						<div
							...
							class="flex grow flex-col justify-between overflow-hidden h-0 group-hover/toast:h-48 transition-all"
						>
							<p class="overflow-y-auto m-0 text-base font-sans font-normal text-text-body-dark">{toast.message}</p>
							<div class="flex flex-row gap-4">
								{#if toast.closeButton}
									<button
										class="w-full rounded-full bg-neutral-600 dark:bg-neutral-700 text-text-heading-dark p-2 grow opacity-0 group-hover/toast:opacity-100 transition-opacity"
					...
					<!-- {:else} -->
						<h3 class="text-xl m-0 font-bold w-52 text-end overflow-hidden group-hover/toast:w-0 transition-all {toastColors[toast.type]}" in:fade={{ delay: 150, easing: quintOut }}>
							{toast.title}
						</h3>
					<!-- {/if} -->
				</div>
