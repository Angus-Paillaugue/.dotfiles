...
export async function createNewUser(username: string, hash: string): Promise<void> {
	await db.insert(users).values({ username, password_hash: hash, admin: false });
}
...
export async function getProfile(userId: number): Promise<Profile> {
	...
	const userRank = await db
		.select({
			...
			no_exercises: sql`0`.as<number>() // Temporary value to include the property
		})
		.from(users)
	...
	const recentActivity = await db
		.select({
			completed_at: sql`COALESCE(${submissions.completed_at}, NOW())`.as<Date>(),
			exercise_id: sql`COALESCE(${exercises.id}, 0)`.as<number>(),
			exercise_difficulty: sql`COALESCE(${exercises.difficulty}, 'unknown')`.as<string>(),
			title: sql`COALESCE(${exercises.title}, 'unknown')`.as<string>(),
			slug: sql`COALESCE(${exercises.slug}, 'unknown')`.as<string>()
		})
		.from(submissions)
...
export async function getDefaultProfilePicturePath(): Promise<string> {
	const result = await db
		.select({ default_path: sql`Column_Default` })
		.from(sql`Information_Schema.Columns`)
		.where(sql`Table_Name = 'users' AND Column_Name = 'profile_picture'`);

	if (result.length === 0) {
		throw new Error(m.error_messages_db_users_default_profile_picture_path_not_found());
	}

	return result[0].default_path as string;
}
