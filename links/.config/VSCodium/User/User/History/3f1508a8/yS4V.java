		...
		if (target.getName() != null && target.getName().equals("cancelButton")) {
			...
			for(int i = 0; i < this.vue.radioPanel.getComponentCount(); i++) {
				...
				if(radio.isSelected()) {
					...
					List<BienLouable> biens = Constants.proprietaire.getBatiment().stream().flatMap(bat -> bat.getBiensLouables().stream()).collect(Collectors.toList());
					new ContractDeLocation(Constants.proprietaire.getLocataires().get(idLocataire)).setVisible(true);
				}
			}
		} else {
