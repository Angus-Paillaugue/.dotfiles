<script>
  ...
  import { searchModalShown, scrollPercentage } from '$lib/stores';
  import { Button } from '$lib/components';
  import { spring } from 'svelte/motion';

  const { title } = $props();

  let progressWidth = spring(
    $scrollPercentage,
    {
      stiffness: 0.1,
      damping: 0.25
  ...
  scrollPercentage.subscribe((value) => {
    progressWidth.set(value);
  });
</script>

<nav
  class="sticky top-0 z-20 h-16 w-full border-b border-main bg-white/50 backdrop-blur dark:border-main-dark dark:bg-neutral-900/50 xl:h-24"
>
  <div class="flex flex-row items-center justify-between relative w-full h-full px-4">
    ...
    <div class="absolute h-px top-full left-0 right-0">
      <div class="bg-primary-400 h-px" style:width={$progressWidth+"%"}></div>
    </div>
  </div>
</nav>
