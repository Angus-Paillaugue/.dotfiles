<script>
  ...
  function getLocaleConfig(locale, locales) {
    const configLocales = Object.keys(i18n.locales);
    let selectedLocale = locales.includes(locale) ? locale : null;
    if (!selectedLocale) {
      ...
      selectedLocale = locales.includes(languageCode) ? languageCode : 'en';
    }

    return selectedLocale
  }

  console.log(getLocaleConfig($locale, $locales));
