...
function createBackup() {
  ...
  const unallowedFiles = ['.git', 'node_modules', 'backup'];
  const thingsToCopy = readdirSync(process.cwd()).filter((name) => !unallowedFiles.includes(name));
  for (const thing of thingsToCopy) {
    cpSync(thing, path.join(backupDir, thing));
  }
}
