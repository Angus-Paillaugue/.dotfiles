...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
			...
			{:else}
				<div class="flex flex-col items-center">
					{#each databases.list as database}
						<button class="w-full flex flex-row gap-4 items-center my-4 border-b last:border-none transition-colors hover:bg-secondary rounded" data-name={database.name} onclick={(e) => {databaseModalDatabaseName = e.target.closest('button').dataset.name;databaseModalOpen = true}}>
	...
	<Card>
		...
		<CardContent>
			<div class="flex flex-col items-center">
				<div class="w-full flex flex-row gap-4 items-center p-4 border-b last:border-none">
					<Cpu class="size-6" />
					...
					{:else}
						<div class="flex flex-row justify-between grow">
							...
							<p class="ml-auto text-muted-foreground text-sm">
								{(serverMetrics.memory.free / 1024).toFixed(2)} / {(serverMetrics.memory.total / 1024).toFixed(2)} Gb
							</p>
						</div>
					{/if}
				</div>
