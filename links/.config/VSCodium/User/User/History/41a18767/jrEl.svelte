...
<div class="mx-auto w-full flex max-w-screen-md flex-col gap-4 p-4 md:gap-8 md:p-8">
  <Tabs.Root value="account">
    <!-- Panel selector -->
    <Tabs.List class="grid w-full grid-cols-2">
      <Tabs.Trigger value="account">Your Account Settings</Tabs.Trigger>
      {#if hasPermission(user?.role, PERMISSIONS.READ_OTHER_ACCOUNTS) || hasPermission(user?.role, PERMISSIONS.CREATE_OTHER_ACCOUNTS) || hasPermission(user?.role, PERMISSIONS.DELETE_OTHER_ACCOUNTS) || hasPermission(user?.role, PERMISSIONS.UPDATE_OTHER_ACCOUNTS)}
                      ...
                      <Card.Root>
                        <Card.Header class="p-6">
                          <div class="flex flex-row justify-between items-center">
                            ...
                            {#if !isOwner && (hasPermission(user?.role, PERMISSIONS.UPDATE_OTHER_ACCOUNTS) || hasPermission(user?.role, PERMISSIONS.DELETE_OTHER_ACCOUNTS))}
                              <DropdownMenu.Root>
                                ...
                                <DropdownMenu.Content>
                                  <DropdownMenu.Group>
                                    ...
                                    >
                                      <Trash2 class="size-4 mr-2 text-red-600" />
                                      Delete
                                    </DropdownMenu.Item>
                                    {/if}
                                  </DropdownMenu.Group>
