<script lang="ts">
  ...
  let deleteAccountModalConfirm:{ visible: boolean; user:User|null} = $state({ visible: false, user: null });
  let isDeleingUserAccount: boolean = $state(false);
  let editUserModal: { visible: boolean; user: User | null } = $state({ visible: false, user: null });
    ...
    if (form?.success) {
      ...
      switch (form.action) {
        ...
        case 'deleteUserAccount':
          allUsers.splice(allUsers.findIndex((u) => u.id === deleteAccountModalConfirm?.user?.id), 1);
        ...
        case 'editUserAccount':
          if (editUserModal?.user) {
            ...
            allUsers[userIndex].username = editUserModal.user.username;
            allUsers[userIndex].role = editUserModal.user.role;
            console.log(allUsers[userIndex], editUserModal.user);
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        This action cannot be undone. This will permanently delete {deleteAccountModalConfirm.user?.username}'s account.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
      <form
      ...
      >
        ...
        <Button disabled={isDeleingUserAccount} loading={isDeleingUserAccount} type="submit">Continue</Button>
      </form>
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
          ...
          >
            <div class="flex flex-col gap-1">
              ...
              <Input
                ...
                bind:value={editUserModal.user.username}
              />
            </div>

            <div class="flex flex-col gap-1">
              <Label for="role">Role</Label>
              <Select.Root type="single" name="role" bind:value={editUserModal.user.role}>
            ...
            <Button type="submit" disabled={isSavingUserAccount} loading={isSavingUserAccount}>Save</Button>
          </form>
          ...
          >
            ...
            <div class="flex flex-col gap-1">
              ...
              <Select.Root type="single" name="role">
                <Select.Trigger>{editUserModal.user?.role}</Select.Trigger>
                <Select.Content>
                  {#if editUserModal.user?.role === 'admin'}
                    <Select.Item value="admin" data-selected>Admin</Select.Item>
                    <Select.Item value="guest">Guest</Select.Item>
                  {:else}
                    <Select.Item value="guest" data-selected>Guest</Select.Item>
                    <Select.Item value="admin">Admin</Select.Item>
                  {/if}
                </Select.Content>
              </Select.Root>
        ...
        <Card.Content class="space-y-8">
          <div class="flex flex-col gap-4">
            {#if allUsers.length === 0}
              <p class="text-muted-foreground font-medium">No guest accounts have been created yet.</p>
            {:else}
                    ...
                    <Card.Content>
                      {#if !isOwner}
                        <div class="grid grid-cols-2 gap-4">
                          <Button onclick={() => { if (user) editUserModal = { visible: true, user: u }; }}>Edit</Button>
                          <Button variant="destructive" onclick={() => { if (user) deleteAccountModalConfirm = { visible: true, user: u }; }}>Delete</Button>
                        </div>
                      {/if}
                    </Card.Content>
                  </Card.Root>
