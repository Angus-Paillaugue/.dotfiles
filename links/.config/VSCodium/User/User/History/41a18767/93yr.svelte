                        ...
                        <Card.Header class="p-6">
                          <div class="flex flex-row items-center justify-between">
                            <div class="flex flex-col gap-2">
                              <Card.Title>{u.username} {user?.id === u.id ? '(You)' : ''}</Card.Title>
                              <Card.Description>{u.role}</Card.Description>
                            </div>
                            {#if user?.role !== ROLES.OWNER && hasAtLeastOnePermission(user?.role, PERMISSIONS.UPDATE_OTHER_ACCOUNTS, PERMISSIONS.DELETE_OTHER_ACCOUNTS)}
                              <DropdownMenu.Root>
