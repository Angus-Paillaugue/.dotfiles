<script>
  ...
  const allowedElements = "a:not([disabled]), button:not([disabled])";
  const offset = {
    x:8,
    y:4
  };
  ...
  // Update cursor position after navigation
  afterNavigate(() => {
...
<div class="rounded-full fixed z-50 pointer-events-none transition-colors ring-black ring-0 backdrop-grayscale backdrop-invert max-lg:hidden" style="top: {$extCirclePos.y}px; left:{$extCirclePos.x}px; width: {$extCirclePos.width}px; height: {$extCirclePos.height}px;" bind:this={cursor}></div>
