...
<div class="rounded-full fixed z-50 pointer-events-none transition-colors backdrop-grayscale backdrop-invert max-lg:hidden" style="top: {$extCirclePos.y}px; left:{$extCirclePos.x}px; width: {$extCirclePos.width}px; height: {$extCirclePos.height}px;" bind:this={cursor}></div>
<div class="rounded-full fixed z-50 pointer-events-none transition-colors size-2 backdrop-invert max-lg:hidden" style="top: {$interiorCirclePos.y}px; left:{$interiorCirclePos.x}px;" ></div>
