	...
	<div class="w-full bg-primary/10 group/section md:rounded-xl">
		...
		>
			...
			<div class="flex flex-col grow" id="contactLinks">
				...
				{/each}
				<div id="contactLinksTooltip" style="opacity: {$tooltipPos.opacity}; left: {$tooltipPos.x}px; top: {$tooltipPos.y}px;" class="p-4 z-50 mb-4 rounded-br-none rounded-xl transition-opacity fixed bg-primary text-white -translate-y-full -translate-x-full">test test</div>
			</div>
		</div>
	</div>
</section>
