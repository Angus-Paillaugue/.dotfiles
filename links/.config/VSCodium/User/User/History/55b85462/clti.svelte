<script>
	...
	function handleMouseMove(e) {
		...
		if (hovering) {
			...
			const username = hovering.getAttribute('data-username');
			const displayName = hovering.getAttribute('data-display-name');
			const img = hovering.getAttribute('data-img');
			tooltip.querySelector('img').src = img;
			tooltip.querySelector('h2').textContent = displayName;
			tooltip.querySelector('h3').textContent = username;
			...
			<div class="flex flex-col grow" id="contactLinks">
				{#each contacts as contact, i}
					<a
						...
						use:reveal
						data-img="https://media.licdn.com/dms/image/v2/D4D03AQGRrRCmpAJQaw/profile-displayphoto-shrink_200_200/profile-displayphoto-shrink_200_200/0/1725908782882?e=1731542400&v=beta&t=Acaee3zD8UuKEtZ5IuOuT-Qh7MIk1_6Ma8VZ8nH6wa8"
						data-username="angus-paillaugue"
						data-display-name="Angus Paillaugue"
					>
				...
				{/each}
				<div id="contactLinksTooltip" style="opacity: {$tooltipPos.opacity}; left: {$tooltipPos.x}px; top: {$tooltipPos.y}px;" class="p-4 z-50 mb-4 rounded-br-none rounded-xl transition-opacity fixed bg-white shadow text-black -translate-y-full -translate-x-full" aria-hidden="true">
					<div class="flex flex-row items-center">
						<img src="" alt="Profile picture" class="rounded-full size-24">
