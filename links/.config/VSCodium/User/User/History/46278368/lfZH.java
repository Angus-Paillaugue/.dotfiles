...
public class LocationDAOTests {
  ...
  // Initialiser les DAO
  locationDAO=new LocationDAO();this.location=new Location(this.locataire,"123 Rue des fleurs");
  ...
  @Test
    public void testFind() {
        // Arrange
        locationDAO.create(location, logement); // Créer une location pour le test

  @Test
  public void testFind() {
    // Arrange
    Location location = new Location("Cinema", "123 Rue des Fleurs");
    location = locationDAO.create(location); // Créer une location pour le test
    // Assert
    assertNotNull(location);
    assertEquals(location.getId(), result.getId());
    assertEquals("Cinema", result.getNom());
    assertEquals("123 Rue des Fleurs", result.getAdresse());
  }
