<script>
  ...
  let isSavingOutfit = $state(false);
  ...
  $effect(() => {
    if(form?.error) {
      ...
      toast.success({ message: form.message });
      outfit = form.outfit;
      selectedItems = outfit.elements.map((item) => item.id);
      editOutfitModalVisible = false;
    }
    form = null;
  });
</script>
...
{#if editOutfitModalVisible}
  ...
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-screen-md w-full z-40" transition:fly={{ y:'100%' }}>
    <Card>
      <h3>Edit outfit</h3>
      <Input bind:value={outfit.name} placeholder="Outfit name" />
      <!-- Display items -->
      ...
      <form action="?/save" method="POST"
        use:enhance={(e) => {
          ...
          e.formData.append('name', outfit.name);
          return async ({ update }) => {
      ...
      >
        <Button disabled={isSavingOutfit} loading={isSavingOutfit}>Save</Button>
      </form>
    </Card>
  </div>
{/if}
