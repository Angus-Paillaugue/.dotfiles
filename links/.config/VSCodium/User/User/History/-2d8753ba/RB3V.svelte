<script>
  ...
  $effect(() => {
    if(!editOutfitModalVisible || items.length > 0) return;
    fetchItems(1);
  });
</script>
  ...
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-screen-md w-full z-40" transition:fly={{ y:'100%' }}>
    <Card>
      ...
      {#if items?.totalPages > 1}
        <div class="shrink-0 flex flex-row items-center justify-center mx-auto w-fit gap-px rounded-xl overflow-hidden mt-4">
