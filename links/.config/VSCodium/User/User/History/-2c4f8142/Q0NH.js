...
export const exercises = mysqlTable('exercises', {
	...
	learning_path_id: int('learning_path_id').references(() => learning_path.id, {
		onDelete: 'restrict',
		onUpdate: 'restrict'
	}),
...
export const languages = mysqlTable('languages', {
	...
	created_at: timestamp('created_at', { mode: 'string' }).default(timestamp().defaultNow().notNull())
});

export const learning_path = mysqlTable('learning_paths', {
	id: int('id').autoincrement().primaryKey().notNull(),
	name: varchar('name', { length: 50 }).notNull(),
	description: text('description').notNull()
});
...
export const exercise_tests_relations = relations(exercise_tests, ({ one }) => ({
...
export const exercise_tests_relations = relations(exercises, ({ one, many }) => ({
	exercise_tests: many(exercise_tests),
	language: one(languages, {
		fields: [exercises.languageId],
		references: [languages.id]
	}),
	learning_path: one(learning_path, {
...
export const learning_paths_relations = relations(learning_path, ({ many }) => ({
	exercises: many(exercises)
}));
