<script lang="ts">
  ...
  let toRef = $state();
  let curvature = 0;
  let reverse = false; // Include the reverse pro;
  let duration = Math.random() * 3 + 4;
  let delay = 0;
  let pathColor = "stroke-neutral-600 dark:stroke-neutral-400";
  let pathWidth = 2;
  let pathOpacity = 0.2;
  let gradientStartColor = "#ffaa40";
  let gradientStopColor = "#9c40ff";
  let startXOffset = 0;
  let startYOffset = 0;
  let endXOffset = 0;
  let endYOffset = 0;
