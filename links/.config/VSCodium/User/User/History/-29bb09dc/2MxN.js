...
export function getImageUrl(item) {
	if (!item.collectionId || !item.id || !item.image) return null;
	return `${dev ? PUBLIC_DEV_POCKETBASE_HOST_URL : PUBLIC_POCKETBASE_HOST_URL}/api/files/${item.collectionId}/${item.id}/${item.image}`;
}

/**
 * Retrieves unique values from an array of clothing items.
 * @param {Array} items - The array of clothing items.
 * @returns {Object} An object containing unique values for each property in the clothing items.
 */
export function getUniqueValues(items = []) {
		...
		for (const key in item) {
			if (Array.isArray(item[key])) {
				...
				item[key].forEach((value) => {
					if (!filterValues[key]?.includes(value)) filterValues[key].push(value);
				});
			} else {
	...
	for (const color of colors) {
		if (!filterValues['color']?.includes(color)) {
			if (!filterValues['color']) filterValues['color'] = [];
			filterValues['color']?.push(color);
		}
	}
	for (const t of type) {
		if (!filterValues['type']?.has(t)) {
			if (!filterValues['type']) filterValues['type'] = new Set();
			filterValues['type']?.add(t);
		}
	}
	// Convert Sets to Arrays
	for (const key in filterValues) {
		filterValues[key] = Array.from(filterValues[key]).sort();
	}
