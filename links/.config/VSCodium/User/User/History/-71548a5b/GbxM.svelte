<script>
  ...
  function handleLanguageFiltering(e) {
    const lang = e.target.value;
    if (lang === 'all') {
      displayedCategories = allCategories;
    } else {
      displayedCategories = allCategories.filter((c) => c.lang === lang);
    }
  }
...
{#if visible}
	...
	>
      ...
      <div class="grow flex overflow-y-hidden w-full flex-col p-6 pt-0">
        <select name="" id="" class="ml-auto mr-4 mb-4 bg-white text-text-body text-sm rounded-xl block p-2.5" onchange={handleLanguageFiltering} bind:value={languageFilterValue}>
