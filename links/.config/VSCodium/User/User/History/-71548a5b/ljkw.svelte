...
{#if visible}
	...
	>
      ...
      <div class="grow flex overflow-y-hidden w-full flex-col p-6 pt-0">
        ...
        <!-- Save button -->
        <button class="flex flex-row items-center justify-center text-lg font-medium gap-2 w-full px-4 py-2 bg-neutral-800 rounded-full text-text-heading-dark disabled:bg-neutral-600 disabled:cursor-not-allowed transition-all mt-6" onclick={saveCategories}>
          {#if !hasUnsavedChanges}
            Close
          {:else}
