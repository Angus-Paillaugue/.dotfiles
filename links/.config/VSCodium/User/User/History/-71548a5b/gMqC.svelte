<script>
  ...
  $effect(() => {
    hasUnsavedChanges = categories.some((c) => !initialCategories.some((ic) => ic.id === c.id));
  });
  ...
  console.log(allCategories.map(c => c.lang));
...
{#if visible}
	...
	>
      ...
      <div class="p-4 md:p-6 flex w-full flex-row justify-start gap-8 items-center">
        <!-- Close button -->
        <button
          onclick={() => {if(hasUnsavedChanges) {confirmCloseModalVisible = true} else {visible = false}}}
      ...
      <div class="grow flex overflow-y-hidden w-full flex-col p-6 pt-0">
        <select name="" id="" class="ml-auto mr-4 mb-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
              ...
              <div class="w-full relative flex flex-row ietms-center">
                ...
                <div class="flex flex-row items-center grow gap-[63px] z-10">
                  ...
                  <div class="bg-white grow rounded-r-full flex flex-row items-center justify-between pr-4 h-full">
                    ...
                    <div class="bg-neutral-800 px-3 py-0.5 text-center text-xl font-bold rounded-full">
                      {category.lang}
                    </div>
                  </div>
                </div>
        ...
        <!-- Save button -->
        <button class="flex flex-row items-center justify-center text-lg font-medium gap-2 w-full px-4 py-2 bg-neutral-800 rounded-full text-text-heading-dark disabled:bg-neutral-600 disabled:cursor-not-allowed transition-all mt-6" onclick={saveCategories} disabled={!hasUnsavedChanges}>
