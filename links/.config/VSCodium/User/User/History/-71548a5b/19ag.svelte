...
{#if visible}
	...
	>
      ...
      <div class="grow flex overflow-y-hidden w-full flex-col p-6 pt-0">
        ...
        <!-- Save button -->
        <button class="flex flex-row items-center justify-center text-lg font-medium gap-2 w-full h-12 bg-neutral-800 rounded-full text-text-heading-dark disabled:bg-neutral-600 disabled:cursor-not-allowed transition-all mt-6 relative" onclick={saveCategories}>
          {#if !hasUnsavedChanges}
            <div transition:fly={{ y:'100%', easing:backOut }} class="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
              Close
            </div>
          {:else}
            <div transition:fly={{ y:'100%' }} class="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
