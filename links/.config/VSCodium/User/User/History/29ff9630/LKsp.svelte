<script>
	...
	async function fetchContents(url) {
		...
		if (contentContainer) {
			const excludeSelectors = provider.articleContentExcludeSelector.split('||').map((s) => `:not(${s})`);
			const selector = 'p h1 h2 h3 h4 h5 h6 img'.split(' ').map((s) => excludeSelectors.map(es => `${s}${es}`)).join(', ');
