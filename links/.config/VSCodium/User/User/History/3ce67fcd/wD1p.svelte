...
{#if latestRunnedTestsResults && latestRunnedTestsResults?.consoleOutput?.length > 0}
	...
	>
		...
		>
			...
			<Tooltip content={consoleVisible ? m.app_exercise_console_hide() : m.app_exercise_console_show()} position="left">
				<button
					aria-label={consoleVisible ? m.app_exercise_console_hide() : m.app_exercise_console_show()}
