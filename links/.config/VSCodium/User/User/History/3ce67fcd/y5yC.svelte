	...
	>
		...
		{#if latestRunnedTestsResults && latestRunnedTestsResults?.averageRamUsage}
			<div class="flex flex-row gap-2">
				<Tooltip class="flex flex-row items-center gap-2" content={m.app_exercise_test_case_panel_ram_usage()}>
			...
			{#each exercise.tests as _test, index}
				...
				>
					{#if latestRunnedTestsResults}
						...
						></div>
					{/if}
					{m.app_exercise_test_case_panel_case_number({ case:index + 1})}
				</Button>
			{/each}
		</div>
		{#if latestRunnedTestsResults}
			...
			>
				{latestRunnedTestsResults.results[selectedTestIndex].passed ? m.app_exercise_test_case_panel_accepted() : m.app_exercise_test_case_panel_wrong_answer()}
			</h2>
...
{#if latestRunnedTestsResults && latestRunnedTestsResults?.consoleOutput?.length > 0}
	...
	>
		...
		>
			...
			<Tooltip content={consoleVisible ? m.app_exercise_console_hide() : 'Show console'} position="left">
				<button
					aria-label={consoleVisible ? m.app_exercise_console_hide() : 'Show console'}
					title={consoleVisible ? m.app_exercise_console_hide() : 'Show console'}
