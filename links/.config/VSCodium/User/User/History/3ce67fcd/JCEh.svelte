...
<div class="flex grow flex-col overflow-hidden rounded-xl lg:mt-2 lg:shrink-0">
	<div
		class="flex h-10 shrink-0 flex-row flex-nowrap items-center justify-between overflow-x-auto bg-card p-1 px-2"
	>
	...
	<div class="flex flex-col bg-card space-y-4 p-4 max-lg:grow">
		...
		{#if latestRunnedTestsResults}
			...
			{#if latestRunnedTestsResults.results[selectedTestIndex]?.error}
				<div transition:slide={{ axis: 'y' }} class="mt-2 rounded-xl bg-red-600 p-2 text-neutral-100">
