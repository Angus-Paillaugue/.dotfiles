...
public class ColocationDAO implements ColocationDAODef {
  ...
  public Colocation findByLogement(Logement logement) {
    try {
      ...
      String query = "SELECT * FROM Colocation WHERE idLogement = ? LIMIT 1";
  ...
  public void addColocataire(Colocation c, Logement l, Personne p) {
    if (c.getContrats() != null && c.getContrats().size() > 0) {
      c.addColocataire(p, null, null, null, 0);
      this.create(c, l);
      return;
    }
  }

  public void removeColocataire(Colocation c, Personne p) {}
