...
public class ColocationDAO implements ColocationDAODef {
  ...
  public void create(Location l, Logement logement) {
    try {
      Connector db = Connector.getInstance();
      String query = "INSERT INTO Colocation (dateContrat, caution, idLieuContrat, idLogement) VALUES (?, ?, ?, ?)";
  ...
  public Colocation delColoc(Colocation c, Personne p) {
	  c.delColoc(p);
    Logger.log(c.getContrats().stream().map(loc -> loc.getLocataires()).collect(Collectors.toList()));
    List<Location> locations = c.getContrats().stream()
