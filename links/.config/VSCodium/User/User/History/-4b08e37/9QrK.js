...
export async function POST({ request, locals }) {
	...
	const { user } = locals;
	if (!user.admin) {
		return json({ message: m.error_messages_api_create_exercise_unauthorized() }, { status: 401 });
	}

	if (!title || !description || !difficulty || !tests || !content) {
		return json({ message: m.error_messages_api_create_exercise_missing_fields() }, { status: 400 });
	}
	...
	try {
		...
		return json({ message: m.error_messages_api_create_exercise_success(), id: newExerciseId, slug });
	} catch (error) {
