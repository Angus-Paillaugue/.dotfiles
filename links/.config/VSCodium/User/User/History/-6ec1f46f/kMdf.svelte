<script>
  ...
  async function fetchContents(url) {
    ...
    console.log(xmlDoc.querySelector('span.m-pub-dates__date:nth-child(2) > time:nth-child(1)'));
    ...
    const date = xmlDoc.querySelector('meta[property="article:published_time"]').getAttribute('content');
