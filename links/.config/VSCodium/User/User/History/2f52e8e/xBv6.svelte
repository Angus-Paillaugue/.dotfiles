  ...
  >
    ...
    >
      ...
      {#if homepage.version.showHomepageMessage}
        <a
          class="group relative mb-5 flex w-max items-center gap-x-2 rounded-full border border-main px-2 py-1 text-sm transition-colors hover:bg-neutral-600/10 focus-visible:bg-neutral-600/20"
    ...
    {#if $mode}
      <div
        class="overflow-hidden rounded-xl border border-main max-md:self-start max-sm:hidden"
        style="animation: float 5s ease-in-out infinite;"
      >
        <picture>
          <source
