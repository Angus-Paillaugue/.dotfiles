  ...
  return (tree) => {
    visit(tree, 'blockquote', (node) => {
      ...
      if (firstChild && firstChild.type === 'text') {
        const match = firstChild.value.match(alertRegex);
        if (match) {
          const alertType = match[1].toLowerCase();
          const iconNode = getAlertIcon(alertType);

          // Remove the alert type from the text
          firstChild.value = firstChild.value.replace(alertRegex, '').trim();

          // Add the icon node at the beginning of the blockquote
          node.children.unshift(iconNode);

          // Add the "github-alert" class to the blockquote
          node.data = node.data || {};
          node.data.hProperties = node.data.hProperties || {};
          node.data.hProperties.class = (node.data.hProperties.class || '') + ' github-alert';
        }
      }
    });
  };
