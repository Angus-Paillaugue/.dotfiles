  ...
  return (tree) => {
    visit(tree, "blockquote", (node) => {
      ...
      if (children.length > 0 && children[0].type === 'paragraph') {
        ...
        let text = paragraph.children[0].value;
        let match = text.match(alertRegex);
        if (match) {
          let type = match[1].toLowerCase();
          let icon = getAlertIcon(type);
          paragraph.children[0] = icon;
          paragraph.children[1] = {
            type: "text",
            value: text.replace(alertRegex, "").trim(),
          };
        }
      }
    });
