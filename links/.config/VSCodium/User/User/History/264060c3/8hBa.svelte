<script lang="ts">
  ...
  const buttonVariants = tv({
    base: 'flex flex-row px-4 py-4 rounded-lg items-center gap-4 w-full bg-background border border-border',
    variants: {
      variant: {
        info: 'text-blue-600',
...
{#if $toasts.length > 0}
  <div class="fixed bottom-2 right-2 flex flex-col z-40 max-w-md w-full">
		{#each $toasts.slice(0, 4) as activeToast (activeToast.id)}
			<div class={cn(buttonVariants({ variant:activeToast.type }))} transition:fly={{ x: '100%' }}>
