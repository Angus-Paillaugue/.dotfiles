<script lang="ts">
	...
	function play(e: Event) {
		...
		console.log(target.closest('.track')?.classList.contains('disabled'), target.closest('.track')?.classList)
		if(target.closest('button') || target.closest('.track')?.classList.contains('disabled')) return;
...
{#if format === 'list'}
	...
	<div
		onkeydown={play}
		class={cn(
			'text-text track flex w-full flex-row items-center justify-start gap-4 overflow-hidden rounded-lg p-2 transition-colors cursor-pointer shrink-0 track',
