	...
	>
		...
		<div class="flex flex-col text-start grow">
			...
			<p class="text-sm text-muted">
				<a href="/artist/{song.artist.id}">{song.artist.name}</a> - {formatTime(song.duration)}</p>
		</div>
...
{:else}
	...
	>
		...
		<div class="flex flex-col gap-1">
			<h3 class="text-base font-medium">{song.title}</h3>
			<a href="/artist/{song.artist.id}"><p class="text-sm text-muted line-clamp-2">{song.artist.name}</p></a>
		</div>
	</div>
{/if}
