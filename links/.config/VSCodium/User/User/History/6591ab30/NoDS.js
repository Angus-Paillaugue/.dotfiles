...
export const load = async ({ params, locals }) => {
	...
	if (identifier.id) {
        ...
        SELECT
            ...
            (SELECT
                JSON_ARRAYAGG(
                    JSON_OBJECT('input', et.input, 'expected_output', et.expected_output, 'displayed_value', et.displayed_value)
                )
        ...
        WHERE
            e.id = ?
        GROUP BY
            e.id, e.title, e.description, e.content, e.difficulty, e.created_at, e.slug;
    `,
