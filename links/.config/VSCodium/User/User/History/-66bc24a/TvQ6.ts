...
export async function getArtistMetadataFromTheWeb(name: string) {
	...
	try {
		...
		// Scrape the artist's description (usually in the first paragraph)
		const description = await page.$eval('p', (p) => p?.textContent?.trim());
