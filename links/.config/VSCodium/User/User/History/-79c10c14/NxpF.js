...
(() => {
  ...
  chrome.runtime.sendMessage({ message:'getBookmarks' }, (res) => {
    ...
    const bookmarkElements = res.map((bookmark) => {
      ...
      bookmarkElement.innerHTML = `<i class="ri-${bookmark.icon}-line text-6xl"></i> <span class="absolute -bottom-14 h-14 left-0 right-0 text-center transition-all group-hover:bottom-0 py-2">${bookmark.title}</span>`;
