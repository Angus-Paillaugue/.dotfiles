...
(() => {
  ...
  chrome.runtime.sendMessage({ message:'getBookmarks' }, (res) => {
    ...
    const bookmarkElements = res.map((bookmark) => {
      ...
      bookmarkElement.classList.add(...'rounded-lg bg-gruvboxDark-bg1 text-gruvboxLight-100 p-2 text-4xl text-center flex flex-col items-center justify-center gap-6 aspect-square relative group'.split(' '));
      bookmarkElement.innerHTML = `<i class="ri-${bookmark.icon}-line text-6xl"></i> <span class="absolute bottom-0 left-0 right-0 text-center transition-all group-hover:">${bookmark.title}</span>`;
