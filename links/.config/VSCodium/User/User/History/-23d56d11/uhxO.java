...
public class BienLouableDAO implements GenericDAO<BienLouable> {
  ...
  public int create(BienLouable b) {
    try {
      ...
      int idBienLouable =  db.executeUpdate(query, Arrays.asList(b.getAdresse().getId(), b.getRefCadastrale(), b.isBati(),
          b.getNumeroFiscal(), b.getTaxeFonciere(), loyerId, chargesId));

      query = "INSERT INTO Logement (id, typePorte, datePermisConstruction, dateConstruction, constatAmiante, constatPlomb, constatElectricite, constatEnergetique, emissionGazEffetDeSerre, referenceLogement, meuble, equipementAccesTechnologie) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      int idLogement = db.executeUpdate(query, Arrays.asList(
          idBienLouable,
  ...
  public BienLouable read(int id) {
    try {
      ...
      Map<String, Object> bien = db.getFirst(query, Arrays.asList(id));
      if (bien == null) {
        return null;
      }
