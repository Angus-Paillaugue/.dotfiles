  ...
  <div class="flex flex-row col-[content] gap-8 relative transition-transform duration-300 ease-in-out mt-20" bind:this={projectsCarousel}>
    {#each projects as project}
      <div class="aspect-square group rounded-xl relative w-full md:w-1/3 shrink-0 group/card overflow-hidden {project.bgColor}">
        ...
        <div class="absolute inset-4 bg-center bg-no-repeat bg-contain" style="background-image: url({project.image});"></div>
        <div class="absolute flex flex-col bottom-0 left-0 right-0 transition-all p-4 {project.bgColor}">
          ...
          <div class="h-0 overflow-hidden flex flex-col justify-between group-hover/card:h-full duration-300 ease-in transition-all {project.bgColor}">
