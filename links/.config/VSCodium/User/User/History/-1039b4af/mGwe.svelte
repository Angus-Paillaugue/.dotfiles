...
<section id="projects" class="max-w-screen-xl mx-auto p-2 lg:p-4">
  ...
  <div class="flex flex-row col-[content] gap-8 relative transition-transform duration-300 ease-in-out mt-20" bind:this={projectsCarousel}>
    {#each projects as project}
      <div class="aspect-square group rounded-xl relative flex flex-col w-full md:w-1/3 shrink-0 group/card overflow-hidden bg-center bg-no-repeat bg-contain {project.bgColor}" style="background-image: url({project.image});">
        <p class="absolute top-4 right-4 text-neutral-200 bg-white/10 text-sm px-2 py-1 rounded-full">{project.date}</p>
        <div class="p-4 mt-auto {project.bgColor}">
        ...
        <div class="h-0 px-4 overflow-hidden flex flex-col justify-between group-hover/card:grow duration-300 ease-in transition-all {project.bgColor}">
