...
import { getAllUsers, deleteUser } from '$lib/server/db/user';
...
export const actions: Actions = {
  ...
  async editUserAccount({ request, locals }) {
    ...
    try {
      // Edit user
      await updateUsername(parseInt(userId), username);
      await updateRole(parseInt(userId), role);
      ...
      console.error(error);
      return fail(400, { error: true, message: 'An error occurred!', action: 'editUserAccount' });
    }
  }
};
