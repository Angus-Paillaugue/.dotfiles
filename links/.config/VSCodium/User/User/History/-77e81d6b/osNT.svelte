...
<section class="max-w-screen-xl mx-auto p-4" id="about">
	...
	<ol role="list" class='list-none mt-4'>
		{#each experience[$locale ?? 'en'] as job, index}
			<li
				class={cn('relative m-0 pl-10 pt-1 before:absolute before:left-0 before:top-1.5 before:flex before:size-5 before:flex-col before:items-center before:justify-center before:rounded-full before:bg-primary before:hover:scale-125 before:transition-transform w-fit', index < experience[$locale].length - 1 &&'after:absolute after:bottom-0 after:left-[9px] after:top-8 after:w-0 after:border-l after:border-neutral-300 pb-8')}
				use:reveal
			>
				<h3>{job.name}</h3>
