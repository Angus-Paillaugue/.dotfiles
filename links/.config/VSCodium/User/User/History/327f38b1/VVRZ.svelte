...
<main class="mx-auto w-full max-w-screen-lg space-y-4 md:space-y-8">
	...
	<div class="grid w-full grid-cols-1 gap-4 md:grid-cols-3">
		<Card class="user-info-card md:col-span-2">
			<div class="flex flex-row gap-4">
				<img src={user.profile_picture} class="size-20 rounded-xl object-cover" alt={m.app_account_page_profile_picture_alt()} />
		...
		<Card>
			<div class="flex flex-col space-y-1.5">
				<p class="text-lg font-semibold leading-none tracking-tight">{m.app_account_page_solved_problems()}</p>
	...
	<!-- Contribution grid -->
	<Card class="w-full">
		<h3 class="text-lg font-semibold leading-none tracking-tight">{m.app_account_page_contributions()}</h3>
					...
					<div class="flex shrink-0 flex-col gap-1">
						{#each { length: 7 } as _, dayNo}
							...
							{#if thisDay <= today}
								...
								>
									<button
										aria-label={m.app_account_page_contributions_grid_activity_for({ date:formattedDate})}
										title={m.app_account_page_contributions_grid_activity_for({ date:formattedDate})}
	...
	<Card class="w-full">
		<div class="flex flex-col space-y-1.5">
			<h3 class="text-lg font-semibold leading-none tracking-tight">{m.app_account_page_activity_title()}</h3>
			...
			{:else}
				<table class="w-full table-auto text-sm">
					<thead class="sticky top-0 bg-neutral-800"
						><tr
							><th class="h-12 px-4 text-left align-middle font-medium text-neutral-400">{m.app_account_page_activity_table_col_name()}</th>
							<th class="h-12 px-4 text-right align-middle font-medium text-neutral-400">{m.app_account_page_activity_table_col_date()}</th
							></tr
						></thead
					>
