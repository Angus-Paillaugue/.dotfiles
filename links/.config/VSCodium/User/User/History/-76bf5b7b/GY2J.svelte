...
<main class="mx-auto w-full max-w-screen-lg space-y-4 md:space-y-8">
	...
	<Card class="flex flex-row gap-4 space-y-0">
		...
		>
		<h1 class="text-3xl font-bold">{app_account_settings_title()}</h1>
	</Card>
	...
	<Card>
		...
		>
			<h2 class="text-2xl font-semibold">{m.app_account_settings_change_passwords_title()}</h2>
			<Input
				id="oldPassword"
			...
			<Button class="mt-4" loading={isChangingPassword} variant="primary">{m.app_account_settings_change_password_inputs_change_button()}</Button>
		</form>
	</Card>
...
{#if confirmAccountDeletion}
	...
	>
		<Card class="space-y-4">
			<h2 class="text-2xl font-semibold">{m.app_account_settings_delete_account_confirm_modal_title()}</h2>
