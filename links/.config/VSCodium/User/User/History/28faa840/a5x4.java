  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (target.getName().equals("saveButton")) {
      ...
      try {
        ...
        locataire = personneDAO.create(locataire, adr.getId());
        ...
        if (b.isLocation()) {
          if (b.getLocation() != null && b.getLocation().getLocataires().size() == 1) {
            if (b.getLocation().getLocataires().get(0).getSituationFamiliale()
              ...
              new ModalDanger(
                      "Erreur",
                      "Le logement est déja occupé par une personne mariée, vous pouvez uniquement ajouter une autre personne mariée",
                      true)
                  .setVisible(true);
            }
          } else {
          ...
          Location location =
              new Location(
                  ...
                  adr,
                  VuesUtils.toFloat(VuesUtils.getInputValue(vue.panel, "caution")));
          b.getColocation().addColoc(location);
          ColocationDAO colocationDAO = new ColocationDAO();
          colocationDAO.create(location, b);
          new MesAppartements().setVisible(true);
