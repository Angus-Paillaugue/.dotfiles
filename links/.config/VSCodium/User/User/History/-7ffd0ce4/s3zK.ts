...
export async function getConversations(userId: number): Promise<Conversation[]> {
  ...
  Promise.all(conversations.map(async (conversation: Conversation) => {
...
export async function createMessage(data: BuildingMessage, idConversation: number): Promise<Message> {
  const sql = `
  INSERT INTO message (conversationId, content, fromUser)
  VALUES (?, ?, ?)
  `;
  const msg = typeof data.text === 'string' ? data.text : data.text.message;
  const result = await DB.query(sql, [idConversation, msg, data.from]);
  return { ...data, id: result.insertId };
}
