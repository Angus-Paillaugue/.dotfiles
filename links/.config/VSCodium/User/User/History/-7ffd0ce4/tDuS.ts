...
export async function deleteMessage(idMessage: number, userId: number): Promise<void> {
  const sql = `
  DELETE FROM message
  WHERE id = ?
  `;
...
async function deleteConversation(idConversation: number,): Promise<void> {
  ...
  Promise.all(conversation.messages.map((message) => deleteMessage(message.id, userId)));
