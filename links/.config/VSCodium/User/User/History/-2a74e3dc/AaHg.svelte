<script>
  ...
  console.log(Array.isArray(page.name) ? pages[0].name.find(l => l ===localeConfig.locale) : '', localeConfig.locale);
      ...
      {:else}
        ...
        >
          {#if page.icon}
            ...
            />
          {/if}
          {i18n.enabled ? Array.isArray(page.name) ? page.name[localeConfig.locale] ?? page.name[i18n.defaultLocale] : "String" : page.name}
        </a>
      {/if}
    {/each}
  </div>
