      ...
      {:else}
        ...
        >
          {#if page.icon}
            ...
            />
          {/if}
          {i18n.enabled ? Array.isArray(page.name) ? JSON.stringify(page.name.find((l) => Object.keys(l)[0] === localeConfig.locale)) ?? page.name[i18n.defaultLocale] : "String" : page.name}
