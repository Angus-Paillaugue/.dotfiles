<script>
	...
	const showTooltip = () => {
		...
		// Get the coordinates of the tooltip relative to the viewport
	...
	const getStyles = () => {
		...
		const top = tooltipCoords.y - scrollTop;
		const left = tooltipCoords.x - scrollLeft;

		if (position === 'top') {
			return `top: 100%; left: ${left + tooltipCoords.width / 2}px;`;
