...
<div class="flex min-h-screen w-full flex-col mx-auto max-w-screen-lg">
  {#key data?.name}
    <Navbar title={data?.name ?? 'Docs'} />

   <div class="mx-auto flex h-full w-full grow flex-col-reverse justify-center max-lg:items-center lg:gap-8 lg:flex-row">
      <Sidebar open={sidebarOpen} style={sidebarStyle} />
      <main class="flex h-full w-full grow flex-col p-2 lg:shrink-0">
      ...
      {#if headings.length > 0}
        ...
        <!-- Toc -->
        <div class="text-nowrap lg:px-2 lg:py-4 lg:sticky lg:top-24 lg:h-fit lg:w-[250px] overflow-auto max-lg:fixed max-lg:inset-0 max-lg:bg-white max-lg:dark:bg-neutral-900 max-lg:flex max-lg:flex-col max-lg:items-center max-lg:justify-center transition-transform max-lg:z-10 lg:shrink-0 {mobileTocVisible ? 'max-lg:translate-x-0' : "max-lg:-translate-x-full"}" id="toc-container">
