...
export function remarkExtractHeaders() {
  return (tree, file) => {
    ...
    visit(tree, 'heading', (node) => {
      ...
      if (node.depth <= 1) {
      ...
      } else if (node.depth === 2 && currentHeader) {
        // Check if the heading is an h3 and there is a current header
        const childId = generateId(headerText, headerCounter); // Generate a unique ID for the h3 header
        ...
        // If the heading is an h3 and there is no current header, add it as a top-level header
        const headerText = getHeaderText(node);
        const childId = generateId(headerText, headerCounter); // Generate a unique ID for the header
        ...
        file.data.headers.push(currentHeader); // Add the current header to the headers array
        addIdToNode(node, childId); // Add the ID to the heading node
      }
    });
