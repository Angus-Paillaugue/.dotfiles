  ...
  return (tree, file) => {
    ...
    visit(tree, 'heading', (node) => {
      ...
      if (node.depth <= 1) {
        ...
        currentHeader.children.push(newHeader); // Add the h32header to the children array of the h1 header

        addIdToNode(node, childId); // Add the ID to the h2 node

        // Update the current header to the h2 header
