<?php
  ...
  class Button {
    ...
    public function __construct($params = [])
    {
      ...
      $variantClasses = implode(" ", array_map(fn($val) => $variantClasses[$val], explode(" ", $variant)));
      $this->component = "<button type='submit' class='cursor-pointer transition-all duration-200 ease-in-out px-4 py-2 font-medium rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex flex-row gap-2 items-center $variantClasses $class' $disabled type='$type'>$label</button>";
    }
    ...
    public function __construct($params = [])
    {
      ...
      $this->component = "
        <div class='block w-full'>
          $label
          <input type='$type' id='$id' name='$id' class='$class' placeholder='$placeholder'>
        </div>
      ";
    }
