...
export async function updateDashboard(id: string, userId: number, data: Dashboard): Promise<void> {
  ...
  AND userId = ?
  `;
  await DB.query(dashboardSql, [data.title, data.description, id, userId]);
  ...
  await Promise.all(
    cards.map(async (card) => {
      const cardSql = `
      UPDATE card(title,type,request,colSpan,rank)
      SET (?,?,?,?,?)
      WHERE card.id = ?
      `;
