...
export async function updateDashboard(data: Dashboard, userId: number): Promise<Dashboard> {
  ...
  await Promise.all(
    cards.map(async (card) => {
      ...
      `;
      await DB.query(cardSql, [card.id, card.title, card.type, card.request, card.colSpan, card.rank, data.id, card.title, card.type, card.request, card.colSpan, card.rank]);
    })
  );
