...
export async function updateDashboard(data: Dashboard, userId: number): Promise<Dashboard> {
  ...
  await Promise.all(
    cards.map(async (card) => {
      const cardSql = `
      INSERT INTO card (id, title, type, request, colSpan, \`rank\`)
      VALUES(?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
      title = ?, type = ?, request = ?, colSpan = ?, \`rank\` = ?
      `;
      await DB.query(cardSql, [card.id, card.title, card.type, card.request, card.colSpan, card.rank, card.title, card.type, card.request, card.colSpan, card.rank]);
    })
  );
