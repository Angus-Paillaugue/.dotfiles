...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const [numberOfPages] = await db.query('SELECT COUNT(*) count FROM logs WHERE message LIKE CONCAT("%", ?, "%")', [query]);
    console.log(numberOfPages[0].count);
    return json({
      success: true,
      message: 'Query ran successfully',
      results,
      numberOfPages: numberOfPages[0].count
    });
  } catch (error: unknown) {
