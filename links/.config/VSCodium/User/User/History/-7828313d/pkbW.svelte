...
{#snippet copyCodeButton(absolute = true, snippet = false)}
  {#if copyCode !== 'false'}
    <button
      ...
      tabindex="0"
      class={cn(
        'flex items-center justify-center rounded-full p-2 text-neutral-6002',
        absolute
...
{:else}
  ...
  >
    {#if name}
      <div
        class="codeBlockName relative flex w-full flex-row items-center rounded-t-md border border-b-0 px-3 py-1.5"
      >
        <p
