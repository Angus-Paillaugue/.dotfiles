...
{#snippet copyCodeButton(classes)}
  {#if copyCode !== 'false'}
    <button
      ...
      tabindex="0"
      class={cn(
        'flex items-center justify-center rounded-full p-2 text-neutral-600', classes
      )}
      name="Copy code"
...
{#if snippet == 'true'}
  <div class="snippet" bind:this={codeContainer}>
    {@render children()}
    <div class="shrink-0 rounded-none bg-neutral-50 flex flex-col items-center justify-center">
