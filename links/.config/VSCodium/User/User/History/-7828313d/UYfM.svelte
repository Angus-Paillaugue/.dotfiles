...
{#snippet copyCodeButton(absolute = true, snippet = false)}
  {#if copyCode !== 'false'}
    <button
      ...
      tabindex="0"
      class={cn(
        'flex items-center justify-center rounded-full p-2 text-neutral-600',
        absolute
          ? 'absolute bottom-1 right-5 top-4 aspect-square'
          : snippet ? 'shrink-0 rounded-none bg-neutral-50' : 'aspect-square h-full ml-auto',
      )}
      name="Copy code"
...
{#if snippet == 'true'}
  <div class="snippet" bind:this={codeContainer}>
    {@render children()}
    {@render copyCodeButton('shrink-0 rounded-none bg-neutral-50')}
  </div>
{:else}
  <div
    class={cn(
  ...
  >
    {#if name}
      <div
        class="codeBlockName relative flex w-full flex-row items-center rounded-t-xl border border-b-0 border-neutral-300/50 px-3 h-12"
      >
        <p
        ...
        {@render copyCodeButton('aspect-square h-full ml-auto')}
      </div>
    {/if}
    <div class={className}>
      {@render children()}
