...
{#snippet copyCodeButton(classes)}
  {#if copyCode !== 'false'}
    <button
      ...
      tabindex="0"
      class={cn(
        'flex items-center justify-center rounded-full text-neutral-600', classes
      )}
      name="Copy code"
...
{#if snippet == 'true'}
  <div class="snippet" bind:this={codeContainer}>
    {@render children()}
    <div class="shrink-0 rounded-none bg-neutral-50 flex flex-col items-center justify-center px-1">
      {@render copyCodeButton('p-2')}
    </div>
  </div>
{:else}
  <div
  ...
  >
    {#if name}
      ...
      >
        ...
        {@render copyCodeButton('aspect-square p-2 ml-auto')}
      </div>
    {/if}
    <div class={className}>
      {@render children()}
