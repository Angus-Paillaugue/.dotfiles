<script>
  ...
  const { children, class: className, name } = $props();
...
<div class="codeContainter my-4">
  {#if name}
    <p class="flex flex-row items-center gap-4 rounded-t-md bg-neutral-200 px-3 py-1 text-base font-medium text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200 m-0">
      {name}
    </p>
  {/if}
