    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if(!isset($username) || !isset($password)) {
        ...
        if(count($rows) > 0) {
          ...
          $conn->run_query("INSERT INTO users (username, password_hash) VALUES (?, );", $username, $hashed_password);
  ...
  <div class="max-w-xl mx-auto w-full p-4 prose">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="p-4 border space-y-4 flex flex-col rounded-xl w-full bg-white">
      ...
      <?php
        ...
        if($error) {
          $alert = new Alert(['text' => $error, 'variant' => 'danger']);
        ...
        echo $createAccountLink->construct();
      ?>
    </form>
  </div>
</body>
