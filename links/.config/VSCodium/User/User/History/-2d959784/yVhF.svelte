<script>
  ...
  function onResultsScroll(e) {
    ...
    if (e.target.scrollTop > 0) {
      searchContainer.classList.add('border-main');
    } else {
      searchContainer.classList.remove('border-main');
    }
  }
...
{#if $searchModalShown}
  ...
  <div class="fixed inset-0 z-50 p-4 sm:p-6 md:p-12">
    <div
      class="no-scrollbar mx-auto max-h-full w-full max-w-screen-md overflow-hidden overflow-y-auto rounded-2xl border border-main bg-neutral-100 dark:bg-neutral-950 dark:text-white"
      ...
      >
        <div class="relative">
          ...
          <input
            ...
            id="search"
            class={cn(
              'w-full rounded-xl border border-main bg-transparent py-4 pl-10 text-xl focus:outline-none dark:bg-transparent md:px-12',
