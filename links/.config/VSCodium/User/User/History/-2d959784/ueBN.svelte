        ...
        <div class="p-2" id="results">
          {#each results as result, index}
            <a
              href={result.url}
              class={cn(
                'group relative m-0 flex list-none flex-row items-center justify-between rounded-xl p-2 px-4 py-2 transition-colors hocus:bg-neutral-300/50 focus:outline-none dark:hocus:bg-neutral-800/50',
            ...
            >
              ...
              <Icon
                ...
                class="text-primary-400 opacity-0 transition-opacity group-hocus:opacity-100 group-focus:opacity-100 dark:text-primary-600 [.group.selected_&]:opacity-100"
              />
            </a>
          {/each}
        </div>
