      ...
      {#if results.length > 0}
        ...
        >
          {#each results as result, index}
            <a
              href={result.url}
              class="relative rounded-xl list-none transition-colors hover:bg-neutral-300/50 focus:bg-neutral-300/50 dark:focus:bg-neutral-800/50 dark:hover:bg-neutral-800/50 m-0 flex flex-row justify-between items-center p-2 px-4 py-2 group focus:outline-none {index === selectedIndex && "bg-neutral-300/50 dark:bg-neutral-800/50"}"
              id={`result-${index}`}
            ...
            >
              ...
              <Icon icon="material-symbols:arrow-forward-ios-rounded" class="transition-opacity opacity-0 group-hover:opacity-100 group-focus:opacity-100" />
            </a>
          {/each}
        </div>
      {/if}
