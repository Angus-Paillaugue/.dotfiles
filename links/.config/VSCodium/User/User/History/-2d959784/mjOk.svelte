  ...
  <div class="fixed z-50 inset-0 p-4 sm:p-6 md:p-12">
    ...
    >
      <div class="w-full sticky top-0 z-10 bg-neutral-100 dark:bg-neutral-950 p-2 border-b border-transparent transition-all" id="searchContainer">
        <div class="relative">
          <Icon icon="line-md:search" class="absolute left-3 top-1/2 size-4 md:size-6 -translate-y-1/2" />
          <input
            type="text"
            name="search"
            id="search"
            class="w-full bg-transparent pl-10 md:px-12 py-4 text-xl focus:outline-none dark:bg-transparent rounded-xl border border-neutral-300/50 dark:border-neutral-700/50 {results.length >
              0 && 'border-b'}"
        ...
        >
          {#each results as result, index}
            ...
            >
              ...
              <Icon icon="material-symbols:arrow-forward-ios-rounded" class="text-primary-400 dark:text-primary-600 transition-opacity opacity-0 group-hover:opacity-100 group-focus:opacity-100 [.group.selected_&]:opacity-100" />
            </a>
          {/each}
        </div>
      {/if}
