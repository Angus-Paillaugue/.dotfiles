...
public class GenererContractDeLocation extends JFrame {
  ...
  public GenererContractDeLocation() {
    ...
    int nbLocation = Math.min(logements.stream().filter(Logement::isLocation).collect(Collectors.toList()).size(), 1);
    int nbColocation = Math.min(logements.stream().filter(Logement::isColocation).collect(Collectors.toList()).size(), 1);
    System.out.println("nbLocation: " + nbLocation);
    System.out.println("nbColocation: " + nbColocation);
    ...
    for (int i = 0; i < logements.size(); i++) {
      if (logements.get(i).isLocation()) {
        addBien(logements.get(i), i, radioPanelLocation);
      } else {
        addBien(logements.get(i), i, radioPanelColocation);
      }
    }
  ...
  private void addBien(Logement bien, int index, JPanel panel) {
