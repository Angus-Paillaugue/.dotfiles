...
public class GenererContractDeLocation extends JFrame {
  ...
  private void addLocation(Location l, int index) {
    // Check that the property has a tenant
    if (bielLocation() && !bielColocation()) {
      JRadioButton radioButtonLocation = new JRadioButton(bieltAdresse().toString());
      group.add(radioButtonLocation);
      radioButtonLocation.setName(String.valueOf(index));
      radioPanelLocation.add(radioButtonLocation);
    }
    if (bielColocation() && !bielLocation()) {
      JRadioButton radioButtonColocation = new JRadioButton(bieltAdresse().toString());
