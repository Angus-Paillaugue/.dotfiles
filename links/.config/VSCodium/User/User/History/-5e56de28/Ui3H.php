...
class FeuilleMatchDAO {
  ...
  public function getForRencontre($id) {
    echo $id;
    $data = $this->conn->run_query('SELECT * FROM feuille_match WHERE id_rencontre = ?;', $id);
    $dataArray = array();

    foreach ($data as $feuilleMatch) {
      $feuilleMatch = new FeuilleMatch($feuilleMatch['id_rencontre'], $feuilleMatch['id_joueur'], $feuilleMatch['role_debut'], $feuilleMatch['role_fin'], $feuilleMatch['poste'], $feuilleMatch['evaluation']);
      $feuilleMatch->setId($feuilleMatch['id']);
      $joueur = (new JoueurDAO())->get($feuilleMatch->getIdJoueur());
      $feuilleMatch->setJoueur($joueur);
