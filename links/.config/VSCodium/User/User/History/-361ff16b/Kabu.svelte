  ...
  function onclick(e: MouseEvent) {
    ...
    if (e.target && (e.target as HTMLElement).closest('.log-row-checkbox')) {
      ...
      if (e.shiftKey) {
        ...
        const startIndex = logs.findIndex((log) => log.id === min);
        const endIndex = logs.findIndex((log) => log.id === max);

        console.log({ start, end, min, max, startIndex, endIndex });
