<script lang="ts">
  ...
  function onclick(e: MouseEvent) {
    ...
    if (e.target && (e.target as HTMLElement).closest('.log-row-checkbox')) {
      ...
      if (e.shiftKey) {
        const start = checkboxes.findIndex(
        ...
        if(start === end) {
          target.dataset.state = target.dataset.state === 'checked' ? 'checked' : 'unchecked';
        }
        if(start > end) {
          checkboxes
            .slice(Math.min(end, start), Math.max(end, start))
            .forEach((checkbox) => ((checkbox as HTMLInputElement).dataset.state = 'checked'));
        } else {
          checkboxes
