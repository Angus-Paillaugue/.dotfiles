<script lang="ts">
  ...
  function onclick(e: MouseEvent) {
    ...
    if (e.target && (e.target as HTMLElement).closest('.log-row-checkbox')) {
      ...
      if (e.shiftKey) {
        const start = selectedRows.length > 1 ? selectedRows[selectedRows.length - 2] : 0;
        const end = getValue(target);
        const [min, max] = [Math.min(start, end), Math.max(start, end)];
        console.log(min, max, selectedRows);
                    ...
                    <div class="flex size-full flex-row items-center">
                      <Checkbox
                        ...
                        onclick={(e) => {
                          ...
                          if (selectedRows.includes(log.id)) {
                            selectedRows = selectedRows.filter((id) => id !== log.id);
                          } else {
                            selectedRows = [...selectedRows, log.id];
                          }
                        }}
