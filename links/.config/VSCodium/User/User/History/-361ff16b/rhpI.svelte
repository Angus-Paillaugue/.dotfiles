<script lang="ts">
  ...
  const isChecked = (el: HTMLElement) => (el as HTMLInputElement).dataset.state === 'checked';
  const setChecked = (el: HTMLElement, state: 'checked' | 'unchecked') => {
    (el as HTMLInputElement).dataset.state = state;
    (el as HTMLInputElement).setAttribute('aria-checked', state === 'checked' ? 'true' : 'false');
  }
  const toggleChecked = (el: HTMLElement) =>
    setChecked(el, isChecked(el) ? 'unchecked' : 'checked');
  ...
  function onclick(e: MouseEvent) {
    ...
    if (e.target && (e.target as HTMLElement).closest('.log-row-checkbox')) {
      const target: HTMLInputElement | null = (e.target as HTMLElement).closest('.log-row-checkbox');
      if (!target) return;
      const checkboxes: HTMLElement[] = Array.from(document.querySelectorAll('.log-row-checkbox'));

      if (e.shiftKey) {
        const start = checkboxes.map((checkbox) => getValue(checkbox)).indexOf(selectedRows[0]);
        const end = checkboxes.findIndex((checkbox) => getValue(checkbox) === getValue(target));
        ...
        if(start > end) {
          checkboxes
            .slice(Math.min(end, start), Math.max(end, start))
            .forEach((checkbox) => setChecked(checkbox, 'checked'));
        } else {
          checkboxes
      ...
      const checked = checkboxes.filter(
        (checkbox) => (checkbox as HTMLInputElement).dataset.state === 'checked'
      );
      selectedRows = checked.map((checkbox) =>
        parseInt((checkbox as HTMLInputElement).dataset.value || '')
      );
    }
  }
</script>
