<script>
   ...
   * @property {boolean} open = true - Indicates if the tree is open or closed.
  ...
  {#each tree as element}
    {#if element?.children}
      ...
      <details open={element?.open ?? open}>
        ...
        <Tree tree={element.children} root={false} {open} class={className} {...restProps} />
      </details>
    {:else}
      <!-- Is a file -->
      <li
