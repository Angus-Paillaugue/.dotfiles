<script>
  ...
  import { getImageUrl, getUniqueValues } from '$lib/utils';
  ...
  let editItemBrandsSuggestions = $state([]);
	const uniqueValues = getUniqueValues();

	/**
	 * Function to debounce the editItemBrands function.
	 */
	function editItemBrandsDebounce() {
		if (editedItem.brand.length < 3) editItemBrandsSuggestions = [];
		else
			editItemBrandsSuggestions = uniqueValues.brand.filter((brand) =>
				brand.toLowerCase().startsWith(editedItem.brand.toLowerCase())
			);
	}
  ...
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-screen-md w-full z-40" transition:fly={{ y:'100%' }}>
    <Card>
      <form
        ...
        <Input label="Name" id="name" bind:value={item.name} />

        <Input tag="textarea" id="description" bind:value={item.description} label="Description" />
        ...
        <Input
          ...
          on:input={editItemBrandsDebounce}
        ></Input>
        {#if editItemBrandsSuggestions.length > 0}
          <div class="flex flex-row flex-wrap gap-4 mt-1">
            {#each editItemBrandsSuggestions as brandName}
              <button
                class="border rounded-2xl bg-neutral-900 text-sm px-2 cursor-pointer"
                onclick={() => {
                  editedItem.brand = brandName;
                  editItemBrandsSuggestions = [];
                }}>{brandName}</button
              >
            {/each}
          </div>
