<script>
  ...
  import { getImageUrl, findNearestColorName } from '$lib/utils';
  ...
  let editItemModalVisible = $state(false);
  ...
  $effect(() => {
		if (form?.error) toast.error({ message: form.error }); else if (form?.success) {
      toast.success({ message: form.success });
      resetItem(form.updatedItem)
  ...
  /**
	...
	async function handleImageUpload(removeBg = false) {
		...
		editedItem.color = [findNearestColorName(averageColor)];
    ...
    try {
      ...
      toast.error({ message: 'Failed to calculate the average color of the image' });
    }
	}

  async function getItem() {
    const res = await fetch(`/api/fetchItem?id=${itemId}`);
    if (!res.ok) {
      toast.error({ message: 'Failed to fetch item' });
      return;
    }
  ...
  <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-screen-md w-full z-40" transition:fly={{ y:'100%' }}>
    <Card>
      <form
        ...
        <Button type="submit" loading={isSaving} variant="primary">Save</Button>
      </form>
    </Card>
  </div>
{/if}
...
{#if !item}
  ...
  <Card>
    <!-- Image -->
    <div class="animate-pulse bg-neutral-800 aspect-square w-full rounded-lg"></div>
    <hr class="animate-pulse bg-neutral-800 h-px" />
    <!-- Title and category -->
    <div class="flex flex-row justify-between items-center">
      <!-- Title -->
      <div class="animate-pulse bg-neutral-800 h-6 w-7/12 rounded-xl"></div>
      <!-- Category -->
      <div class="w-1/12 bg-neutral-800 h-5 rounded-xl"></div>
    </div>
    <!-- Description -->
    <div class="flex flex-col gap-2">
      <div class="animate-pulse bg-neutral-800 h-3 w-2/5 rounded-xl"></div>
      <div class="animate-pulse bg-neutral-800 h-3 w-1/4 rounded-xl"></div>
      <div class="animate-pulse bg-neutral-800 h-3 w-1/2 rounded-xl"></div>
    </div>
    <!-- Details -->
    <div class="flex flex-row items-center gap-4">
      <!-- Color -->
      <div class="animate-pulse bg-neutral-800 size-[30px] rounded-full"></div>
      <!-- Brand -->
      <div class="animate-pulse bg-neutral-800 h-6 w-1/5 rounded-xl"></div>
      <!-- Link -->
      <div class="animate-pulse bg-neutral-800 h-6 w-1/5 rounded-xl"></div>
    </div>
  </Card>
{:else}
  <Card>
    <div class="relative">
      <img src={getImageUrl(item)} class="size-full object-contain aspect-square rounded-lg" alt="">
