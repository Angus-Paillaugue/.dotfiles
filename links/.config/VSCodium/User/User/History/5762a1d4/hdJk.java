  ...
  public DetailAppartement(Logement logement) {
    ...
    JButton supprimerLocataire =
        new Button("Supprimer un locataire", Constants.ICONS_PATH + "minus-16x16-heading.png")
            .build();
    supprimerLocataire.addActionListener(
        e -> {
          if (logement.isLocation()) {
            logement.getLocation().removeLocataires();
            LocationDAO locationDAO = new LocationDAO();
            locationDAO.supprimerLocataires(logement.getLocation());
            new DetailAppartement(logement).setVisible(true);
            this.dispose();
          } else if (logement.isColocation()) {
            new SupprimerLocataire(logement).setVisible(true);
            this.dispose();
          }
        });
    ...
    ajouterLocataire.addActionListener(
        e -> {
          new AjouterLocataire(logement).setVisible(true);
          this.dispose();
        });
    ...
    modifyButton.addActionListener(
        e -> {
          new EditAddress(logement);
        });

    // return Button
    JButton retourButton = new Button("Retour").build();

    retourButton.addActionListener(
        e -> {
          this.dispose();
