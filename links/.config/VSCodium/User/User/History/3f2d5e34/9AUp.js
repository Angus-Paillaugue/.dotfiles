...
/**
 * Retrieves the locale configuration based on the provided locale.
 *
 * @param {string} locale - The locale to retrieve the configuration for.
 * @returns {object} - The locale configuration object.
 */
export function getLocaleConfig(locale) {
  const configLocales = Object.keys(i18n.locales);
  let selectedLocale = configLocales.includes(locale) ? locale : null;
  if (!selectedLocale) {
    // If the full locale doesn't match, extract the language code
    const languageCode = locale.split('-')[0];

    // Check if the generic language code exists in the locales array
    selectedLocale = configLocales.includes(languageCode) ? languageCode : 'en';
  }

  return { locale: selectedLocale, ...i18n.locales[selectedLocale] };
}
