<script lang="ts">
  ...
  let toastId: string | null = $state(null);
  ...
  function webSocket(initial = true) {
    if(!initial) {
      if(!toastId) {
        ...
        toastId =toast.info('Connecting to the server...', { id: toastId });
      }
    }
    ...
    socket.addEventListener('open', () => {
      if(!initial && toastId) toast.info('Connected to the server', { id: toastId });
    });
