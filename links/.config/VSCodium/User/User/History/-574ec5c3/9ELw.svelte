<script lang="ts">
  ...
  import { type LogLevel as LogLevelType, type Log, type ServerStatistics, type Server } from '@shared/types';
  ...
  let logStatisticsByDay: { series: { name: string; data: number[] }[]; categories: string[]; } | null = $state(null);
  let logsOverviewStatistics: ({ [key in LogLevelType]: number } & { recentLogs: Log[]; total: number })
    | undefined = $state();
  let serversStatuses: ({
    server: Server;
    online: boolean;
  ...
  <div class="grid gap-4 md:gap-8 xl:grid-cols-3">
                ...
                {:else}
                  {#each logsOverviewStatistics.recentLogs as log}
                    <Table.Row>
                      ...
                      <Table.Cell><p class="line-clamp-1">{log.logMessage} Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis in voluptates inventore suscipit quo nesciunt, numquam at eveniet repellendus necessitatibus eius corrupti ex architecto nostrum, culpa tenetur qui quos animi.</p></Table.Cell>
                    </Table.Row>
                  {/each}
                {/if}
              {:else}
    ...
    {#if hasPermission(user?.role, PERMISSIONS.READ_SERVER)}
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description
            >Your monitored servers statuses.</Card.Description
          >
        </Card.Header>
        <!-- TODO: Fix this fixed max height -->
        <Card.Content class="flex flex-col gap-4 overflow-y-auto max-h-[626px] no-scrollbar">

          {#if serversStatuses}
            {#each serversStatuses as serverStatus}
              <a href="/app/servers/{serverStatus.server.id}" class="flex flex-row items-center gap-4">
                    ...
                    <Tooltip.Trigger>
                      <div class="relative size-10">
                        ...
                        class={cn(
                          ...
                          serverStatus.online ? 'bg-green-500 dark:bg-green-600' : 'bg-red-400 dark:bg-red-600'
                        )}
                        ></div>
                        <div class={cn(
                          'absolute inset-0 z-10 p-2 rounded-full',
                          serverStatus.online ? 'bg-green-500 dark:bg-green-600' : 'bg-red-400 dark:bg-red-600'
                        )}>
                ...
                <div class="flex flex-col gap-1">
                  <p class="text-primary font-semibold">{serverStatus.server.name}</p>
                  {#if serverStatus.server.description}
                    <span class="text-sm text-muted-foreground line-clamp-1">{serverStatus.server.description}</span>
                  {/if}
                </div>
              </a>
            {/each}
    ...
    <Card.Root class="xl:col-span-2">
      <Card.Header>
        ...
        <Card.Description
          >Logs analytics.</Card.Description
        >
      </Card.Header>
      <Card.Content>
