...
{#if outfitNameModalVisible}
  <div class="inset fixed z-30 bg-neutral-900/50" transition:fade></div>
  <div class="fixed inset-0 flex flex-col items-center justify-center"></div>
{/if}
...
{#if selectedItems.length > 0}
  <div class="fixed bottom-24 left-0 right-0" transition:fly={{ y:'50%' }}>
    <form
      method="POST"
      action='?/createOutfit'
      class="max-w-screen-md mx-auto w-full"
      use:enhance={(e) => {
    ...
    >
      <Card class="ml-auto w-fit">
        ...
        <Button loading={isCreatingOutfit}>Create outfit</Button>
      </Card>
    </form>
  </div>
{/if}
