...
# Set password in SQL file
sqlFilePath="$(dirname "$0")/backend/database.sql"
initSQLFilePath="$(dirname "$0")/backend/init.sql"
touch $initSQLFilePath
cp $sqlFilePath $initSQLFilePath
sed -i "s/{{MYSQL_PASSWORD}}/$MYSQL_PASSWORD/g" $initSQLFilePath

# END - ENV setting

# Start the docker containers
curl -s https://raw.githubusercontent.com/ownlogs/ownlogs/main/docker-compose.yaml -o docker-compose.yaml
dockerfileLocation="$(dirname "$0")/docker-compose.yaml"
docker compose -f $dockerfileLocation up

# Remove the init.sql file
rm $initSQLFilePath
