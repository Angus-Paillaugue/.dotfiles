...
public class AssuranceDAO implements AssuranceDAODef {
  ...
  public Assurance create(Assurance assurance) {
    try {
      ...
      String query = "INSERT INTO Assurance (numeroContrat, typeContrat, nomAssureur, prime) VALUES (?, ?, ?, ?)";
      db.executeUpdate(
          query,
          Arrays.asList(
  ...
  public Assurance update(Assurance assurance, int numeroContractOldAssurance) {
    try {
      Connector db = Connector.getInstance();
      String query = "UPDATE Assurance SET codePostal = ?, complement = ?, rue = ?, numero = ?, ville = ? WHERE id = ?";
