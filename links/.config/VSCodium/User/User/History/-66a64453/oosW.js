 ...
 */
function urlEndsWith(url, path) {
	if (!url || !path) return false;
	if (path instanceof Array) return path.some((p) => urlEndsWith(url, p));
	// For the `/` path
	if (path.length === 1) return url.at(-1) === path;

	return url.endsWith(path) || url.endsWith(path + '/');
}
...
const originalHandle = async ({ event, resolve }) => {
	...
	response.headers.set(
		'X-Robots-Tag',
		urlEndsWith(url.pathname, PROTECTED_ROUTES) ? 'noindex, nofollow' : 'index, follow'
	);
