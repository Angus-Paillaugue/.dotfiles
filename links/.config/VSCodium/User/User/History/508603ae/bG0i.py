...
class Logger:
    ...
    def debug(self, *messages: Any):
      ...
      print(f"{self.COLORS['bgWhite']}{self.COLORS['black']}[ DEBUG ]{self.COLORS['reset']} {message}")
      self.push_log({'level': 'debug', 'message': message, 'source': source, 'timestamp': datetime.now()})

    def info(self, *messages: Any):
      message = self.format_message(*messages)
      source = self.get_caller_info()
      print(f"{self.COLORS['bgBlue']}[ INFO ]{self.COLORS['reset']} {message}")
      self.push_log({'level': 'info', 'message': message, 'source': source, 'timestamp': datetime.now()})

    def warn(self, *messages: Any):
      message = self.format_message(*messages)
      source = self.get_caller_info()
      print(f"{self.COLORS['bgYellow']}[ WARN ]{self.COLORS['reset']} {message}")
      self.send_log_immediately({'level': 'warn', 'message': message, 'source': source, 'timestamp': datetime.now()})

    def error(self, *messages: Any):
      message = self.format_message(*messages)
      source = self.get_caller_info()
      print(f"{self.COLORS['bgRed']}[ ERROR ]{self.COLORS['reset']} {message}")
      self.send_log_immediately({'level': 'error', 'message': message, 'source': source, 'timestamp': datetime.now()})

    def fatal(self, *messages: Any):
        message = self.format_message(*messages)
        source = self.get_caller_info()
        print(f"{self.COLORS['red']}[ FATAL ]{self.COLORS['reset']} {message}")
        self.send_log_immediately({'level': 'fatal', 'message': message, 'source': source, 'timestamp': datetime.now()})
    ...
    def send_log_immediately(self, log: Dict[str, Any]):
        log['serverId'] = self.server_id
        try:
            response = requests.post(f"{self.backend_url}/logs", headers={'Content-Type': 'application/json'}, data=json.dumps([log]))
    ...
    def flush(self):
        ...
        try:
            while self.buffer:
                ...
                try:
                    response = requests.post(f"{self.backend_url}/logs", headers={'Content-Type': 'application/json'}, data=json.dumps(logs_to_send))
                    response.raise_for_status()
                except requests.RequestException as error:
