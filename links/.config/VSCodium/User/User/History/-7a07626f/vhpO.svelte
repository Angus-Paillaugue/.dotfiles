<script lang="ts">
  ...
  const rowClasses = new Map<LogLevel, string>([
    ['info' as LogLevel, 'bg-blue-600 text-blue-500'],
    ['warn' as LogLevel, 'bg-amber-600 text-amber-500'],
    ['error' as LogLevel, 'bg-red-600 text-red-500'],
    ['fatal' as LogLevel, 'bg-rose-600 text-rose-500'],
  ]);
</script>

<div class={cn("rounded-lg flex items-center bg-opacity-10 flex-row px-2 py-1", className, rowClasses.get(log.level))}>
